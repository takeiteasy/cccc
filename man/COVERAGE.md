...
| `sizeof`/`_Alignof` of a `from_include` type, reached through a context other than a bare expression node — array dimensions (`char buf[sizeof(struct statfs)]`), `_Static_assert`, `case` labels, bitfield widths, enum values, a global initializer's byte image | folds against CCCC's own (correct-for-the-VM) type projection | a bare `sizeof(T)`/`_Alignof(T)` *expression* re-materializes the operator textually against the real host layout once member access re-resolves against it (#1031); every other context above consumes `const_expr()`/`eval()` and discards the node before serialization ever sees it, so the folded literal — CCCC's own, possibly smaller, projection — stays stale. Residual of #1031, not yet attempted |
