...
behaves as the VM behaves. Twelve constructs cannot fully honour that, and
...
| `sizeof`/`_Alignof` of a `from_include` **scalar** typedef whose real host size genuinely differs from CCCC's own (e.g. `sigset_t`: `unsigned int`/4 bytes here, a 128-byte type in glibc) | folds against CCCC's own (possibly wrong-for-this-host) projection | stays folded — not re-materialized. `type_layout_is_host_owned()` (`src/serialize_type.c`) only recognizes `TY_STRUCT`/`TY_UNION`/`TY_ENUM` as host-owned (#1168, see below); a scalar typedef's `Type` is origin-identical to its underlying builtin (`typedef unsigned int sigset_t;` shares the same `Type` object `copy_type()`'s `origin` field points back to), so no identity check on the folded `Type` can distinguish `sizeof(sigset_t)` from `sizeof(unsigned int)` once parsing has already folded either down to a bare `Type` — unlike the struct/union/enum case, there is no separate provenance signal left to gate on. `time_t`/`size_t`/`off_t`/`pid_t` are unaffected (uniform layout on every supported target). Tracked as an open residual, not fixed here (#1169) |
