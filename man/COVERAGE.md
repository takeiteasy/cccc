...
| `<stdarg.h>` | ✓ | CCCC-specific implementation for the VM calling convention: `va_list` is a struct tracking position in the register-spill and stack-overflow regions, and `va_start`/`va_arg` expand to VM-ABI pointer arithmetic over `__builtin_frame_address(0)`. `struct va_list`'s real fields total 24 bytes but the type is padded to 64 (a trailing `char __reserved[40]`), sized to cover every supported host's own real `va_list` too (measured: macOS arm64 8B, macOS x86_64 24B, glibc x86_64 32B, glibc aarch64 32B) so a guest-folded `sizeof(va_list)`/`offsetof` stays correct even though `-c=native` replays the user's own `#include <stdarg.h>` verbatim, which resolves to the real host header at native-compile time (#1059, same soundness class as `<setjmp.h>`'s `jmp_buf` below). **Known limitation ([#1018](https://todo.sr.ht/~takeiteasy/cccc/1018)):** unlike `<setjmp.h>`, the `va_start`/`va_arg`/`va_copy`/`va_end` expansion itself is not yet translated to the host's own `stdarg.h` forms under `-c=native` — CCCC's VM-ABI expansion is what the serializer prints, so a variadic function *definition* (not a call to an already-declared host variadic function like `printf`) fails to compile natively with "member reference base type 'va_list' (aka '...') is not a structure or union" |
| `<stddef.h>` | ✓ | |
| `<stdio.h>` | ✓ | |
| `<stdlib.h>` | ✓ | |
