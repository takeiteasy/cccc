...
| Improved `enum` — underlying type, forward declaration, wide values | ✓ | `enum E : unsigned char { … }` sets size/align/signedness; `enum E : int;` forward-declares; values stored as `int64_t` (C23 §6.7.2.2). A plain enum with no `: type` of its own now also *selects* its underlying type from the enumerator values actually seen, matching gcc-16/clang (an extension predating C23 §6.7.2.2's "must represent every enumerator" requirement, [#1175](https://todo.sr.ht/~takeiteasy/cccc/1175)): any negative enumerator plus one past `INT32_MAX`/before `INT32_MIN` widens to `long long` (8/8, signed); an all-non-negative enum with a value past `INT32_MAX` stays 4 bytes but becomes `unsigned int` if it still fits `UINT32_MAX`, else widens to `unsigned long` (8/8). One documented divergence: a *small* all-non-negative enum (every value fits `INT32_MAX`) is `unsigned int` on gcc/clang but stays CCCC's plain signed `int` — `sizeof`/`_Alignof` agree either way, so it's outside the layout-parity bar this fix targets, deliberately not folded in |
...
| `<stddef.h>` | ✓ | |
| `<stdio.h>` | ✓ | `popen`/`pclose`, `fseeko`/`ftello`, `flockfile`/`ftrylockfile`/`funlockfile`, and `getc_unlocked`/`getchar_unlocked`/`putc_unlocked`/`putchar_unlocked` are registered VM cfuncs (`src/stdlib/stdio.c`) that had no declaration in this header at all, found (like `<string.h>`'s `strspn`/`strcspn`/`strcoll` below, #1042) auditing every `cc_register_cfunc` name against every bundled header's own declarations once implicit function declaration became a hard error at C99+/`-c=native` (#1144). Fixed |
...
`--posix-emulation` -- see their `COVERAGE.md` entries below and #824.
...
| `<dlfcn.h>` | ✓ | VM-managed dynamic loading (`dlopen`, `dlsym`, `dlclose`, `dlerror`); `dlsym` function symbols are callable through typed function pointers for scalar/pointer signatures. `dlclose` refuses to close a handle with any still-"live" `dlsym`'d symbol (`live_symbol_count`, `src/vm.c`) -- and that count is never decremented, so once a handle has had any symbol resolved through it, it can never be closed. Serializes under `-c=native`/`-m`/`-c=generated` too (#1105) via a registry shim reproducing this exact policy rather than forwarding to the host's libdl -- see [Serialized-output divergences](#serialized-output-divergences) for the shim's shape. `include/dlfcn.h`'s `RTLD_*` values are derived from the real host `<dlfcn.h>` this `cccc` binary was built against rather than hand-transcribed, so `mode` being passed through to the host `dlopen()` unchanged is correct on every platform, VM and native alike. The `dlsym` pseudo-handles (`RTLD_NEXT`/`RTLD_DEFAULT`/`RTLD_SELF`/`RTLD_MAIN_ONLY`) are deliberately not provided, since `dlsym`'s handle argument is resolved through CCCC's own registry, not a raw host handle |
