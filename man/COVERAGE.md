...
| `ppoll`; `sched_setparam`/`getparam`/`setscheduler`/`getscheduler`/`rr_get_interval` (`--posix-emulation`); `gethostbyname_r`/`gethostbyaddr_r`/`getnetbyname_r` (ungated) on a host with no real primitive (macOS for all of these) | the VM's own emulation/shim/stub (`src/stdlib/posix_poll.c`/`posix_sched.c`/`posix_net.c`) | `-c=native` (#1140) ports the same emulation/shim/stub into the emitted C via `serialize_posix_compat_shims` (`src/serialize.c`), guarded `#if !defined(__linux__)` so a real host symbol is always preferred where one exists — parity, not a divergence, for the VM behaviour itself. Two residuals remain, both accepted rather than fixed: `ppoll`'s emulation is not atomic (a signal between the mask swap and `poll()`'s wait is not guaranteed to interrupt it, same as the VM); and the emitted `gethostbyname_r`/`gethostbyaddr_r`/`getnetbyname_r` mutex only serializes against each other, not against a concurrent plain `gethostbyname()`/`gethostbyaddr()`/`getnetbyname()` call in the same program (unlike the VM, where one shared mutex covers both families) — see #1146. Separately, and not part of this shim: CCCC's canonical constant numbering (`POLLWRNORM`/`POLLWRBAND`, `nl_item`) is not translated to the host's real values under `-c=native` at all, for `ppoll`/`poll()` and `nl_langinfo()`/`nl_langinfo_l()` alike — also #1146 |
