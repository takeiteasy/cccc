...
| `L` length modifier on an integer conversion (`%Ld`/`%Li`/`%Lu`/`%Lo`/`%Lx`, `printf` and `scanf`) | the VM formatter follows glibc: `L` == `ll`, a full 64-bit slot (#1228) | the host libc decides. glibc agrees with the VM; BSD/Apple libc treats `L` as no-op on an integer conversion and reads/writes a 32-bit slot — `L` on `d` etc. is undefined behaviour in ISO C, so neither is "wrong". Not serializer-fixable: the emitted call site is verbatim guest source handed to the host's own `printf`/`scanf`. Coverage is `tests/test_printf_L_integer_modifier.c`, `CCCC_NATIVE_SKIP` |
...
| `__builtin_types_compatible_p(t1, t2)` | `1` if types `t1` and `t2` are compatible. Top-level qualifiers are ignored (`int` / `const int` → `1`); qualifiers below the top level are honored (`char *` / `const char *` → `0`, `char **` / `const char **` → `0`). `_Atomic` is treated as a non-qualifier (`_Atomic int` / `int` → `1`), following GCC; clang reports `0` there — a clang-family policy switch is a possible follow-up (same shape as the `--bitfield-abi` proposal). |
