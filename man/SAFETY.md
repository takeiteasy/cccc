...
*deeper* than that, including
through an interior pointer with a runtime-computed offset (e.g. `&arr[i]`
for non-constant `i`) whose owning array/struct escapes its frame
...
- Compiler-internal automatic storage -- a VLA's backing storage (`ALCV`
  ...
  deliberately excluded from this list: each still gets a full
  ...
  kind) exists because heap reclamation below) treats them differently:
...
- Resolves **interior pointers** (`p = q + k`) back to their containing
  ...
  `__builtin_dynamic_object_size`), not just exact base pointers, so
  a UAF reached through an interior pointer is caught too
...
- Split into two checks: `CHKB`/`CHKBN` (formation-time —
  "pointer *subscript* addition"/"pointer subtraction") validate a
...
- Resolves **interior pointers** (`p = q + k`) back to their containing
  ...
  *resolved allocation's* start, so `p[-1]` on an interior pointer that
  stays within the allocation is valid
- A pointer **difference** (`&a - &b`, result type `ptrdiff_t`/`long`)
  ...
  allocation's size would be checking an unrelated value
- **Formation vs. dereference**: forming a one-past-the-end
...
- **Atomic ops**: `CHKD` is also emitted ahead of the four atomic
  ...
  register-aliasing hazard; a standalone `CHKD` instruction
...
### `--checked-pointers` — Checked-pointer bounds checking
...
```c
// test_checked_nt.c - Checked-pointer terminator-invariant example
int main(void) {
    int n = 3;
...
- Resolves the allocation via the same `vm->sorted_allocs` binary search
  as `--bounds-checks`/`--uaf-detection`, so it works
...
- Uniform coverage: the VM has no optimiser, so every dereference takes
  ...
  optimiser, so there is no longer an opt-level-dependent gap to
  guard against.
...
- **Address-based locals are exempt from the CHKI check**: a
  struct/union/array/wide-`_BitInt`/`_Decimal` local was already exempt. The same exemption now
...
### `--pointer-sanitizer` — Comprehensive pointer checking
...
- **Mixed atomic/non-atomic access detection**: `atomic_load`/`atomic_store` (and `atomic_load_explicit`/`atomic_store_explicit`) emit dedicated `ALDR`/`ASTR` opcodes that tag a per-address `atomic_shadow` map with the accessing thread id. If a second thread performs a plain (non-atomic) memory access to the same address without holding a mutex, a diagnostic is printed at runtime. Forward-only detection: the atomic access must occur before the non-atomic one to be detected.
  ...
  ```
  `atomic_fetch_add`/`sub`/`or`/`xor`/`and` route through a CAS retry loop over the same `ACAS` opcode `atomic_compare_exchange_*` uses, so they set the atomic tag exactly as `atomic_load`/`atomic_store` do — mixing `atomic_fetch_*` with `atomic_store`/`atomic_load` on the same address across threads is correctly tracked, not a known limitation.
...
- Enforced in `CHKP3`, the same gate that runs before every pointer
  ...
  escape analysis only prunes a redundant recording, see below):
  1. **Range check.** The VM stack grows downward, so every *live*
  ...
  2. **Frame-epoch liveness.** Every activation gets a monotonic
     ...
     catches it. Since, a dead exact tag no longer concludes dangling
     ...
     prefer-live arbitration layer 3 already had for, now shared by
     layer 2
  3. **Interior interval-stabbing.** An interior stack pointer with a
     ...
     *final* dereferenced address is never itself the one recorded in
     ...
     whether escape analysis proved it escaping: element access
     ...
     `vm->stack_intervals`. Unlike `sorted_allocs` (the heap analogue) — a bump allocator whose base addresses are globally ordered and
     ...
     containing interval** — only when every containing interval is
...
- **Local aggregate member/element access is not gated on `CHKP3` at all.** A local struct/union's own member access (`t.a`) lowers
  ...
  **Related layer-2 exact-tag collision, resolved separately.**
  ...
  exact tag in `stack_ptr_epochs`. Rather than skip `CHKP3` at these sites fixed the collision at its source: layer 2 now falls through to
...
- **Second consumer:** the epoch/interval bookkeeping above
  ...
  its own, independently of `-1`/`-2`/`-3` — see [STDLIB.md](STDLIB.md)'s
  `__builtin_dynamic_object_size` entry.
- **Lazy per-function push:** activation is scoped to the functions
  ...
  *current top* epoch. A function with no escaping local/param of its own
  ...
  on every call in the program — the same shape of win as pruning
  `LEA3` recording, but for the `ENT3`/`LEV3` push/pop itself (lazy
  per-function activation).
- Recording is pruned to addresses that provably *escape* their creating
  frame — a post-parse pass (`mark_addr_escapes` in `src/parse_analysis.c`)
  ...
  arithmetic on that base such as `arr + i`) is observed as a call
  ...
  from the earlier escape-analysis mistake below: an **unmarked** local
...
- This replaces an earlier scope-exit check, which
...
dereference-time checks in `CHKP3`: a range check and a per-frame
epoch liveness check.
...
already returned — that's the range check firing above, the cheap
common case.

**The deeper-call case:** passing the dangling pointer one call
...
Dereferenced a pointer into a stack frame that has already returned
Address:    0x31fffffc8
Creating frame's epoch is no longer live (dereferenced through a deeper call)
...
**The interior-pointer case:** an interior stack pointer with a
...
used to rely on the range check alone (and could be missed the same way
...
void use(int *p) {
    ...
    pad[0] = 0;
    int y = *p; // NOT caught by the range check alone; caught by.
    (void)y;
    (void)pad;
}
...
Dereferenced a pointer into a stack frame that has already returned
Address:    0x167ffffb4
Creating frame's epoch is no longer live (dereferenced through an interior pointer into a deeper call)
...
**Discarded-value dereferences are still checked:** a dereference
...
- **Note on detection coverage:** `CHKL` is a liveness *guard*, not a
  ...
  no longer fires in correct programs after; catching a genuine
...
- **Limitation:** side table is keyed by address; if freed memory is reallocated at the same
  ...
  active* — `--memory-tagging` is one of the exact features that keeps heap reclamation below) permanently disabled, precisely
...
as of this flag *disables* the VM heap (it's on by default) and cannot be combined with
`-1`/`-2`/`-3` or `--safety=basic/standard/max`, nor (as of) with any individual flag
...
silent no-op — before, `-V --bounds-checks` compiled and ran with the requested check
...
A Checked C-inspired spatial-safety layer: a pointer's
...
| `ntarray` | Like `array`, but widens the checked range by one element (`sizeof(T)` bytes at the declared end of the range) for a null-terminator slot, under all three bounds forms; writing that slot is permitted **only with a null value** |
...
**Bounds declarations** can reference any other in-scope
...
**Bounds propagation across assignment**. `q = p + k;` on its own
...
- **FULL** — **every** assignment to `q` reached in the function is
  ...
  to a declared checked pointer variable or a member access with an
  enforceable bounds form). `q[i]` is checked against a snapshot taken at
  the most recent assignment, unconditionally — the original
  rule, unchanged.
- **OPT** — a *mix* of checked-rooted and non-checked-rooted
...
s = p;     // s's OWN initializer wasn't checked-rooted -- s is OPT, not
           // FULL (see below), NOT poisoned to NONE the way alone
           // would have left it
```

**Path-sensitive propagation via runtime snapshot validity**. An
...
int *r;    // no initializer -- still a candidate
if (c) r = p;
...
plain `CHKR` — so its codegen is byte-for-byte identical to before.

A member is a valid propagation *source* too, so the two features
...
**Chained propagation**. A local that is itself only propagated (never
...
(today's rule); each later round additionally trusts whichever
...
Chaining needs one more soundness argument beyond's own: a chained
...
→ sentinel, per above) plus at function entry, so `q`'s temps are always
...
**OPT-ness also threads through the chain**: a candidate chained from an
...
**`CHKNT` propagation**. The terminator-slot fact travels alongside
...
it — declared source directly, or transitively through a chain — is
...
one (matching's own direct-access split, which has no `lo` to give the
CAS-loop's `ND_CAS` node) — onto the mirror once they're known.

**Assignment-time bounds implication**. The propagation pass above only ever
...
whose RHS is rooted at a **directly declared-checked** source (not a
...
is `(temp = source bounds), (q = E)`, mirroring's own before-the-store
...
- The RHS must be a **directly declared-checked** source
  (`checked_prop_source_bounds()`'s kind 1) — a-propagated local can
...
callers), so native builds compile and run declarations
...
arrays that `--bounds-checks` structurally cannot (it is enforced by the
VM's `CHKR` opcode on every checked dereference).
...
the access (`addr == hi - elem_size && val != 0`); `CHKNTZ` traps a
...
**Which pointee types are guarded**:
...
Read-modify-write on the terminator slot is covered as directly as
...
**`CHKNT`/`CHKNTZ` propagate across assignment**. `q =
s;` where `s` is an `[[cccc::ntarray]]` source makes `q[n] = 'x'` trap
through `q` exactly the same way `s[n] = 'x'` traps directly — FULL, OPT,
and chained propagation candidates alike, through the
read-modify-write and `_Atomic` compare-exchange desugars, and for
...
- **A member-wise write into a `struct`/`union` terminator slot is
  unguarded**. `CHKNTZ` only ever sees a whole-object store
...
(`ALCA`/`ALCV` opcodes) is the one exception — see **Heap Reclamation** below.

#### Heap Reclamation

`alloca`/VLA storage would otherwise grow the VM heap without bound in a
long-running loop or recursive function (the symptom was filed for),
since already excludes it from the `malloc`/`free` leak-tracking model
...
The opcode-level mechanics (`HMRK`/`HREL`, the `ALCA`/`ALCV`/`ALCB` split,
`heap_rewind_to`'s pinning behavior, and the `CALLT`/`longjmp` interaction)
live with the VM internals.
...
simply keeps the the earlier unbounded-growth behavior. Tracked as a
follow-up.
...
- `aligned_alloc`/`posix_memalign` are intercepted the same way as `malloc`/`calloc`, via
