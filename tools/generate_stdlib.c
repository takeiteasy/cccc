// generate_stdlib.c - builds get_std_header as AST; use -c=generated to emit C.
...
Node *make_strcmp_return(Obj *fn, Type *char_ptr_ty, const char *header) {
    ...
    Node *args[2] = {MakeParamRef(fn, "filename"), MakeStringLiteral(header)};
...
Node *make_strcmp_return_fn_name(Obj *fn, const char *header,
                                 const char *fn_name) {
    Node *args[2] = {MakeParamRef(fn, "header"), MakeStringLiteral(header)};
    Node *cmp     = MakeFuncCall(MakeVarRef("strcmp"), args, 2);
    Node *cond    = MakeBinary(NK_EQ, cmp, MakeIntLiteral(0));
...
void generate_stdlib_reg_fn(void) {
    ...
    WithFn(fn) {
        ...
        for (int i = 0; headers[i]; i++) {
            if (reg_fn_for_header(headers[i]))
                total++;
        }
        if (!total) {
        ...
        for (int i = 0; headers[i]; i++) {
            ...
            for (int j = 0; headers[j]; j++) {
                ...
                if (!fn_name)
                    continue;
                case_stmts[cn++] =
                    make_strcmp_return_fn_name(fn, headers[j], fn_name);
            }
...
void generate_std_header(void) {
    ...
    WithFn(fn) {
        ...
        for (int i = 0; headers[i]; i++) {
            ...
            for (int j = 0; headers[j]; j++) {
                if ((unsigned char)headers[j][0] == c) {
                    if (read_header_file(header_source_path(headers[j])))
                        bucket_size++;
                }
            }
            if (bucket_size == 0)
            ...
            for (int j = 0; headers[j]; j++) {
                ...
                if (!read_header_file(header_source_path(headers[j])))
                    continue;
                case_stmts[cn++] =
                    make_strcmp_return(fn, char_ptr_ty, headers[j]);
            }
            case_stmts[cn++] = MakeReturn(MakeIntLiteral(0));
...
void generate_stdlib_mark_headers(void) {
    ...
    WithFn(fn) {
        ...
        for (int n = 0; headers[n]; n++) {
            if (read_header_file(header_source_path(headers[n])))
                total++;
        }
        if (!total) {
            FunctionSetBody(fn, MakeReturn(MakeIntLiteral(0)));
        ...
        for (int n = 0; headers[n]; n++) {
            if (!read_header_file(header_source_path(headers[n])))
                continue;
            SwitchAddCase(sw, MakeIntLiteral(idx),
                          MakeReturn(MakeStringLiteral(headers[n])));
            idx++;
        }
