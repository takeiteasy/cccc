...
// reflection.h, testing.h and cccc_build.h are private headers. Keep embedding
...
const char *header_source_path(const char *header) {
    ...
    if (strcmp(header, "cccc_build.h") == 0)
        return "include/cccc/cccc_build.h";
