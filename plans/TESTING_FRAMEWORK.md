...
`[[jcc::comptime]]` and the planned `[[jcc::build]]` / `[[jcc::build_target]]`
...
file (replace `int main()` with `[[jcc::test]] int test_<filename>()`,
replace `EXPECT_*` comments with `[[jcc::negative_test(...)]]`).
...
- **One binary, one command** — `./jcc --testing tests.c` is the only
...
- **JCC options live in source** — test files declare their
  VM-configuration needs (safety level, debug, profiling) via
  `#pragma jcc ...`. The runner collects pragmas from each test
...
- **Setup/teardown fixtures.** Test functions are plain C functions.
  ...
  a `[jcc::before_all]` / `[jcc::after_all]` pair on a suite (v2).

## CLI

```
./jcc --testing tests.c                       # run every test in tests.c
./jcc --testing tests.c --test-suite=pointers # filter to one suite
./jcc --testing tests.c --test=pointer_*      # filter to a name glob
./jcc --testing tests.c --list                # list, don't run
./jcc --testing tests.c --format=tap          # TAP (default)
./jcc --testing tests.c --format=json         # machine-readable
./jcc --testing tests.c --format=plain        # original human format
./jcc --testing tests.c -v                    # show each assertion
./jcc --testing tests.c --fail-fast           # stop on first failure
./jcc --testing tests.c --test-target=vm      # run only VM-mode tests
./jcc --testing tests.c --test-target=native  # run only native-build tests
./jcc --testing tests.c --test-target=both    # run only "both" tests
./jcc --testing tests.c --test-target=all     # run all (default)
./jcc --testing tests.c -j8                   # parallel (future)
./jcc --testing tests.c --seed=0xC0FFEE       # deterministic seeding
```
...
`[[jcc::comptime]]` is — they never reach the general attribute
parser.
...
int test_fortytwo(void) {
    return 42;  // JCC convention: a program "passes" by returning 42
...
| Form | Meaning |
|------|---------|
| `[[jcc::test]]` | Plain test, runs in the VM (default). |
| `[[jcc::test(mode = "vm")]]` | Same as the plain form. |
| `[[jcc::test(mode = "native")]]` | The test is built natively (`-c=native`) and run as a child process. The C source must compile as a standalone program with `main()`. |
| `[[jcc::test(mode = "both")]]` | The test is run twice — once in the VM, once built natively. Both must pass. This is the "I want to make sure this works in both worlds" mode. |
| `[[jcc::test(suite = "name")]]` | Test belongs to suite `name`. |
| `[[jcc::test(name = "foo")]]` | Override the auto-derived test name. |
| `[[jcc::test(flags = "--foo")]]` | Extra JCC flags applied when this test runs in the VM (e.g. `--bounds-checks`, `--optimize=2`). For native tests, forwarded to the underlying `cc`. |
| `[[jcc::test(timeout = 1000)]]` | Reserved. v1 ignores. |
...
[JCC Pragma Extensions](#jcc-pragma-extensions) is the
source-driven way to express JCC options; the `flags = "..."` form
...
| Form | Meaning |
|------|---------|
| `[[jcc::negative_test(compile)]]` | The function body must fail to compile. |
| `[[jcc::negative_test(runtime)]]` | The function compiles; running it must fail. |
| `[[jcc::negative_test(compile, msg = "...")]]` | The compile error output must contain `...`. |
| `[[jcc::negative_test(runtime, code = N)]]` | The runtime exit code must be `N`. Default: any non-zero. |

Compile-mode negative tests are compiled in a child JCC process with
...
```c
[[jcc::suite("pointers")]]
void pointers_suite(jcc_test_ctx_t *ctx) {
    JCC_RUN_TEST(ctx, test_pointer_basic);
    JCC_RUN_TEST(ctx, test_pointer_advanced);
}
```
...
suite. Identified the same way `[[jcc::build]]` is:

1. `--test-entry=NAME` (highest precedence)
2. `[[jcc::test_main]]` attribute (must be unique)
...
```c
[[jcc::test]]                       // C23
__attribute__((jcc::test))          // GNU namespace
__attribute__((test))               // GNU short form (jcc namespace prefix is optional)
```
...
`#pragma jcc ...`. The pragma is the source-level counterpart to the
CLI flag it mirrors.
...
had invoked `./jcc --testing -2 tests.c`, but the source itself
states the requirement.

Full list of pragmas is in
[BUILD_MODE.md → JCC Pragma Extensions](BUILD_MODE.md#jcc-pragma-extensions).
...
earlier ones. **[Q: do we want `#pragma jcc push` /
...
any JCC-specific option it would have needed in the VM half
...
// Suite descriptor
[[jcc::suite("basic")]]
void basic_suite(jcc_test_ctx_t *ctx) {
    JCC_RUN_TEST(ctx, test_simple);
    JCC_RUN_TEST(ctx, test_undeclared);
    JCC_RUN_TEST(ctx, test_segfault);
}

// Test main
[[jcc::test_main]]
int test_main(jcc_test_ctx_t *ctx) {
    return jcc_run_all_tests(ctx);
}
```
...
JCC_ASSERT_GT(a, b)
JCC_ASSERT_GE(a, b)
JCC_ASSERT_STREQ(a, b)
JCC_ASSERT_NOT_NULL(p)
...
test that calls `JCC_ASSERT` does not abort its sibling tests.
...
#define JCC_ASSERT(cond) \
    do { \
        if (!(cond)) { \
            __jcc_test_record_failure(__FILE__, __LINE__, #cond); \
            return 1; \
        } \
    } while (0)
```
...
the rest of JCC's "no hidden language features" philosophy. A
...
|------|---------------|----------------|
| `vm` (default) | JCC VM, in-process with the runner | Compiled by the runner; pragmas applied. |
| `native` | A child process forked by the runner | Compiled with `cc` via the same `-c=native` path the build system uses; JCC pragmas are ignored. |
...
`./jcc test.c` runs them in the VM). Migration to native-mode is
...
[JCC Pragma Extensions](#jcc-pragma-extensions)). For native-mode
tests, pragmas are **ignored** — the system toolchain has no
concept of `-2` or `--vm-profile`. The `flags = "..."` form on
`[[jcc::test]]` is the way to pass extra flags to either side.
...
```c
typedef struct jcc_test_runner jcc_test_runner_t;

// vtable
typedef struct {
    int (*run)(jcc_test_runner_t *r, jcc_test_ctx_t *ctx, jcc_test_t *t);
    void (*discover)(jcc_test_runner_t *r, jcc_test_ctx_t *ctx);
} jcc_test_runner_ops_t;

jcc_test_runner_t *jcc_runner_in_vm(void);      // v1: positive tests
jcc_test_runner_t *jcc_runner_native(void);     // v1: native-mode tests
jcc_test_runner_t *jcc_runner_per_test(void);    // future
jcc_test_runner_t *jcc_runner_per_suite(void);   // future
jcc_test_runner_t *jcc_runner_hybrid(void);      // future
```
...
./jcc --testing tests.c
   ...
   │  2. Parse; jcc_test/jcc_negative_test attrs  │
   │     are intercepted by the preprocessor      │
   │  3. Find test_main (attr/name/flag)          │
   │  4. Compile test_main into the JCC VM        │
   │  5. Discover all [[jcc::test]] and           │
   │     [[jcc::negative_test]] functions by      │
   │     scanning the program list, partitioned   │
   │     by mode (vm / native / both)             │
   │  6. Invoke test_main inside the VM           │
   │     ├─ test_main calls jcc_run_all_tests    │
   │     │   (or jcc_run_suite / jcc_run_matching)│
   ...
   │     │   - fork a child JCC process with the  │
...
The runner forks a child JCC process and asks it to compile just the
...
1. Writes a tiny C file that `#include`s the test source with a
   ...
   visible.
2. Invokes `./jcc --no-run -I./include tmp.c`.
...
```c
#ifdef JCC_NEGATIVE_COMPILE_test_undeclared
[[jcc::negative_test(compile)]]
int test_undeclared(void) {
...
For the file's normal `tests.c` compilation, `JCC_NEGATIVE_COMPILE_*`
...
The runner forks `./jcc --no-stdlib -I./include tmp.c` and runs it.
...
The user can run this file directly: `./jcc --testing
tests/test_pointer_basic.c`.
...
contains one or more `[[jcc::test]]` (or `[[jcc::negative_test]]`)
...
|----------------|-----------|
| `int main(void) { return 42; }` | `[[jcc::test]] int test_<filename>(void) { return 0; }` |
| `int main(void) { ... return 42; }` (positive, complex body) | `[[jcc::test]] int test_<filename>(void) { ... return 0; }` |
| `/* EXPECT_COMPILE_ERROR */` header + `int main(void) { ... }` | `[[jcc::negative_test(compile)]] int test_<filename>(void) { ... }` |
| `/* EXPECT_RUNTIME_ERROR */` header + `int main(void) { ... }` | `[[jcc::negative_test(runtime)]] int test_<filename>(void) { ... }` |
| `// JCC_FLAGS: --foo` in header | `[[jcc::test(flags = "--foo")]]` (or `[[jcc::negative_test(compile, flags = "--foo")]]`) — runner injects the flag when running this test |
| `JCC_EXPECT_STDERR: ...` / `JCC_REJECT_STDERR: ...` | `[[jcc::test(stderr_match = "...")]]` / `[[jcc::test(stderr_no_match = "...")]]` |
| Multiple test functions in one file | Each gets its own `[[jcc::test]]` attribute. Names come from the function names. |
...
declares a single `[[jcc::test_main]]` function.

### Fallback: keep `tools/tests.py` as a wrapper

During the migration, `make test` (and the eventual `jcc build`)
should call `./jcc --testing tests.c` for the new path but keep
...
`test` target is `./jcc --testing tests.c` invoked from inside
the build entry.
...
**[Q:2]** Should `JCC_ASSERT` early-return from the *test* or from
...
**[Q:3]** Should the existing `JCC_FLAGS: ...` per-test flags be
preserved as `[[jcc::test(flags = "...")]]`? **Resolved:** yes.
The migration uses `flags = "..."` on the attribute for the
short term, with the pragma mechanism ([JCC Pragma
Extensions](#jcc-pragma-extensions)) as the recommended
long-term form for JCC-specific options.

**[Q:4]** How should multiple `[[jcc::test]]` functions in the same
file share setup? v1: just write a helper. v2: a `[jcc::before_all]`
...
**[Q:6]** Should `#pragma jcc push` / `#pragma jcc pop` for
block-scope JCC-option overrides ship in v1 or v2? My
...
1. **`[[jcc::test]]` / `[[jcc::negative_test]]` / `[[jcc::suite]]` /
...
5. **The `native` runner** — for each `mode=native` and the
   native half of `mode=both` tests, fork a child JCC process with
...
6. **JCC pragma collection** — a small parser pass over each
   `mode=vm` test's source file that lifts `#pragma jcc ...` lines
   into the equivalent CLI flag list, applied to the in-VM
   process for that test.
7. **Negative-test forking** — the runner forks a child JCC for
...
- **Per-test, per-suite, and hybrid execution modes.** The
  ...
  implementation. v2 adds the others as `jcc_runner_per_test`,
  `jcc_runner_per_suite`, `jcc_runner_hybrid`. The dispatch loop
...
- **Setup / teardown fixtures.** A `[jcc::before_all]` /
  `[jcc::after_all]` attribute on a suite descriptor, plus
  `[jcc::before_each]` / `[jcc::after_each]` for per-test
  fixtures. v2.
- **Block-scope pragmas.** `#pragma jcc push` / `#pragma jcc pop`
  for local JCC-option overrides inside a test function. v2.
...
- [BUILD_MODE.md](BUILD_MODE.md) — the build system that
  ...
  libraries. The two systems share the JCC pragma namespace —
  the full list is in
  [BUILD_MODE.md → JCC Pragma Extensions](BUILD_MODE.md#jcc-pragma-extensions).
...
- [DEBUGGER.md](../docs/DEBUGGER.md) — the `-g` interactive
  ...
  `#pragma jcc debug` for interactive debugging of a single
  test.
...
- [OPTIMIZATION.md](../docs/OPTIMIZATION.md) — the bytecode
  optimisation passes that `#pragma jcc optimise(N)` activates
  for VM-mode tests.
