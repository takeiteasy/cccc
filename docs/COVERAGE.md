...
| `<assert.h>` | ✓ | |
| `<ctype.h>` | ✓ | |
| `<errno.h>` | ✓ | `errno` aliases the host's real per-thread errno via an accessor function (`#define errno (*__cccc_errno_ptr())`, same pattern as `stdin`/`stdout`/`stderr` in `<stdio.h>`), so a failing host-backed call (`access`, `sysconf`, etc.) is actually observable from guest code. Windows targets keep the old plain-global behavior (untested target; not wired to the accessor). All numeric `E*` codes are `#ifdef __APPLE__`-guarded per-platform (Darwin and glibc/Linux diverge past the common 1-34 range, including some low-numbered codes like `EDEADLK`/`EAGAIN` swapping between the two), verified against a real glibc header via a native x86_64 container. |
