...
| `__builtin_object_size(ptr, type)` | Compile-time object-size computation for statically-known objects (local/global arrays, scalars, struct members reachable via constant-offset chains). Returns exact remaining byte count for `type` 0–3 (bit 0: whole object vs nearest subobject; bit 1: max vs min fallback). Falls back conservatively to `(size_t)-1` (type 0/1) or `0` (type 2/3) for unknown pointers (function parameters, heap, non-constant indices) — preserving `_FORTIFY_SOURCE` safety. |
| `__builtin_dynamic_object_size(ptr, type)` | Runtime object-size query. For statically-known objects (stack/global arrays, constant-offset chains) the result is folded to a compile-time constant (identical to `__builtin_object_size`). For VM heap allocations made via `malloc`/`calloc`/`realloc` under `--vm-heap` (`-V`), the `DYNOBJSZ` opcode reads `AllocHeader.requested_size` at runtime to return the exact requested byte count. The `type` argument has the same bit encoding as `__builtin_object_size`. **v1 limitations**: interior heap pointers (`p + k`) and `alloca`/VLA stack buffers fall back to the conservative value — interior pointer support requires wiring the `sorted_allocs` range-query table (follow-up). |
