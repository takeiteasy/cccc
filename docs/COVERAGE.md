...
| `<dirent.h>` | ✓ | Directory entry iteration (`opendir`, `readdir`, `readdir_r`, `closedir`, `seekdir`, `telldir`, `rewinddir`, `alphasort`, `DIR`, `struct dirent`); `scandir` intentionally not provided -- its callback arguments require a host-callable function pointer the FFI layer cannot produce for a guest function reference |
...
| `<netdb.h>` | ✓ | Network database (`gethostbyname`, `gethostbyaddr`, `getaddrinfo`, `freeaddrinfo`, `getnameinfo`, `struct hostent`, `struct addrinfo`), `EAI_*` and `NI_*` constants |
| `<netinet/in.h>` | ✓ | Internet address family (`struct sockaddr_in`, `struct in_addr`, `in_port_t`, `in_addr_t`, `INADDR_*`, `IPPROTO_*` (including `IPPROTO_ICMP`/`IPPROTO_ICMPV6`/`IPPROTO_RAW`), `IPPORT_RESERVED`) |
...
| `<sys/socket.h>` | ✓ | Socket API (`socket`, `socketpair`, `bind`, `listen`, `accept`, `connect`, `setsockopt`, `getsockopt`, `getsockname`, `getpeername`, `sockatmark`, `shutdown`, `struct sockaddr`, `socklen_t`), data transfer (`recv`, `send`, `recvfrom`, `sendto`), `AF_UNIX`/`AF_LOCAL`, expanded `SO_*` (`SO_ERROR`, `SO_KEEPALIVE`, `SO_BROADCAST`, `SO_RCVBUF`, `SO_SNDBUF`, `SO_LINGER`, `SO_TYPE`, `SO_REUSEPORT`, ...) and `MSG_*` (`MSG_PEEK`, `MSG_DONTWAIT`, `MSG_WAITALL`, `MSG_OOB`, `MSG_EOR`, `MSG_TRUNC`, `MSG_CTRUNC`, plus Linux `MSG_NOSIGNAL`) constants; `SOCK_CLOEXEC`/`SOCK_NONBLOCK` declared under `__linux__` (macOS lacks these). `recvmsg`/`sendmsg` and `struct msghdr`/`cmsghdr`/`CMSG_*` intentionally not provided -- their layout differs macOS vs Linux and needs dedicated design work |
...
| `<sys/wait.h>` | ✓ | Process wait (`wait`, `waitpid`), `WNOHANG`, `WUNTRACED`, `WCONTINUED`, `WIFEXITED`, `WEXITSTATUS`, `WIFSIGNALED`, `WIFSTOPPED`, `WIFCONTINUED`, `WSTOPSIG`, `WCOREDUMP`, `waitid()`-flag constants `WEXITED`/`WSTOPPED`/`WNOWAIT`; `waitid` itself intentionally not provided -- it needs a guest-visible `siginfo_t`, which `<signal.h>` does not yet declare |
...
| `<unistd.h>` | ✓ | Core POSIX API (`read`, `write`, `pread`, `pwrite`, `close`, `lseek`, `access`, `unlink`, `rmdir`, `chdir`, `getcwd`, `getpid`, `getppid`, `getuid`, `geteuid`, `getgid`, `getegid`, `seteuid`, `setegid`, `setuid`, `setgid`, `getgroups`, `setgroups`, `initgroups`, `getlogin`, `getlogin_r`, `fchown`, `chown`, `lchown`, `readlink`, `symlink`, `link`, `fdatasync`, `getpagesize`, `sleep`, `usleep`, `alarm`, `pause`, `nice`, `getpgid`, `setpgid`, `getpgrp`, `setsid`, `getsid`, `fchdir`, `gethostname`, `sethostname`, `ttyname_r`, `readv`, `writev`, `struct iovec`, `pipe`, `fork`, `execv`, `execve`, `execl`, `execlp`, `execle`, `execvp`, `_exit`, `ssize_t`, `STDIN/STDOUT/STDERR_FILENO`, `SEEK_*`, `F_OK`/`R_OK`/`W_OK`/`X_OK`, `sysconf`, `pathconf`, `fpathconf`, `confstr`, canonical `_SC_*`/`_PC_*`/`_CS_*` constants, `_POSIX_VERSION`/`_POSIX2_VERSION`/`_XOPEN_VERSION`; `splice` declared and registered under `__linux__`) |
