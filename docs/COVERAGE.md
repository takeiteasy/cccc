...
| `_BitInt(N)` arbitrary-precision integers | ~ | `N` in `[1,256]`. `N<=64` uses scalar-register storage with mask/shift truncation; `N` in `(64,256]` uses multi-word (address-based) storage with runtime helper functions for arithmetic, shifts, comparisons, and conversions. `wb`/`uwb` literal suffixes are limited to 64-bit values even when the inferred width is wide (see follow-up ticket); `N > 256` tracked in a follow-up ticket |
