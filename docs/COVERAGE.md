...
| `<sys/resource.h>` | ✓ | Resource usage (`getrusage`), `struct rusage` (identical layout on macOS and Linux), `RUSAGE_SELF`/`RUSAGE_CHILDREN`. `ru_maxrss` is measured in bytes on macOS but kilobytes on Linux -- a real semantic divergence, not normalized by this header |
