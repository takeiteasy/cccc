...
| `__attribute__((vector_size(N)))` generic vectors | ~ | 128-bit (16-byte) vectors only — `v2f64`/`v4f32`/`v2i64`/`v4i32`/`v8i16`/`v16i8` lane layouts; wider vectors are rejected with a diagnostic. Element-wise `+ - * /` (float lanes only for `/`) and unary `-`; scalar operands broadcast through arithmetic (`v + 5.0f`), matching GCC/clang — a bare scalar cannot initialize or be assigned to a whole vector. `v[i]` subscript supports a runtime-variable index. Not yet supported: brace-initializer syntax (`v4sf a = {1,2,3,4};` — construct via per-lane assignment instead), by-value function args/returns, `%`/`&`/`|`/`^`, integer lane division, comparison-to-mask-vector, `__builtin_shuffle`/`__builtin_convertvector`. See [VM.md](VM.md#simd--vector-operations) for the opcode set |
