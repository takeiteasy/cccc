...
- `sentinel` — warns on a call to a function marked `sentinel`/`sentinel(N)` whose expected trailing variadic argument is not a literal `NULL`/`0`; also warns if the call doesn't supply enough variadic arguments for the sentinel position to exist. Purely syntactic (no flow analysis — a variable holding `NULL` still warns); part of `-Wall`
