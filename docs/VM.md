...
| `VADD_I64X2`, `VSUB_I64X2`, `VMUL_I64X2`, `VDIV_I64X2`, `VMOD_I64X2`, `VNEG_I64X2` | Per-lane `+ - * / %` and unary negate, 2×i64. `VDIV`/`VMOD` trap on a zero divisor or `INT64_MIN/-1` overflow (see below) |
| `VADD_I32X4`, `VSUB_I32X4`, `VMUL_I32X4`, `VDIV_I32X4`, `VMOD_I32X4`, `VNEG_I32X4` | Same, 4×i32 |
| `VADD_I16X8`, `VSUB_I16X8`, `VMUL_I16X8`, `VDIV_I16X8`, `VMOD_I16X8`, `VNEG_I16X8` | Same, 8×i16 |
| `VADD_I8X16`, `VSUB_I8X16`, `VMUL_I8X16`, `VDIV_I8X16`, `VMOD_I8X16`, `VNEG_I8X16` | Same, 16×i8 |
| `VAND`, `VOR`, `VXOR`, `VNOT` | Whole-128-bit bitwise `& \| ^ ~`, width-agnostic (implemented over `i64[2]`) — integer lanes only, no per-lane-family variant needed |
| `VCEQ_*`, `VCNE_*`, `VCLT_*`, `VCLE_*` (one family per lane layout, `F64X2`…`I8X16`) | Per-lane comparison: writes all-ones (`-1`) if true, `0` if false, into a same-width **signed** integer lane (GCC semantics) |
| `VCLTU_*`, `VCLEU_*` (`I64X2`…`I8X16` only) | Unsigned-view ordered comparison — selected by lane element signedness at codegen; `>`/`>=` are parsed as swapped-operand `<`/`<=`, so no separate opcodes exist |
| `VSEL_8`, `VSEL_16`, `VSEL_32`, `VSEL_64` | GNU vector `?:` select, by lane byte width: `rd[i] = cond[i] ? then[i] : rd[i]` — `rd` is pre-loaded by codegen with the else-arm, so this is a **read-modify-write** on `rd` (like `VINSERT_*`), not a pure write |
| `VCVT_I32_F32`, `VCVT_F32_I32`, `VCVT_I64_F64`, `VCVT_F64_I64` | `__builtin_convertvector` lane conversion (truncating toward zero for float→int, C cast semantics) |
...
**Frontend lowering (codegen.c `gen_vector_expr`).** A vector local/global lives in a 16-byte memory slot, exactly like a small struct — no frame-layout changes were needed beyond sizing the slot correctly (`assign_stack_offsets`/`var_stack_slots`). The *value* of a vector expression flows through a `vregs[]` index in `dest_reg`, mirroring how float-typed expressions flow through `fregs[]` even though float locals also live in memory: `VLDR`/`VSTR` move between the slot and a vreg around each operation. `v[i]` subscript is **not** lowered through `VEXTRACT`/`VINSERT` — it is intercepted at parse time (before array-decay pointer arithmetic, which would misinterpret the vector's `base` field) and rewritten to `*(elem_ty*)(&v + i)`, reusing ordinary scalar load/store and supporting a runtime-variable index for free. `VEXTRACT`/`VINSERT` remain reserved for the autovectorizer (horizontal reductions), though `__builtin_shuffle`'s constant-mask lowering also reuses this same subscript machinery (see below) rather than `VEXTRACT`/`VINSERT` directly.

**`__builtin_shuffle` (constant-mask only).** CCCC accepts `__builtin_shuffle(v, {i0,...,iN-1})` (1-vector permute) or `__builtin_shuffle(v1, v2, {i0,...,iN-1})` (2-vector blend), where the index mask must be a bare, compile-time-constant brace list parsed directly by the builtin (not a general vector-typed expression, and not a `(vTYPE){...}` compound literal — that would be ambiguous with the two-vector form's second argument, since both start with `(`). No new opcode: the parser materializes the source vector(s) into hidden locals (so side effects run once), then builds a chain of per-lane scalar reads/writes through the same vector-subscript lvalue lowering `v[i]` uses. This differs from upstream GCC's `__builtin_shuffle`, which takes a general vector-typed mask (constant or runtime) — CCCC's form is closer to clang's `__builtin_shufflevector`. A runtime/named vector mask is rejected with a diagnostic (tracked as a follow-up).
...
**By-value function args/returns (#714).** Reuses the struct-by-value call ABI rather than a register (FReg/GReg) convention, since a 128-bit vector doesn't fit an 8-byte arg slot. Argument: the caller materializes the vector's value (a `vregs[]` register) into a fresh 16-byte frame scratch slot via `VSTR`, then passes that slot's address in the integer arg register — like a struct-by-value arg, except the slot is a compiler-synthesized copy rather than the argument's own addressable storage, giving true by-value semantics for vector rvalues too. Return: the callee requests a buffer from the `RETBUF` rotating pool, `VSTR`s the result into it, and returns the buffer address; the caller `VLDR`s out of it. The callee reads a vector parameter the same way a struct parameter is read — the param's stack slot holds a pointer to the value, not the value itself (`gen_addr`'s param-slot dereference). Not supported through the native FFI marshalling path (int/float-only slot classification), a variadic `...` parameter (register spilling for `va_arg` is int/float-only), or a GNU/Apple block invocation (no by-memory ABI for aggregates at all) — each rejected at compile time. Tail-call elimination (`can_emit_tail_call`) excludes any vector-returning call and any call with a vector argument, since `CALLT` reuses the caller's frame and both the RETBUF buffer and the argument scratch slot are frame-local.
