...
CCCC has optional LLVM linkage for bytecode-to-LLVM IR backend work. Enable it at
build time with `CCCC_HAS_LLVM=1`.

```bash
make CCCC_HAS_LLVM=1 LLVM_CONFIG=/opt/homebrew/opt/llvm/bin/llvm-config
```

The LLVM integration is intentionally internal. CCCC still compiles C to VM
...
CCCC frontend, including preprocessing and compile-time macros, serializes the
post-macro C program to a temporary file, and invokes a system compiler. Compiler
selection uses `CCCC_NATIVE_CC` when set, otherwise `cc`, `clang`, then `gcc`.
