/*
 ...
 write-a-C-interpreter tutorial by Jinzhou Zhang
...
cccc_find_dynamic_library(VirtualMachine *vm, long long token, int *idx_out) {
    if (!vm)
        return NULL;
...
static int cccc_add_dynamic_library(VirtualMachine *vm, void *handle,
                                    const char *path) {
    if (vm->dynlib_count >= vm->dynlib_capacity) {
        ...
        if (!new_libs) {
            cccc_set_dyn_error(vm,
                               "dynamic library registry allocation failed");
            return 0;
        }
        vm->dynlibs         = new_libs;
...
static int cccc_add_dynamic_symbol(VirtualMachine *vm, int lib_idx,
                                   void *func_ptr, const char *name) {
...
long long cccc_rt_dlclose(VirtualMachine *vm, long long handle_token) {
    ...
    if (lib->live_symbol_count > 0) {
        cccc_set_dyn_error(vm,
                           "cannot dlclose handle with live callable symbols");
        return -1;
    }
...
#define VM_TRAP_OR_RETURN(rc)                                                  \
        ...
        for (int _sig = 1; _sig < CCCC_NSIG; _sig++) {
            ...
            if (_slot->action == 2) {
                ...
                if (_target == CCCC_INVALID_PC || _target > vm->text_ptr) {
                    fprintf(stderr,
                            "error: invalid signal handler for sig %d\n", _sig);
                    VM_TRAP_OR_RETURN(-1);
                }
                ...
                *--vm->sp = (long long)vm->pc;
                ...
                goto dispatch; /* resume at handler; delivers one signal per
                                  re-entry */
            }
    ...
    /* Poll pending SIGEV_THREAD notifications (#870) -- same safe point and
       ...
       thread (aio_*()/mq_notify(), src/stdlib/posix_aio.c/posix_mqueue.c) can
       only latch a cookie async-signal-safely, so the guest
       sigev_notify_function is actually invoked here, one notification per
       re-entry. Shares the pending-signal poll's async-delivery hazard -- this
       can land between ANY two bytecode instructions, not just a call boundary
       -- and shares its fix (#877): cccc_signal_push_async_frame snapshots the
       full register file into a SigFrame before REG_A0 is overwritten below,
       and cccc_signal_poll_handler_returns (called at the top of this same
       dispatch loop, gated on vm->sig_depth != 0) restores it once the guest
       sigev_notify_function genuinely returns. As with the pending-signal poll,
       a full frame stack defers this notification (leaves it pending) rather
       than delivering with unsaved registers. */
    if (__builtin_expect(_cccc_sigev_any_pending != 0, 0)) {
        _cccc_sigev_any_pending = 0;
        for (int _idx = 0; _idx < CCCC_SIGEV_MAX; _idx++) {
            if (!_cccc_sigev_pending[_idx])
                continue;
            if (vm->sig_depth >= CCCC_SIG_FRAME_MAX) {
                _cccc_sigev_any_pending =
                    1; /* defer -- retry once a frame frees up */
                break;
            }
            _cccc_sigev_pending[_idx] = 0;
            long long _sigev_fn = 0, _sigev_sival_addr = 0;
            if (!cccc_sigev_cookie_guest_fn(_idx, &_sigev_fn,
                                            &_sigev_sival_addr))
                continue; /* cancelled/deregistered since the trampoline fired
                           */
            Pc _sigev_target = cc_byte_offset_to_pc(_sigev_fn);
            if (_sigev_target == CCCC_INVALID_PC ||
                _sigev_target > vm->text_ptr) {
            ...
            *--vm->sp = (long long)vm->pc;
            if (vm->flags & CCCC_CFI)
                *--vm->shadow_sp = (long long)vm->pc;
            cccc_signal_push_async_frame(
                vm); /* snapshot regs before REG_A0 is clobbered below */
...

    {
        ...
        if (__builtin_expect(
                op < 0 || op >= (int)(sizeof(op_table) / sizeof(op_table[0])) ||
                    !op_table[op],
                0)) {
...
void vm_alloc_segments(VirtualMachine *vm) {
    ...
    // Stack: sp/bp at top of reservation; stack_base = bottom of committed
    // region
...
int cccc_exec_state_alloc_stack(VirtualMachine *vm, ExecState *state) {
    ...
    if (vm->flags & CCCC_CFI) {
        ...
        if (cccc_vm_commit(state->shadow_stack, stack_top_off, initial_stack) !=
            0) {
            cccc_exec_state_release_stack(vm, state);
            return -1;
        }
        state->shadow_sp =
            (long long *)((char *)state->shadow_stack + reserved_stack);
    }
...
int vm_heap_grow(VirtualMachine *vm, size_t need) {
    ...
    if (chunk < need)
        chunk = ((need + (size_t)vm->poolsize - 1) / (size_t)vm->poolsize) *
                (size_t)vm->poolsize;
...
void cc_init(VirtualMachine *vm, uint32_t flags) {
    ...
    vm->poolsize_max =
        256 * 256 * 1024;      // 64M elements reserved (address space only)
    vm->debug_vm = 0;
    ...
    vm->compiler.return_buffer_index =
        0;        // Compile-time index (unused with RETBUF)
    vm->runtime_return_buffer_index =
        0;        // Runtime rotation index for RETBUF opcode
    for (int i = 0; i < RETURN_BUFFER_POOL_SIZE; i++) {
        vm->compiler.return_buffer_pool[i] =
            NULL; // Will be set to data segment locations
        vm->compiler.return_buffer_offsets[i] = -1;
    }

    // Initialize parser arena BEFORE init_macros to avoid orphaning blocks
    // (init_macros allocates from the arena, so arena must be initialized
    // first)
    ...
    vm->init_state.capacity =
        0; // Will be allocated on first use by hashmap_put
    ...
    // Initialize stack canary (will be set to random or fixed value based on
    // flag) The flag CCCC_RANDOM_CANARIES will trigger regeneration in main.c
    // For now, initialize to fixed value; it will be regenerated if random
    // canaries enabled
...
void cc_destroy(VirtualMachine *vm) {
    ...
    for (BuildTargetFnRecord *b = vm->compiler.build_target_fns, *nb; b;
         b                      = nb) {
        nb = b->next;
        free(b->name);
        free(b->kind);
...
void cc_print_stack_report(VirtualMachine *vm) {
    ...
    for (int i = 0; i < vm->stack_var_meta.capacity; i++) {
        if (vm->stack_var_meta.buckets[i].key != NULL) {
            StackVarMeta *meta =
                (StackVarMeta *)vm->stack_var_meta.buckets[i].val;
            if (meta) {
                const char *status = meta->is_alive ? "alive" : "dead";
                printf("%-20s %10d %10lld %10lld %10s\n",
                       meta->name ? meta->name : "<unknown>", meta->scope_id,
                       meta->read_count, meta->write_count, status);
            }
        }
    }
...
void cc_register_cfunc_ex(VirtualMachine *vm, const char *name, void *func_ptr,
    ...
    vm->compiler.ffi_table[vm->compiler.ffi_count++] =
        (ForeignFunc){.name            = strdup(name),
...
void cc_register_variadic_cfunc(VirtualMachine *vm, const char *name,
    ...
    vm->compiler.ffi_table[vm->compiler.ffi_count++] = (ForeignFunc){
        ...
        .double_arg_mask =
            0 // Variadic functions don't use mask - doubles passed as bits
    };
}
...
int cc_dlsym(VirtualMachine *vm, const char *name, void *func_ptr, int num_args,
    ...
    for (int i = 0; i < vm->compiler.ffi_count; i++) {
        if (vm->compiler.ffi_table[i].name_len == name_len &&
            ...
            if (vm->compiler.ffi_table[i].num_args != num_args ||
                get_return_kind(&vm->compiler.ffi_table[i]) != returns_double) {
...
int cc_dlopen(VirtualMachine *vm, const char *lib_path) {
    ...
    if (lib_path) {
        ...
        if (!handle) {
            ...
            fprintf(stderr,
                    "error: failed to load library %s: error code %lu\n",
                    lib_path, err);
            return -1;
        }
    } else {
    ...
    for (int i = 0; i < vm->compiler.ffi_count; i++) {
        ...
        if (!func_ptr) {
            ...
            if (vm->debug_vm)
                fprintf(
                    stderr,
                    "warning: failed to resolve symbol '%s': error code %lu\n",
                    ff->name, err);
        } else {
    ...
    if (vm->debug_vm)
        printf("Loaded %d/%d FFI functions from %s\n", success_count,
               total_count, lib_path ? lib_path : "default libraries");
...
static const char *find_libc() {
    ...
    libname                   = "libc.so.6";
    const char *linux_paths[] = {"/lib64/",   "/lib/x86_64-linux-gnu/",
                                 "/lib/",     "/usr/lib64/",
                                 "/usr/lib/", NULL};
...
static void cc_scan_dynobjsz(VirtualMachine *vm) {
    ...
    long long pc =
        vm->dynobjsz_scan_pc ? vm->dynobjsz_scan_pc : 1; // [0] = entry slot
    while (pc < num_words) {
...
static int cc_run_at_regs(VirtualMachine *vm, Pc entry, long long a0,
                          long long a1, int debugger_argc,
                          char **debugger_argv) {
    if (!vm || !vm->text_seg) {
    ...
    // Setup stack — use poolsize_max so sp/bp sit at top of full reservation
    {
        ...
        vm->stack_base = (long long *)((char *)vm->stack_seg + reserved_stack -
                                       initial_stack);

        // Shadow stack for CFI — lazily allocated here to handle per-test -C
        // flag changes where vm_alloc_segments ran before CCCC_CFI was set
        // (#605).
        if (vm->flags & CCCC_CFI) {
            if (!vm->shadow_stack) {
                size_t stack_top_off = reserved_stack - initial_stack;
    ...
    *--vm->sp = 0;
    ...
    int rc = ((vm->flags & CCCC_ENABLE_DEBUGGER) && !vm->dbg.crash_debug_auto)
                 ? debugger_run(vm, debugger_argc, debugger_argv)
                 : vm_eval(vm);
...
static void cc_run_init_entries(VirtualMachine *vm, CCCCInitEntry *list,
                                int count) {
    for (int i = 0; i < count; i++)
    ...
    while (vm->atexit_count > 0) {
        ...
        if (fn_value <= CCCC_FFI_TOKEN_BASE) {
            ...
            if (ffi_idx < 0 || ffi_idx >= vm->compiler.ffi_count)
                continue;
            ForeignFunc *ff = &vm->compiler.ffi_table[ffi_idx];
            cccc_call_native_function(vm, ff->func_ptr, ff->name, NULL, 0, 0, 0,
                                      0, 0, ff->is_variadic,
                                      ff->num_fixed_args);
            continue;
        }
...
int cccc_vm_commit(void *base, size_t off, size_t len) {
    ...
    void  *p = VirtualAlloc((char *)base + aligned_off, aligned_len, MEM_COMMIT,
                            PAGE_READWRITE);
    return (p == NULL) ? -1 : 0;
}
