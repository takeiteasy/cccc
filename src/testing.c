...
// calls that arrive outside a test run (e.g. from a [[cccc::comptime]] during
// the comptime pass) -- ticket #334.
typedef struct {
    jmp_buf jmp;
    int     failed;
    int     timed_out;
...
#endif
    if (s_run)
        longjmp(s_run->jmp, 2); // 2 = timeout sentinel
}

static void impl_assert(long long cond, long long expr, long long file,
                        long long line) {
    if (!cond) {
        if (!s_run) {
...
static void impl_assert_eq(long long a, long long b, long long as, long long bs,
    ...
    if (a != b) {
        ...
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s != %s (%lld != %lld) (%s:%lld)", (char *)as, (char *)bs, a,
                 b, (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
...
static void impl_assert_neq(long long a, long long b, long long as,
                            long long bs, long long file, long long line) {
    if (a == b) {
        if (!s_run) {
        ...
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s == %s (both %lld) (%s:%lld)", (char *)as, (char *)bs, a,
                 (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
...
static void impl_assert_null(long long p, long long ps, long long file,
                             long long line) {
    if (p != 0) {
        if (!s_run) {
        ...
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s is not null (%s:%lld)", (char *)ps, (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
}

static void impl_assert_not_null(long long p, long long ps, long long file,
                                 long long line) {
    if (p == 0) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertNotNull called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s is null (%s:%lld)", (char *)ps, (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
}

static void impl_assert_streq(long long a, long long b, long long as,
                              long long bs, long long file, long long line) {
    if (strcmp((char *)a, (char *)b) != 0) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertStrEq called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s != %s (\"%s\" != \"%s\") (%s:%lld)", (char *)as,
                 (char *)bs, (char *)a, (char *)b, (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
}

static void impl_assert_false(long long cond, long long expr, long long file,
                              long long line) {
    if (cond) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertFalse called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg), "!%s (%s:%lld)",
...
static void impl_assert_fail_msg(long long msg, long long file,
                                 long long line) {
    if (!s_run) {
    ...
    snprintf(s_run->fail_msg, sizeof(s_run->fail_msg), "%s (%s:%lld)",
             (char *)msg, (char *)file, line);
    s_run->failed = 1;
    longjmp(s_run->jmp, 1);
}

static void impl_assert_gt(long long a, long long b, long long as, long long bs,
                           long long file, long long line) {
    if (!(a > b)) {
        if (!s_run) {
...
static void impl_assert_lt(long long a, long long b, long long as, long long bs,
                           long long file, long long line) {
    if (!(a < b)) {
        if (!s_run) {
...
static void impl_assert_ge(long long a, long long b, long long as, long long bs,
                           long long file, long long line) {
    if (!(a >= b)) {
        if (!s_run) {
...
static void impl_assert_le(long long a, long long b, long long as, long long bs,
                           long long file, long long line) {
    if (!(a <= b)) {
        if (!s_run) {
...
static void impl_assert_within(long long delta, long long expected,
                               long long actual, long long ds, long long es,
                               long long as, long long file, long long line) {
    ...
    if (diff > delta) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertWithin called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s |%s - %s| = %lld > %s (%lld) (%s:%lld)", (char *)as,
                 (char *)es, (char *)as, diff, (char *)ds, delta, (char *)file,
                 line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
...
static void impl_assert_streq_len(long long a, long long b, long long len,
                                  long long as, long long bs, long long file,
                                  long long line) {
    if (strncmp((char *)a, (char *)b, (size_t)len) != 0) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertStrEqLen called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s != %s (first %lld chars differ) (%s:%lld)", (char *)as,
                 (char *)bs, len, (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
}

static void impl_assert_mem_eq(long long expected, long long actual,
                               long long len, long long es, long long as,
                               long long file, long long line) {
    if (memcmp((void *)expected, (void *)actual, (size_t)len) != 0) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertMemEq called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s != %s (%lld bytes differ) (%s:%lld)", (char *)es,
                 (char *)as, len, (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
}

static void impl_assert_float_within(double delta, double expected,
                                     double actual, long long ds, long long es,
                                     long long as, long long file,
                                     long long line) {
    ...
    if (diff > delta) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertFloatWithin called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s |%s - %s| = %g > %s (%g) (%s:%lld)", (char *)as,
                 (char *)es, (char *)as, diff, (char *)ds, delta, (char *)file,
                 line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
...
static void impl_assert_double_within(double delta, double expected,
                                      double actual, long long ds, long long es,
                                      long long as, long long file,
                                      long long line) {
    // Identical to impl_assert_float_within — the C ABI for doubles is the
    // same.
    impl_assert_float_within(delta, expected, actual, ds, es, as, file, line);
}

static void impl_assert_bits(long long mask, long long expected,
                             long long actual, long long ms, long long es,
                             long long as, long long file, long long line) {
    if ((actual & mask) != (expected & mask)) {
        ...
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s & %s = 0x%llx != %s & %s = 0x%llx (%s:%lld)", (char *)as,
                 (char *)ms, (unsigned long long)(actual & mask), (char *)es,
                 (char *)ms, (unsigned long long)(expected & mask),
                 (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
...
static void impl_assert_bit_high(long long bit, long long actual, long long bs,
                                 long long as, long long file, long long line) {
    if (!(actual & (1LL << bit))) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertBitHigh called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s bit %lld of %s is low (%s:%lld)", (char *)bs, bit,
                 (char *)as, (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
}

static void impl_assert_bit_low(long long bit, long long actual, long long bs,
                                long long as, long long file, long long line) {
    if (actual & (1LL << bit)) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertBitLow called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s bit %lld of %s is high (%s:%lld)", (char *)bs, bit,
                 (char *)as, (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
...
static void impl_assert_eq_array(long long expected, long long actual,
                                 ...
                                 long long es, long long as, long long file,
                                 long long line) {
    size_t total = (size_t)elem_size * (size_t)count;
    if (memcmp((void *)expected, (void *)actual, total) != 0) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertArrayEq called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s[0..%lld] != %s[0..%lld] (%lld bytes differ) (%s:%lld)",
                 (char *)es, count - 1, (char *)as, count - 1, (long long)total,
                 (char *)file, line);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
...
// Message-appending variants — append msg to the fail message after the main
// diagnostic.
...
static void impl_assert_msg(long long cond, long long expr, long long msg,
    ...
    if (!cond) {
        ...
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg), "%s (%s:%lld) - %s",
                 (char *)expr, (char *)file, line, (char *)msg);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
}

static void impl_assert_eq_msg(long long a, long long b, long long as,
                               long long bs, long long msg, long long file,
                               long long line) {
    if (a != b) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertEqMsg called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s != %s (%lld != %lld) (%s:%lld) - %s", (char *)as,
                 (char *)bs, a, b, (char *)file, line, (char *)msg);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
}

static void impl_assert_streq_msg(long long a, long long b, long long as,
                                  long long bs, long long msg, long long file,
                                  long long line) {
    if (strcmp((char *)a, (char *)b) != 0) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertStrEqMsg called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s != %s (\"%s\" != \"%s\") (%s:%lld) - %s", (char *)as,
                 (char *)bs, (char *)a, (char *)b, (char *)file, line,
                 (char *)msg);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
...
static void impl_assert_null_msg(long long p, long long ps, long long msg,
    ...
    if (p != 0) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertNullMsg called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s is not null (%s:%lld) - %s", (char *)ps, (char *)file,
                 line, (char *)msg);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
...
static void impl_assert_not_null_msg(long long p, long long ps, long long msg,
    ...
    if (p == 0) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertNotNullMsg called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
                 "%s is null (%s:%lld) - %s", (char *)ps, (char *)file, line,
                 (char *)msg);
        s_run->failed = 1;
        longjmp(s_run->jmp, 1);
    }
...
static void impl_assert_bits_msg(long long mask, long long expected,
                                 long long actual, long long ms, long long es,
                                 long long as, long long msg, long long file,
                                 long long line) {
    if ((actual & mask) != (expected & mask)) {
        if (!s_run) {
            fprintf(stderr,
                    "AssertBitsMsg called outside a test run at %s:%lld\n",
                    (char *)file, line);
            return;
        }
        snprintf(s_run->fail_msg, sizeof(s_run->fail_msg),
...
void cc_load_test_runtime(VirtualMachine *vm) {
    ...
    cc_register_cfunc(vm, "__builtin_assert_false", (void *)impl_assert_false,
                      4, 0);
    cc_register_cfunc(vm, "__builtin_assert_fail", (void *)impl_assert_fail, 2,
                      0);
    cc_register_cfunc(vm, "__builtin_assert_fail_msg",
                      (void *)impl_assert_fail_msg, 3, 0);
    cc_register_cfunc(vm, "__builtin_assert_eq", (void *)impl_assert_eq, 6, 0);
    cc_register_cfunc(vm, "__builtin_assert_neq", (void *)impl_assert_neq, 6,
                      0);
    ...
    cc_register_cfunc(vm, "__builtin_assert_within", (void *)impl_assert_within,
                      8, 0);
    cc_register_cfunc(vm, "__builtin_assert_null", (void *)impl_assert_null, 4,
                      0);
    cc_register_cfunc(vm, "__builtin_assert_not_null",
                      (void *)impl_assert_not_null, 4, 0);
    cc_register_cfunc(vm, "__builtin_assert_streq", (void *)impl_assert_streq,
                      6, 0);
    cc_register_cfunc(vm, "__builtin_assert_streq_len",
                      (void *)impl_assert_streq_len, 7, 0);
    cc_register_cfunc(vm, "__builtin_assert_mem_eq", (void *)impl_assert_mem_eq,
                      7, 0);
    cc_register_cfunc(vm, "__builtin_assert_float_within",
                      (void *)impl_assert_float_within, 8, 0);
    cc_register_cfunc(vm, "__builtin_assert_double_within",
                      (void *)impl_assert_double_within, 8, 0);
    cc_register_cfunc(vm, "__builtin_assert_bits", (void *)impl_assert_bits, 8,
                      0);
    cc_register_cfunc(vm, "__builtin_assert_bit_high",
                      (void *)impl_assert_bit_high, 6, 0);
    cc_register_cfunc(vm, "__builtin_assert_bit_low",
                      (void *)impl_assert_bit_low, 6, 0);
    cc_register_cfunc(vm, "__builtin_assert_eq_array",
                      (void *)impl_assert_eq_array, 8, 0);
    cc_register_cfunc(vm, "__builtin_assert_msg", (void *)impl_assert_msg, 5,
                      0);
    cc_register_cfunc(vm, "__builtin_assert_eq_msg", (void *)impl_assert_eq_msg,
                      7, 0);
    cc_register_cfunc(vm, "__builtin_assert_streq_msg",
                      (void *)impl_assert_streq_msg, 7, 0);
    cc_register_cfunc(vm, "__builtin_assert_null_msg",
                      (void *)impl_assert_null_msg, 5, 0);
    cc_register_cfunc(vm, "__builtin_assert_not_null_msg",
                      (void *)impl_assert_not_null_msg, 5, 0);
    cc_register_cfunc(vm, "__builtin_assert_bits_msg",
                      (void *)impl_assert_bits_msg, 9, 0);
}
...
//   - Prefix match: suite starts with filter followed by '/' (sub-suite
//   selection)
...
// suite and disp are the current test's context; NULL suite means no active
// suite. once_only=true selects once-per-suite hooks; false selects per-test
// hooks.
static void run_hooks(VirtualMachine *vm, Obj *prog, TestSetupRecord *setups,
                      bool is_teardown, bool once_only, const char *suite,
                      const char *disp) {
    for (TestSetupRecord *s = setups; s; s = s->next) {
        if (s->is_teardown != is_teardown)
            continue;
        if (s->once != once_only)
            continue;
        ...
        if (s->suite) {
            ...
            if (s->inherit ? !suite_matches(suite, s->suite)
                           : strcmp(s->suite, suite) != 0)
                continue;
        }
        // Name-pattern filter.
        // For per-test (once_only=false): run if name_pat matches or no
        // name_pat. For once-only (once_only=true): name_pat-only hooks are
        // handled separately by the first-match / end-of-tests logic in
        // cc_run_tests, so skip them here (they have already fired or will fire
        // later).
        if (s->name_pat) {
            if (once_only)
                continue; // handled separately for once-hooks
            if (!disp || fnmatch(s->name_pat, disp, 0) != 0)
                continue;
        }
...
static bool run_hooks_guarded(VirtualMachine *vm, Obj *prog,
                              TestSetupRecord *setups, bool is_teardown,
                              bool once_only, const char *suite,
                              const char *disp, char fail_msg_out[512]) {
...
static bool run_once_hooks(VirtualMachine *vm, Obj *prog,
                           TestSetupRecord *setups, bool is_teardown,
                           const char *suite) {
    return run_hooks_guarded(vm, prog, setups, is_teardown, true, suite, NULL,
                             NULL);
}
    ...
    switch (fmt) {
        ...
        case TEST_FORMAT_JSON:
            ...
            *json_first = false;
            ...
            if (suite)
                printf(",\"suite\":\"%s\"",
                       json_esc(suite, ebuf, sizeof(ebuf)));
            printf(",\"status\":\"%s\"", status);
            if (message)
                printf(",\"message\":\"%s\"",
                       json_esc(message, ebuf, sizeof(ebuf)));
            printf("}");
            break;
    }
}
...
static bool check_output_pattern(const char *pat, const char *buf, bool negate,
                                 const char *label, char fail_msg[512]) {
    if (!pat)
        return true;
    if (!buf)
        buf = "";
    ...
    if (rc != 0) {
        ...
        snprintf(fail_msg, 512, "%s \"%s\" (regex error: %s)", label, pat,
                 errbuf);
        return false;
    }
...
static bool cmp_ret_scalar(const char *addr, Type *mty, const TestRetField *ef,
    ...
    if (mty->kind == TY_FLOAT) {
        ...
        double expect = 0.0;
        if (ef)
            expect = (ef->kind == RET_INT) ? (double)ef->val.i : ef->val.f;
    ...
    if (mty->kind == TY_PTR && ef && ef->kind == RET_STR) {
        ...
        int         cmp = (actual && expect) ? strcmp(actual, expect)
                                             : (actual ? 1 : (expect ? -1 : 0));
        snprintf(field_val, field_val_cap, "\"%s\"",
                 actual ? actual : "(null)");
        return cmp == 0;
    }
    ...
    if (ef && ef->kind == RET_STR)
        return false; // kind mismatch: string expected
    int64_t expect = 0;
    if (ef)
        expect = (ef->kind == RET_FLOAT) ? (int64_t)ef->val.f : ef->val.i;
...
static bool cmp_ret_value(const char *buf, int base_off, Type *ty,
                          const TestRetField *ef, double eps, char *got,
                          size_t got_cap) {
    ...
    char field_val[64];
    bool ok = cmp_ret_scalar(buf + base_off, ty, ef, eps, field_val,
                             sizeof(field_val));
...
static bool cmp_ret_struct_body(const char *buf, int base_off, Type *ty,
    ...
    for (Member *m = ty->members; m; m = m->next) {
        if (!m->name) {
            ...
            if (!cmp_ret_struct_body(buf, base_off + m->offset, m->ty, expected,
                                     eps, got, got_cap, first))
                all_eq = false;
            continue;
        }
        ...
        if (!cmp_ret_value(buf, base_off + m->offset, m->ty, ef, eps, got,
                           got_cap))
            all_eq = false;
        *first = false;
    }
...
static bool cmp_ret_aggregate(const char *buf, int base_off, Type *ty,
    ...
    sb_appendf(got, got_cap, "{");
    bool first  = true;
    bool all_eq = cmp_ret_struct_body(buf, base_off, ty, expected, eps, got,
                                      got_cap, &first);
    sb_appendf(got, got_cap, "}");
    return all_eq;
}
...
int cc_run_tests(VirtualMachine *vm, Obj *prog, const CcTestOptions *opts) {
    ...
    for (TestSetupRecord *s = setups; s; s = s->next)
        if (!s->is_teardown && s->once && s->suite)
            n_once_suite++;
    OnceSuiteHook *once_suite =
        calloc(n_once_suite > 0 ? n_once_suite : 1, sizeof(OnceSuiteHook));
    {
        int idx = 0;
        for (TestSetupRecord *s = setups; s; s = s->next)
            if (!s->is_teardown && s->once && s->suite)
    ...
    // Reverse test records to run in declaration order (test_fns is built by
    // prepending).
    ...
    for (TestListNode *n2 = ordered; n2; n2 = n2->next) {
        ...
        if (opts && opts->test_glob) {
            // Match against the display name so name = "..." aliases are
            // filterable.
    ...
    if (opts && opts->list_only) {
        ...
        for (TestListNode *n2 = ordered, *nx; n2; n2 = nx) {
            nx = n2->next;
            free(n2);
        }
        for (TestListNode *n2 = filtered, *nx; n2; n2 = nx) {
            nx = n2->next;
            free(n2);
        }
        for (TestSetupRecord *s = setups, *nx; s; s = nx) {
            nx = s->next;
            free(s);
        }
    ...
    long global_timeout_ms =
        (opts && opts->test_timeout > 0) ? opts->test_timeout * 1000L : 0;
#define SET_TIMEOUT(ms)                                                        \
    ...
    // and warning flags are baked into codegen, so any test that requests a
    // different config triggers a lazy recompile.
    ...
    for (TestListNode *n2 = filtered; n2 && !stop_early; n2 = n2->next) {
        ...
        // Streaming once-suite-hook close pass (#515): close innermost-first
        // any hook whose subtree no longer covers this test.  "Subtree" uses
        ...
        for (int _i = n_once_suite - 1; _i >= 0; _i--) {
            if (!once_suite[_i].open)
                continue;
            const char *hs = once_suite[_i].hook->suite;
            if (suite_matches(cur_suite ? cur_suite : "", hs))
                continue;
        ...
        // Suite-change header (purely cosmetic; hook lifecycle is now
        // streaming).
        ...
        for (int _i = 0; _i < n_once_suite; _i++) {
            if (once_suite[_i].hook->once_fired)
                continue;
            const char *hs = once_suite[_i].hook->suite;
            ...
            memcpy(vm->data_seg, once_suite_snap(once_suite, _i, cur_snap),
                   snap_size);
        ...
        if (r->error_pat || r->expect_compile_error) {
            if (r->neg_passed == 1) {
                emit_test_result(fmt, disp, cur_suite, "neg_pass", NULL,
                                 &json_first, test_num);
                passed++;
                total_neg_pass++;
                total_neg++;
                ...
                const char *reason = (r->neg_passed == 0)
                                         ? "expected compilation error but "
                                           "code compiled successfully"
                                         : r->neg_actual;
                emit_test_result(fmt, disp, cur_suite, "neg_fail", reason,
                                 &json_first, test_num);
                total_neg_fail++;
                total_neg++;
                if (opts && opts->fail_fast)
                    stop_early = true;
            }
            continue;
        }
        ...
        // (safety level, individual checks, warning flags).  Since those are
        // baked in at cc_compile() time, we recompile the whole program
        // whenever the required config differs from what is currently
        // compiled. Adjacent tests sharing the same flags share one compile;
        // the unflagged base-config compile is also reused across consecutive
        // unflagged tests.
#ifdef _POSIX_VERSION
...
#endif
        {
            ...
            bool     has_any_flags = r->test_flags_mask || r->test_warn_mask ||
            ...
            if (has_any_flags) {
                req_flags =
                    (base_vm_flags & ~r->test_flags_mask) | r->test_flags_or;
                req_warnings =
                    (base_warnings & ~r->test_warn_mask) | r->test_warn_or;
                req_warn_errs = (base_warn_errs & ~r->test_warn_errors_mask) |
                                r->test_warn_errors_or;
                req_warn_ae   = r->test_warn_as_errors_set
                                    ? r->test_warn_as_errors
                                    : base_warn_ae;
            } else {
            ...
            // Determine whether the per-test ffi-allow list differs from
            // current.
            int  req_ffi_allow_count = r->test_ffi_allow_count;
            bool ffi_allow_changed =
                (req_ffi_allow_count !=
                 cur_ffi_allow_count - base_ffi_allow_count);
            if (!ffi_allow_changed && req_ffi_allow_count > 0) {
                for (int i = 0; i < req_ffi_allow_count && !ffi_allow_changed;
                     i++) {
                    if (strcmp(r->test_ffi_allow[i],
                               vm->ffi_allow_list[base_ffi_allow_count + i]) !=
                        0)
            ...
            bool needs_recompile =
                (req_flags != cur_vm_flags) || (req_warnings != cur_warnings) ||
                (req_warn_errs != cur_warn_errs) ||
                (req_warn_ae != cur_warn_ae) || ffi_allow_changed;

            if (needs_recompile) {
                // Discard any per-once-hook snapshot that references the old
                // compile.
                ...
                // globals from scratch (avoids stale bytes for zero-init
                // globals).
                ...
                vm->flags    = req_flags;
                vm->ffi_errors_fatal =
                    (req_flags & CCCC_FFI_ERRORS_FATAL) ? 1 : 0;
                vm->compiler.warnings       = req_warnings;
                vm->compiler.warning_errors = req_warn_errs;
                vm->warnings_as_errors      = req_warn_ae;
                // Apply per-test ffi-allow: truncate to base then add new
                // entries.
                if (ffi_allow_changed) {
                    for (int i = base_ffi_allow_count; i < vm->ffi_allow_count;
                         i++)
                    ...
                    cur_ffi_allow_count =
                        base_ffi_allow_count + req_ffi_allow_count;
                }

                // Capture compile-time stderr when the test asserts on it
                // (#621). This covers warnings/errors emitted by cc_compile()
                // during the recompile, which happen before the runtime
                // fd-redirect below. Do NOT redirect here for exit_code= tests
                // — those use a fork path that comes later and must not inherit
                // a hijacked stderr.
#ifdef _POSIX_VERSION
                bool needs_compile_capture =
                ...
                // Refresh the base snapshot from the newly initialised data
                // segment.
                memcpy(base_snap, vm->data_seg, snap_size);
                cur_snap      = base_snap;
        ...
        if (r->expect_exit_code >= 0) {
            ...
            if (!fn) {
                emit_test_result(fmt, disp, cur_suite, "skip", NULL,
                                 &json_first, test_num);
                total_skipped++;
                continue;
            }
            ...
            memcpy(vm->data_seg,
                   once_suite_snap(once_suite, n_once_suite, cur_snap),
                   snap_size);
            ...
            if (pid == 0) {
                ...
                _exit((unsigned char)rc); /* -1 (VM error) → 255, matching
                                             non-testing exit convention */
            }

            s_fork_child_pid = pid;
            long effective_ms =
                r->timeout_ms > 0 ? r->timeout_ms : global_timeout_ms;
            SET_TIMEOUT(effective_ms);
            if (effective_ms > 0)
                any_timeout_possible = true;
            ...
            if (fork_timed_out) {
                waitpid(pid, NULL, 0);
                emit_test_result(fmt, disp, cur_suite, "timeout", NULL,
                                 &json_first, test_num);
                total_timeouts++;
                if (opts && opts->fail_fast)
                    stop_early = true;
                ...
                if (WIFEXITED(wstatus))
                    actual_code = WEXITSTATUS(wstatus);
                else if (WIFSIGNALED(wstatus))
                    actual_code = 128 + WTERMSIG(wstatus);

                if (actual_code == r->expect_exit_code) {
                    emit_test_result(fmt, disp, cur_suite, "pass", NULL,
                                     &json_first, test_num);
                    passed++;
                } else {
                    char fail_msg[128];
                    ...
                    emit_test_result(fmt, disp, cur_suite, "fail", fail_msg,
                                     &json_first, test_num);
                    total_failures++;
                    if (opts && opts->fail_fast)
                        stop_early = true;
                ...
                if (!fork_timed_out) {
                    char td_fail[512] = {0};
                    run_hooks_guarded(vm, prog, setups, true, false, cur_suite,
                                      disp, td_fail);
                }
            }
            continue;
        ...
        if (r->expect_exit_code >= 0) {
            emit_test_result(fmt, disp, cur_suite, "skip",
                             "exit_code= requires POSIX fork", &json_first,
                             test_num);
            total_skipped++;
            continue;
        }
        ...
        Obj *fn = find_fn(prog, r->name);
        if (!fn) {
            emit_test_result(fmt, disp, cur_suite, "skip", NULL, &json_first,
                             test_num);
            total_skipped++;
            if (opts && opts->fail_fast)
                stop_early = true;
        ...
        memcpy(vm->data_seg,
               once_suite_snap(once_suite, n_once_suite, cur_snap), snap_size);
        ...
        // can be copied out of the rotating return-buffer pool immediately
        // after cc_run_at (before teardown hooks clobber the pool).
        char *ret_struct_buf  = NULL;
        int   ret_struct_size = 0;
        if (r->ret_kind == RET_STRUCT && fn->ty && fn->ty->return_ty &&
        ...
        for (TestSetupRecord *s = setups; s; s = s->next) {
            if (s->is_teardown || !s->once || s->once_fired)
                continue;
        ...
        if (namepat_once_fired) {
            // Re-capture the top-of-stack snapshot to include name_pat
            // side-effects.
        ...
        if (jval == 0) {
            ...
            // Capture return values before teardown hooks clobber the
            // registers.
            ...
            if (ret_struct_buf && ret_struct_size > 0) {
                ...
                if (st_ptr)
                    memcpy(ret_struct_buf, st_ptr, ret_struct_size);
            }
        } else if (jval == 2) {
            run.timed_out = 1;
        }
        ...
        // Prepend compile-phase stderr (#621) so pattern checks see both
        // phases.
        ...
        if (!run.timed_out) {
            ...
            if (!run.failed && r->ret_kind != RET_NONE) {
                ...
                switch (r->ret_kind) {
                    case RET_INT:
                        ret_ok = apply_cmp_op_i64(r->ret_op, ret_int,
                                                  r->ret_expect.ret_int);
                        if (!ret_ok)
                            snprintf(run.fail_msg, sizeof(run.fail_msg),
                                     "expected return value %s %" PRId64
                                     ", got %" PRId64,
                                     cmp_op_str(r->ret_op),
                    ...
                    case RET_FLOAT: {
                        double eps =
                            (r->ret_epsilon > 0.0) ? r->ret_epsilon : 1e-9;
                    ...
                    case RET_STR: {
                        ...
                        if (!ret_ok)
                            snprintf(
                                run.fail_msg, sizeof(run.fail_msg),
                                "expected return string %s \"%s\", got \"%s\"",
                                cmp_op_str(r->ret_op), exp ? exp : "(null)",
                                got ? got : "(null)");
                        break;
                    }
                    case RET_STRUCT: {
                        Type *st = (fn->ty && fn->ty->return_ty)
                                       ? fn->ty->return_ty
                                       : NULL;
                        ...
                        // For != the whole struct must differ (all-equal →
                        // mismatch).
                        char   got_str[512] = {0};
                        double eps =
                            (r->ret_epsilon > 0.0) ? r->ret_epsilon : 1e-9;
                        bool all_eq = cmp_ret_aggregate(
                            ret_struct_buf, 0, st, r->ret_expect.ret_fields,
                            eps, got_str, sizeof(got_str));

                        // For = : all fields must be equal.
                        // For !=: at least one field must differ (not all
                        // equal).
                        if (r->ret_op == CMP_EQ)
                            ret_ok = all_eq;
                        else // CMP_NE
                        ...
                        if (!ret_ok)
                            snprintf(run.fail_msg, sizeof(run.fail_msg),
                                     ...
                                     cmp_op_str(r->ret_op),
                                     r->ret_struct_text ? r->ret_struct_text
                                                        : "(struct)",
                                     got_str);
                        break;
                    }
        ...
        if (!run.timed_out && needs_capture && !run.failed) {
            if (!check_output_pattern(r->expect_stderr, captured_stderr, false,
                ...
                run.failed = 1;
            else if (!check_output_pattern(r->reject_stderr, captured_stderr,
                                           true, "expected stderr not to match",
                                           run.fail_msg))
                run.failed = 1;
            else if (!check_output_pattern(r->expect_stdout, captured_stdout,
                                           false, "expected stdout to match",
                                           run.fail_msg))
                run.failed = 1;
            else if (!check_output_pattern(r->reject_stdout, captured_stdout,
                                           true, "expected stdout not to match",
                                           run.fail_msg))
                run.failed = 1;
        }
        ...
        if (run.timed_out) {
            emit_test_result(fmt, disp, cur_suite, "timeout", NULL, &json_first,
                             test_num);
            total_timeouts++;
            if (opts && opts->fail_fast)
                stop_early = true;
        } else if (!run.failed) {
            emit_test_result(fmt, disp, cur_suite, "pass", NULL, &json_first,
                             test_num);
            passed++;
        } else {
            const char *msg = run.fail_msg[0] ? run.fail_msg : "unknown error";
            emit_test_result(fmt, disp, cur_suite, "fail", msg, &json_first,
                             test_num);
            total_failures++;
            if (opts && opts->fail_fast)
                stop_early = true;
    ...
    switch (fmt) {
        ...
        case TEST_FORMAT_PLAIN:
            ...
            if (total_neg > 0)
                printf(
                    "Negative tests: %d (correctly rejected: %d, failed: %d)\n",
