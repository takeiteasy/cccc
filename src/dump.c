...
static void dump_type_simple(FILE *f, Type *ty) {
    ...
    switch (ty->kind) {
        ...
        case TY_UNION: {
            ...
            // Prefer the tag name (struct_tag survives declarator
            // name-overwrite, #892) over ty->name, which is the *declarator*
            // being typed against this struct/union type -- e.g. `struct P p;`
            // set ty->name to "p", not "P", so printing ty->name here
            // mis-spelled every tagged-type REPL result (#666).
...
static void dump_node_list(FILE *f, const char *label, Node *node, int depth,
                           int verbose) {
    if (!node)
        return;
...
static void dump_obj_list(FILE *f, const char *label, Obj *obj, int depth,
                          int verbose) {
    if (!obj)
        return;
    print_indent(f, depth);
...
#define CC_DUMP_MAX_STRING_LEN                                                 \
    200 // bytes of a char*/char[] string before "..."
...
// validating every byte's address before dereferencing it
// (cc_is_valid_vm_address) so a garbage/uninitialized char* prints as hex
// instead of crashing the REPL -- the case that motivated validating every
// depth uniformly, not just the top-level result (#666).
...
static long long read_int_from_mem(const void *addr, int size,
                                   bool is_unsigned) {
    long long raw = 0;
...
static long long read_bitfield(const void *struct_addr, Member *m) {
    ...
    if (!m->ty->is_unsigned && m->bit_width < 64 &&
        (v & (1LL << (m->bit_width - 1))))
        v -= (1LL << m->bit_width);
    return v;
}

static void dump_value_at(FILE *f, VirtualMachine *vm, Type *ty,
                          const void *addr, int depth);

// Scalar/pointer leaf formatting, reading directly from VM memory at `addr`.
static void dump_scalar_at(FILE *f, VirtualMachine *vm, Type *ty,
                           const void *addr) {
    switch (ty->kind) {
        case TY_BOOL: {
    ...
    switch (ty->kind) {
        ...
        case TY_UNION: {
            ...
            for (Member *m = ty->members; m; m = m->next) {
                ...
                if (m->is_bitfield) {
                    ...
                    dump_value_at(f, vm, m->ty, (const char *)addr + m->offset,
                                  depth + 1);
                }
                fprintf(f, "\n");
            }
        ...
        case TY_ARRAY: {
            ...
            if (ty->base && (ty->base->kind == TY_CHAR)) {
                ...
                int cap =
                    len < CC_DUMP_MAX_STRING_LEN ? len : CC_DUMP_MAX_STRING_LEN;
                int actual = 0;
            ...
            fprintf(f, "{\n");
            int shown =
                len < CC_DUMP_MAX_ARRAY_ELEMS ? len : CC_DUMP_MAX_ARRAY_ELEMS;
            for (int i = 0; i < shown; i++) {
                print_indent(f, depth + 1);
                fprintf(f, "[%d] = ", i);
                dump_value_at(f, vm, ty->base,
                              (const char *)addr +
                                  (size_t)i * (size_t)ty->base->size,
                              depth + 1);
                fprintf(f, "\n");
            }
            if (shown < len) {
    ...
    switch (ty->kind) {
        ...
        case TY_VECTOR:
            if (ival == 0 ||
                ...
                fprintf(f, "<invalid address 0x%llx>",
...
#define JSON_FIELD(f, d, first, name)                                          \
    do {                                                                       \
        if (!(first))                                                          \
            fprintf((f), ",\n");                                               \
        (first) = false;                                                       \
