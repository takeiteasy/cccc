 ...
 * include/cccc/reflection.h) only if get_std_header("reflection.h") ever
 * stops being sufficient to bootstrap tools/generate_stdlib.c's comptime
 * code -- in practice that means "the reflection API grew a construct
 * generate_stdlib.c needs that isn't in this snapshot", which should be
 * rare, since this seed only has to get ONE compiler far enough to run
 * `-G` once per fresh clone; every subsequent build regenerates the real
 * src/std.c from disk (tools/generate_stdlib.c reads headers via
 * read_header_file(), not through this table) and uses THAT as pass 1
 * from then on. See tools/regen_stdlib.sh and build.c's stdlib_regen_step().
 *
...
static char __builtin_reflection_h[103632] = "/*\n"
