...
static void cccc_dec_raise_flags(unsigned f) {
    if (!f)
        return; // feraiseexcept() is not free -- skip the common case
    int host = 0;
    if (f & DEC_FE_INVALID)
        host |= FE_INVALID;
...
static void env_raise(int env, unsigned f) {
    if (env == CCCC_DEC_ENV_DYNAMIC)
        cccc_dec_raise_flags(f);
}
...
bool cccc_dec_binop(int op, int w, void *dst, const void *a, const void *b,
    ...
    switch (w) {
        case 0: {
            ...
            switch (op) {
                case '+':
                    r = __bid32_add(x, y, RND, &f);
                    break;
                case '-':
                    r = __bid32_sub(x, y, RND, &f);
                    break;
                case '*':
                    r = __bid32_mul(x, y, RND, &f);
                    break;
                case '/':
                    r = __bid32_div(x, y, RND, &f);
                    break;
                default:
                    return false;
            }
        ...
        case 1: {
            ...
            switch (op) {
                case '+':
                    r = __bid64_add(x, y, RND, &f);
                    break;
                case '-':
                    r = __bid64_sub(x, y, RND, &f);
                    break;
                case '*':
                    r = __bid64_mul(x, y, RND, &f);
                    break;
                case '/':
                    r = __bid64_div(x, y, RND, &f);
                    break;
                default:
                    return false;
            }
        ...
        case 2: {
            ...
            switch (op) {
                case '+':
                    r = __bid128_add(x, y, RND, &f);
                    break;
                case '-':
                    r = __bid128_sub(x, y, RND, &f);
                    break;
                case '*':
                    r = __bid128_mul(x, y, RND, &f);
                    break;
                case '/':
                    r = __bid128_div(x, y, RND, &f);
                    break;
                default:
                    return false;
            }
...
bool cccc_dec_neg(int w, void *dst, const void *a) {
    ...
    switch (w) {
        case 0:
            store32(dst, __bid32_negate(load32(a)));
            return true;
        case 1:
            store64(dst, __bid64_negate(load64(a)));
            return true;
        case 2:
            store128(dst, __bid128_negate(load128(a)));
            return true;
        default:
            return false;
    }
...
int cccc_dec_cmp(int w, const void *a, const void *b) {
    ...
    switch (w) {
        case 0: {
            ...
            if (__bid32_quiet_equal(x, y, &f))
                return 0;
            if (__bid32_quiet_less(x, y, &f))
                return 1;
            if (__bid32_quiet_greater(x, y, &f))
                return 2;
            return 3;
        }
        case 1: {
            BID_UINT64 x = load64(a), y = load64(b);
            if (__bid64_quiet_equal(x, y, &f))
                return 0;
            if (__bid64_quiet_less(x, y, &f))
                return 1;
            if (__bid64_quiet_greater(x, y, &f))
                return 2;
            return 3;
        }
        case 2: {
            BID_UINT128 x = load128(a), y = load128(b);
            if (__bid128_quiet_equal(x, y, &f))
                return 0;
            if (__bid128_quiet_less(x, y, &f))
                return 1;
            if (__bid128_quiet_greater(x, y, &f))
                return 2;
            return 3;
        }
        default:
...
bool cccc_dec_from_bin(int w, void *dst, uint64_t bits, bool src_is_f32,
    ...
    switch (w) {
        case 0:
            store32(dst, __binary64_to_bid32(d, RND, &f));
            env_raise(env, f);
            return true;
        case 1:
            store64(dst, __binary64_to_bid64(d, RND, &f));
            env_raise(env, f);
            return true;
        case 2:
            store128(dst, __binary64_to_bid128(d, RND, &f));
            env_raise(env, f);
            return true;
        default:
            return false;
...
bool cccc_dec_to_bin(int w, const void *src, bool dst_is_f32,
    ...
    switch (w) {
        case 0:
            d = __bid32_to_binary64(load32(src), RND, &f);
            break;
        case 1:
            d = __bid64_to_binary64(load64(src), RND, &f);
            break;
        case 2:
            d = __bid128_to_binary64(load128(src), RND, &f);
            break;
        default:
            return false;
    }
...
bool cccc_dec_convert(int dst_w, int src_w, void *dst, const void *src,
    ...
    switch (src_w * 3 + dst_w) {
        case 0 * 3 + 1:
            store64(dst, __bid32_to_bid64(load32(src), &f));
            env_raise(env, f);
            return true;
        case 0 * 3 + 2:
            store128(dst, __bid32_to_bid128(load32(src), &f));
            env_raise(env, f);
            return true;
        case 1 * 3 + 0:
            store32(dst, __bid64_to_bid32(load64(src), RND, &f));
            env_raise(env, f);
            return true;
        case 1 * 3 + 2:
            store128(dst, __bid64_to_bid128(load64(src), &f));
            env_raise(env, f);
            return true;
        case 2 * 3 + 0:
            store32(dst, __bid128_to_bid32(load128(src), RND, &f));
            env_raise(env, f);
            return true;
        case 2 * 3 + 1:
            store64(dst, __bid128_to_bid64(load128(src), RND, &f));
            env_raise(env, f);
            return true;
        default:
            return false;
...
static void reposition_point(char *out, size_t n, const char *sign,
    ...
    if (exp >= 0) {
        ...
        size_t pos = 0;
        for (const char *s = sign; *s && pos + 1 < n; s++)
            out[pos++] = *s;
        for (const char *s = digits; *s && pos + 1 < n; s++)
            out[pos++] = *s;
        for (long long i = 0; i < exp && pos + 1 < n; i++)
            out[pos++] = '0';
        out[pos] = '\0';
        return;
    }
    ...
    if (point <= 0) {
        ...
        if (pos + 1 < n)
            out[pos++] = '.';
        for (long long i = 0; i < -point && pos + 1 < n; i++)
            out[pos++] = '0';
        for (const char *s = digits; *s && pos + 1 < n; s++)
            out[pos++] = *s;
        ...
        if (pos + 1 < n)
            out[pos++] = '.';
        for (const char *s = digits + point; *s && pos + 1 < n; s++)
            out[pos++] = *s;
    }
    out[pos] = '\0';
}
...
int cccc_dec_format(char *buf, size_t n, const void *val, int w) {
    ...
    switch (w) {
        case 0:
            __bid32_to_string(canon, load32(val), &f);
            break;
        case 1:
            __bid64_to_string(canon, load64(val), &f);
            break;
        case 2:
            __bid128_to_string(canon, load128(val), &f);
            break;
        default:
            return -1;
    }
...
bool cccc_dec_encode_literal(const char *digits, int w, void *out) {
    ...
    switch (w) {
        case 0:
            store32(out, __bid32_from_string((char *)digits,
                                             BID_ROUNDING_TO_NEAREST, &f));
            return true;
        case 1:
            store64(out, __bid64_from_string((char *)digits,
                                             BID_ROUNDING_TO_NEAREST, &f));
            return true;
        case 2:
            store128(out, __bid128_from_string((char *)digits,
                                               BID_ROUNDING_TO_NEAREST, &f));
            return true;
        default:
            return false;
...
bool cccc_dec_from_string(int w, void *dst, const char *s, int env) {
    ...
    switch (w) {
        case 0:
            store32(dst, __bid32_from_string((char *)s, RND, &f));
            env_raise(env, f);
            return true;
        case 1:
            store64(dst, __bid64_from_string((char *)s, RND, &f));
            env_raise(env, f);
            return true;
        case 2:
            store128(dst, __bid128_from_string((char *)s, RND, &f));
            env_raise(env, f);
            return true;
        default:
            return false;
...
static size_t dec_strtod_scan_prefix(const char *s) {
    ...
    if ((s[i] == 'i' || s[i] == 'I')) {
        ...
        for (int w = 0; w < 2; w++) {
            ...
            size_t j;
            for (j = 0; j < len; j++)
                if (tolower((unsigned char)s[i + j]) != words[w][j])
                    break;
            if (j == len)
                return i + len - start;
        }
    ...
    if (s[i] == 'e' || s[i] == 'E') {
        ...
        while (isdigit((unsigned char)s[i]))
            i++;
        if (i == exp_start)
            i = save; // no exponent digits: back off
    }
    return i - start;
}
...
bool cccc_dec_strtod(int w, void *dst, const char *s, char **endptr, int env) {
    ...
    if (toklen == 0) {
        ...
        switch (w) {
            case 0:
                store32(dst, __bid32_from_string((char *)"0",
                                                 BID_ROUNDING_TO_NEAREST, &zf));
                break;
            case 1:
                store64(dst, __bid64_from_string((char *)"0",
                                                 BID_ROUNDING_TO_NEAREST, &zf));
                break;
            case 2:
                store128(dst, __bid128_from_string(
                                  (char *)"0", BID_ROUNDING_TO_NEAREST, &zf));
                break;
            default:
                if (endptr)
                    *endptr = (char *)s;
    ...
    switch (w) {
        case 0:
            store32(dst, __bid32_from_string(buf, RND, &f));
            break;
        case 1:
            store64(dst, __bid64_from_string(buf, RND, &f));
            break;
        case 2:
            store128(dst, __bid128_from_string(buf, RND, &f));
            break;
        default:
            ok = false;
            break;
...
static bool dec_decompose(const void *val, int w, DecParts *out) {
    ...
    const char *p = canon + 1;
    out->is_nan =
        (strncasecmp(p, "nan", 3) == 0 || strncasecmp(p, "snan", 4) == 0);
...
static int round_sig(const char *digits, int len, long long *exp, int keep,
    ...
    if (rd > '5') {
        ...
        if (rest_nonzero)
            ...
            round_up = false; // tie, no prior digit: "0" is even
        else
            round_up = ((digits[keep - 1] - '0') % 2) != 0;
    }
    *exp += (len - keep);
    if (keep == 0) {
        if (round_up) {
...
static void render_fixed_body(char *out, size_t n, const char *digits, int len,
    ...
    if (pointpos <= 0) {
        ...
        for (long long i = 0; i < take && pos + 1 < n; i++)
            out[pos++] = digits[i];
        for (long long i = take; i < pointpos && pos + 1 < n; i++)
            out[pos++] = '0';
    }
    ...
    for (int i = 0; i < frac && pos + 1 < n; i++) {
        long long src_idx =
            pointpos + i; // index into `digits` (may be <0 or >=len)
...
static void render_sci_body(char *out, size_t n, const char *digits, int len,
    ...
    if (ae == 0)
        ebuf[ei++] = '0';
    while (ae > 0 && ei < (int)sizeof(ebuf)) {
        ebuf[ei++]  = (char)('0' + ae % 10);
        ae         /= 10;
    }
    while (ei < 2)
        ebuf[ei++] = '0'; // C requires >=2 exponent digits
    for (int i = ei - 1; i >= 0 && pos + 1 < n; i--)
        out[pos++] = ebuf[i];
    out[pos] = '\0';
}
...
static void strip_trailing_frac_zeros(char *body) {
    ...
    while (p > dot + 1 && p[-1] == '0')
        p--;
    if (p == dot + 1)
        p = dot; // no fractional digits left: drop the point too
    if (p < stop)
        memmove(p, stop, (size_t)(end - stop) + 1);
}

int cccc_dec_format_ex(char *buf, size_t n, const void *val, int w, int conv,
                       unsigned flags, int field_width, int prec) {
    DecParts parts;
    if (!dec_decompose(val, w, &parts))
        return -1;
    ...
    if (parts.is_nan || parts.is_inf) {
        ...
        if (conv_lower == 'f') {
            ...
            if (keep < 0) {
                ...
                int keep_i =
                    keep > (long long)parts.ndigits ? parts.ndigits : (int)keep;
                rexp = parts.exp;
                rlen = round_sig(parts.digits, parts.ndigits, &rexp, keep_i,
                                 rdigits);
            }
            render_fixed_body(body, sizeof body, rdigits, rlen, rexp, p,
                              force_point);
            ...
            int       rlen =
                round_sig(parts.digits, parts.ndigits, &rexp, p + 1, rdigits);
            render_sci_body(body, sizeof body, rdigits, rlen, rexp, p,
                            force_point, upper);
        } else { // 'g'
            ...
            long long rexp = parts.exp;
            int       rlen =
                round_sig(parts.digits, parts.ndigits, &rexp, P, rdigits);
            long long X =
                (long long)rlen + rexp - 1; // decimal exponent, post-rounding
            if (X >= -4 && X < P) {
                int frac = (int)(P - 1 - X);
                if (frac < 0)
                    frac = 0;
                render_fixed_body(body, sizeof body, rdigits, rlen, rexp, frac,
                                  force_point);
            } else {
                render_sci_body(body, sizeof body, rdigits, rlen, rexp, P - 1,
                                force_point, upper);
            }
            if (!force_point)
                strip_trailing_frac_zeros(body);
        }
        if (upper)
            for (char *c = body; *c; c++)
                *c = (char)toupper((unsigned char)*c);
    ...
    if (parts.is_neg)
        sign = '-';
    else if (flags & CCCC_DECFMT_PLUS)
        sign = '+';
    else if (flags & CCCC_DECFMT_SPACE)
        sign = ' ';
    ...
    bool zero_pad = (flags & CCCC_DECFMT_ZERO) != 0 && !left && !parts.is_nan &&
                    !parts.is_inf;

    size_t pos    = 0;
    if (!left && !zero_pad) {
        for (int i = 0; i < pad && pos + 1 < n; i++) {
            if (buf)
                buf[pos] = ' ';
            pos++;
        }
    ...
    for (const char *s = rest; *s && pos + 1 < n; s++) {
        if (buf)
            buf[pos] = *s;
        pos++;
    }
...
static bool dec_unsupported(void) {
    return false;
}
