...
static int cccc_utf8_decode(const unsigned char *buf, unsigned len, uint32_t *out) {
    uint32_t cp;
    switch (len) {
    case 1:
    ...
    case 3:
        ...
        cp = (uint32_t)(buf[0] & 0x0F) << 12 | (uint32_t)(buf[1] & 0x3F) << 6 | (buf[2] & 0x3F);
        if (cp < 0x800) return -1;
        if (cp >= 0xD800 && cp <= 0xDFFF) return -1;
        break;
    case 4:
        if ((buf[1] & 0xC0) != 0x80 || (buf[2] & 0xC0) != 0x80 || (buf[3] & 0xC0) != 0x80) return -1;
        cp = (uint32_t)(buf[0] & 0x07) << 18 | (uint32_t)(buf[1] & 0x3F) << 12 |
...
static size_t cccc_mbrtoc8(cccc_char8_t *pc8, const char *s, size_t n, mbstate_t *ps) {
