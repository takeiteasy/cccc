...
static void sign_extend_top(uint64_t *a, int words, int width) {
    int rem = width & 63;
    if (rem == 0) return; // already full 64-bit word
...
static void udivmod(uint64_t *dst_q, uint64_t *dst_r,
    ...
    for (int i = 0; i < alen; i++) {
        u[i] |= a[i] << shift;
        if (shift && i + 1 <= alen) u[i + 1] = a[i] >> (64 - shift);
    }
    // Shift b left by `shift` bits into v.
    for (int i = 0; i < blen; i++) {
        v[i] |= b[i] << shift;
        if (shift && i + 1 < blen) v[i + 1] = b[i] >> (64 - shift);
    }
...
void __cccc_bitint_sshr(uint64_t *dst, const uint64_t *a, long long shift,
    ...
    if (is_negative(tmp, words, width)) {
        ...
        if (ones_bits >= width) ones_bits = width;
...
void __cccc_bitint_from_i64(uint64_t *dst, long long val, int words, int width) {
    ...
    for (int i = 1; i < words; i++) dst[i] = fill;
    __cccc_bitint_trunc(dst, words, width);
}

// Convert uint64 → wide _BitInt (zero-extend).
void __cccc_bitint_from_u64(uint64_t *dst, unsigned long long val,
                             int words, int width) {
    dst[0] = val;
    for (int i = 1; i < words; i++) dst[i] = 0;
    __cccc_bitint_trunc(dst, words, width);
}
...
void __cccc_bitint_extend(uint64_t *dst, const uint64_t *src, int words_src,
    ...
    if (words_dst > words_src) {
        ...
        if (is_signed_src) {
            ...
            if (is_negative(dst, copy_words, width_src)) fill = ~(uint64_t)0;
        }
        for (int i = copy_words; i < words_dst; i++) dst[i] = fill;
    }
...
long long __cccc_bitint_to_i64(const uint64_t *a, int words, int width,
                                int is_signed) {
    (void)words; (void)width; (void)is_signed;
    return (long long)a[0];
}

// Convert wide _BitInt → double, returned as raw IEEE-754 bit-pattern (long long).
...
void __cccc_bitint_not(uint64_t *dst, const uint64_t *a, int words, int width) {
    for (int i = 0; i < words; i++) dst[i] = ~a[i];
    __cccc_bitint_trunc(dst, words, width);
}
...
void __cccc_bitint_from_str(uint64_t *dst, const char *str, int base,
    ...
    memset(dst, 0, (size_t)words * 8);
    for (; *str && *str != '\'' ; str++) {
        if (*str == '\'') continue; // digit separator
        int digit;
        char c = *str;
...
void register_wide_bitint_functions(VirtualMachine *vm) {
    ...
    cc_register_cfunc(vm, "__cccc_bitint_udiv",   (void*)__cccc_bitint_udiv,   5, 0);
    cc_register_cfunc(vm, "__cccc_bitint_umod",   (void*)__cccc_bitint_umod,   5, 0);
    cc_register_cfunc(vm, "__cccc_bitint_sdiv",   (void*)__cccc_bitint_sdiv,   5, 0);
    cc_register_cfunc(vm, "__cccc_bitint_smod",   (void*)__cccc_bitint_smod,   5, 0);
    cc_register_cfunc(vm, "__cccc_bitint_shl",    (void*)__cccc_bitint_shl,    5, 0);
    cc_register_cfunc(vm, "__cccc_bitint_ushr",   (void*)__cccc_bitint_ushr,   5, 0);
    cc_register_cfunc(vm, "__cccc_bitint_sshr",   (void*)__cccc_bitint_sshr,   5, 0);
    cc_register_cfunc(vm, "__cccc_bitint_cmp",    (void*)__cccc_bitint_cmp,    5, 0);
    cc_register_cfunc(vm, "__cccc_bitint_trunc",  (void*)__cccc_bitint_trunc,  3, 0);
    cc_register_cfunc(vm, "__cccc_bitint_from_i64",    (void*)__cccc_bitint_from_i64,    4, 0);
    cc_register_cfunc(vm, "__cccc_bitint_from_u64",    (void*)__cccc_bitint_from_u64,    4, 0);
    cc_register_cfunc(vm, "__cccc_bitint_to_i64",      (void*)__cccc_bitint_to_i64,      4, 0);
    cc_register_cfunc(vm, "__cccc_bitint_to_double",   (void*)__cccc_bitint_to_double,   4, 0);
    cc_register_cfunc(vm, "__cccc_bitint_from_double", (void*)__cccc_bitint_from_double, 5, 0);
    cc_register_cfunc(vm, "__cccc_bitint_extend",  (void*)__cccc_bitint_extend,  7, 0);
