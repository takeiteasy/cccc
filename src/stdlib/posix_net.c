...
static long long wrap_accept_gil(long long sockfd, long long addr,
                                 long long addrlen) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)accept((int)sockfd, (struct sockaddr *)addr,
                                 (socklen_t *)addrlen);
    ExecState state;
...
static long long wrap_connect_gil(long long sockfd, long long addr,
                                  long long addrlen) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)connect((int)sockfd, (const struct sockaddr *)addr,
                                  (socklen_t)addrlen);
    ExecState state;
    cccc_posix_save_and_release_gil(vm, &state);
    int r =
...
static long long wrap_recv_gil(long long sockfd, long long buf, long long len,
                               long long flags) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)recv((int)sockfd, (void *)buf, (size_t)len,
                               (int)flags);
    ExecState state;
    cccc_posix_save_and_release_gil(vm, &state);
...
static long long wrap_send_gil(long long sockfd, long long buf, long long len,
                               long long flags) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)send((int)sockfd, (const void *)buf, (size_t)len,
                               (int)flags);
    ExecState state;
    cccc_posix_save_and_release_gil(vm, &state);
...
static long long wrap_recvfrom_gil(long long sockfd, long long buf,
                                   long long len, long long flags,
                                   long long addr, long long addrlen) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)recvfrom((int)sockfd, (void *)buf, (size_t)len,
...
static long long wrap_sendto_gil(long long sockfd, long long buf, long long len,
                                 long long flags, long long addr,
                                 long long addrlen) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)sendto((int)sockfd, (const void *)buf, (size_t)len,
...
static long long wrap_sendmsg_gil(long long sockfd, long long msg,
                                  long long flags) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)sendmsg((int)sockfd, (const struct msghdr *)msg,
                                  (int)flags);
    ExecState state;
    cccc_posix_save_and_release_gil(vm, &state);
...
static long long wrap_recvmsg_gil(long long sockfd, long long msg,
                                  long long flags) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)recvmsg((int)sockfd, (struct msghdr *)msg,
                                  (int)flags);
    ExecState state;
    cccc_posix_save_and_release_gil(vm, &state);
...
static long long wrap_gethostbyaddr_gil(long long addr, long long len,
                                        long long type) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)gethostbyaddr((const void *)addr, (socklen_t)len,
                                        (int)type);
    ExecState state;
    cccc_posix_save_and_release_gil(vm, &state);
#if !defined(_WIN32) && !defined(_WIN64)
...
static long long wrap_getaddrinfo_gil(long long node, long long service,
                                      long long hints, long long res) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
...
static long long wrap_getnameinfo_gil(long long addr, long long addrlen,
                                      long long host, long long hostlen,
                                      long long serv, long long servlen,
                                      long long flags) {
    VirtualMachine *vm = cccc_posix_current_vm();
    if (!vm || !vm->gil_initialized)
        return (long long)getnameinfo(
...
static char **nss_r_copy_ptr_array(char **list, int count, char **cursor,
                                   char *end) {
    char *p = (char *)*cursor;
...
static long long nss_gethostbyname_r_shim(long long name, long long ret,
    ...
    if (!src) {
        *resultp = 0;
        ...
        for (int i = 0; i < naliases; i++)
            need += strlen(src->h_aliases[i]) + 1;
        need =
            nss_r_layout_size(naliases, need) +
        ...
        if (need > blen) {
            ...
            *resultp = 0;
            ...
            char **aliases =
                nss_r_copy_ptr_array(src->h_aliases, naliases, &cursor, end);
            if (!aliases) {
                rc       = ERANGE;
                *resultp = 0;
                goto done;
...
static long long nss_gethostbyaddr_r_shim(long long addr, long long len,
                                          long long type, long long ret,
                                          long long buf, long long buflen,
                                          long long result,
                                          long long h_errnop) {
    struct hostent  *out     = (struct hostent *)ret;
...
static long long nss_getnetbyname_r_shim(long long name, long long ret,
                                         long long buf, long long buflen,
                                         long long result, long long h_errnop) {
        ...
        *resultp = 0;
        ...
        if (need > blen) {
            ...
            *resultp = 0;
            ...
            char **aliases =
                nss_r_copy_ptr_array(src->n_aliases, naliases, &cursor, end);
            if (!aliases) {
                rc       = ERANGE;
                *resultp = 0;
                goto done3;
...
static long long wrap_gethostbyname_r_gil(long long name, long long ret,
                                          long long buf, long long buflen,
                                          long long result,
                                          long long h_errnop) {
#ifdef __linux__
    ...
    if (gil)
        cccc_posix_save_and_release_gil(vm, &state);
    int rc = gethostbyname_r((const char *)name, (struct hostent *)ret,
...
static long long wrap_gethostbyaddr_r_gil(long long addr, long long len,
                                          long long type, long long ret,
                                          long long buf, long long buflen,
                                          long long result,
                                          long long h_errnop) {
    ...
    return (long long)rc;
#else
    return nss_gethostbyaddr_r_shim(addr, len, type, ret, buf, buflen, result,
                                    h_errnop);
#endif
}

static long long wrap_getnetbyname_r_gil(long long name, long long ret,
                                         long long buf, long long buflen,
                                         long long result, long long h_errnop) {
#ifdef __linux__
    VirtualMachine *vm = cccc_posix_current_vm();
    ...
    if (gil)
        cccc_posix_save_and_release_gil(vm, &state);
    int rc = getnetbyname_r((const char *)name, (struct netent *)ret,
                            (char *)buf, (size_t)buflen,
...
static long long wrap_htonl(long long hostlong) {
    return (long long)htonl((uint32_t)hostlong);
}
static long long wrap_htons(long long hostshort) {
    return (long long)htons((uint16_t)hostshort);
}
static long long wrap_ntohl(long long netlong) {
    return (long long)ntohl((uint32_t)netlong);
}
static long long wrap_ntohs(long long netshort) {
    return (long long)ntohs((uint16_t)netshort);
}
static long long wrap_inet_addr(long long cp) {
    return (long long)inet_addr((const char *)cp);
}
...
static long long wrap_bzero(long long s, long long n) {
    bzero((void *)s, (size_t)n);
    return 0;
}
static long long wrap_bcopy(long long src, long long dst, long long n) {
    bcopy((const void *)src, (void *)dst, (size_t)n);
    return 0;
}
static long long wrap_freeaddrinfo(long long res) {
...
void register_posix_net_functions(VirtualMachine *vm) {
    ...
    cc_register_cfunc(vm, "gethostbyname", (void *)wrap_gethostbyname_gil, 1,
                      0);
    cc_register_cfunc(vm, "gethostbyaddr", (void *)wrap_gethostbyaddr_gil, 3,
                      0);
    ...
    cc_register_cfunc(vm, "gethostbyname_r", (void *)wrap_gethostbyname_r_gil,
                      6, 0);
    cc_register_cfunc(vm, "gethostbyaddr_r", (void *)wrap_gethostbyaddr_r_gil,
                      8, 0);
    cc_register_cfunc(vm, "getnetbyname_r", (void *)wrap_getnetbyname_r_gil, 6,
                      0);
