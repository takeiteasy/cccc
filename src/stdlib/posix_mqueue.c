...
// honored the same way aio's is, via cccc_posix_sigevent_prepare() above
// (struct sigevent has the same layout regardless of which header pulled it
// in).
...
static long long wrap_mq_send(long long mqdes, long long msg_ptr,
                              long long msg_len, long long msg_prio) {
    VirtualMachine *vm = cccc_posix_current_vm();
    const char     *p  = (const char *)(intptr_t)msg_ptr;
    if (!vm || !vm->gil_initialized)
        return (long long)mq_send((mqd_t)mqdes, p, (size_t)msg_len,
...
static long long wrap_mq_receive(long long mqdes, long long msg_ptr,
                                 long long msg_len, long long msg_prio) {
    VirtualMachine *vm   = cccc_posix_current_vm();
...
static long long wrap_mq_timedsend(long long mqdes, long long msg_ptr,
                                   long long msg_len, long long msg_prio,
    ...
    if (!vm || !vm->gil_initialized)
        return (long long)mq_timedsend((mqd_t)mqdes, p, (size_t)msg_len,
                                       (unsigned int)msg_prio, ts);
    ExecState state;
    cccc_posix_save_and_release_gil(vm, &state);
    int r = mq_timedsend((mqd_t)mqdes, p, (size_t)msg_len,
                         (unsigned int)msg_prio, ts);
    cccc_posix_acquire_and_restore_gil(vm, &state);
    return (long long)r;
}

static long long wrap_mq_timedreceive(long long mqdes, long long msg_ptr,
                                      long long msg_len, long long msg_prio,
                                      long long abs_timeout) {
    VirtualMachine        *vm   = cccc_posix_current_vm();
    char                  *p    = (char *)(intptr_t)msg_ptr;
    ...
    const struct timespec *ts = (const struct timespec *)(intptr_t)abs_timeout;
    if (!vm || !vm->gil_initialized)
        return (long long)mq_timedreceive((mqd_t)mqdes, p, (size_t)msg_len,
                                          prio, ts);
    ExecState state;
    cccc_posix_save_and_release_gil(vm, &state);
...
static long long wrap_mq_notify(long long mqdes, long long notification) {
    ...
    if (cookie_idx >= 0)
        ...
        mq_notify_binding_clear(
            (mqd_t)mqdes); // SIGEV_NONE/SIGEV_SIGNAL replacing a prior THREAD
                           // registration
    return 0;
}

static long long wrap_mq_setattr(long long mqdes, long long newattr,
                                 long long oldattr) {
    return (long long)mq_setattr((mqd_t)mqdes,
                                 (const struct mq_attr *)(intptr_t)newattr,
                                 (struct mq_attr *)(intptr_t)oldattr);
}

static long long wrap_mq_getattr(long long mqdes, long long attr) {
    return (long long)mq_getattr((mqd_t)mqdes,
                                 (struct mq_attr *)(intptr_t)attr);
}
#endif
...
void register_posix_mqueue_functions(VirtualMachine *vm) {
    ...
    cc_register_cfunc(vm, "mq_timedreceive", (void *)wrap_mq_timedreceive, 5,
                      0);
