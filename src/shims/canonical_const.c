...
// >>> shim: nl_item_xlate
static int __cccc_native_guest_to_host_nl_item(int guest_item, long *host_item) {
#ifdef __APPLE__
    *host_item = guest_item;
    ...
    switch (v) {
        ...
        default:
            if (v >= 7 && v <= 13) *host_item = 131079 + (v - 7);
            else if (v >= 14 && v <= 20) *host_item = 131072 + (v - 14);
            else if (v >= 21 && v <= 32) *host_item = 131098 + (v - 21);
            else if (v >= 33 && v <= 44) *host_item = 131086 + (v - 33);
            else return 0;
            break;
    }
    return 1;
...
static locale_t __cccc_native_newlocale(int guest_mask, const char *locale, locale_t base) {
    return newlocale(__cccc_native_guest_to_host_lc_mask(guest_mask), locale, base);
}
// <<< shim
...
static int __cccc_native_sched_get_priority_min(int policy) {
    return sched_get_priority_min(__cccc_native_guest_to_host_sched_policy(policy));
}
// <<< shim

// >>> shim: sched_get_priority_max
static int __cccc_native_sched_get_priority_max(int policy) {
    return sched_get_priority_max(__cccc_native_guest_to_host_sched_policy(policy));
}
// <<< shim
...
static long __cccc_native_sysconf(int name) {
    switch (name) {
        ...
        case 12: return (long)sysconf(_SC_NPROCESSORS_CONF);
#endif
#ifdef _SC_NPROCESSORS_ONLN
        case 13: return (long)sysconf(_SC_NPROCESSORS_ONLN);
#endif
        ...
        case 19: return (long)sysconf(_SC_HOST_NAME_MAX);
#endif
#ifdef _SC_LOGIN_NAME_MAX
        case 20: return (long)sysconf(_SC_LOGIN_NAME_MAX);
#endif
#ifdef _SC_TTY_NAME_MAX
        case 21: return (long)sysconf(_SC_TTY_NAME_MAX);
#endif
        ...
        case 25: return (long)sysconf(_SC_GETPW_R_SIZE_MAX);
#endif
#ifdef _SC_GETGR_R_SIZE_MAX
        case 26: return (long)sysconf(_SC_GETGR_R_SIZE_MAX);
#endif
#ifdef _SC_MONOTONIC_CLOCK
        case 27: return (long)sysconf(_SC_MONOTONIC_CLOCK);
#endif
