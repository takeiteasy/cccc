...
//
// #1214: local-register promotion (#249), the restrict-value cache (#654 /
// #750), the restrict-derived-locals analysis and indexed-deref extraction
// were all opt_level >= 2 codegen optimisations. The VM has no optimiser any
// more, so the analysis and emission are gone. The entry points below remain
// as inert seams: promotion never fires (nothing is ever promoted) and the
// restrict cache never fills, so every deref takes the ordinary checked
// path. #1219 tracks deleting these seams, their Compiler-struct fields, and
// their now-trivial call sites outright.
