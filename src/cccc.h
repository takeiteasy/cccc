...
#define OPS_X                                                                        \
    ...
    X(CALC, 0)                                                                       \
    X(REALCA,                                                                        \
      0) /* reallocarray: ptr=A0, nmemb=A1, size=A2, result=A0 (#699) */             \
    X(MALCA, 0) /* aligned_alloc: size=A0, alignment=A1, result=A0 */                \
    X(PMEMA,                                                                         \
      0) /* posix_memalign: memptr=A0, alignment=A1, size=A2, result=A0 */           \
    ...
    X(STKTAG,                                                                        \
      5) /* Tag an aggregate local's [bp+offset, bp+offset+size) extent              \
    ...
    X(LDR_H,                                                                         \
      1) /* regs[rd] = *(short*)regs[rs] (load halfword, sign-extend) */             \
    ...
    X(R2FR, 1) /* fregs[rd] = *(double*)&regs[rs] (bit-pattern transfer,             \
               reverse of FR2R) */                                                   \
    ...
    X(FROUND_F32,                                                                    \
      1)           /* fregs[rd] = (float)fregs[rs] (rounded to float prec) */        \
    ...
    X(DYNOBJSZ, 3) /* rd = runtime object byte-size at regs[rs].                     \
                      Reads AllocHeader.requested_size for VM heap                   \
                      allocations; falls back to (size_t)-1 (type 0/1) or 0          \
                      (type 2/3) for non-heap, freed, or unknown pointers.           \
    ...
    X(VLDR,                                                                          \
      1) /* vregs[rd] = <width> raw bytes at regs[rs] (unaligned-safe) */            \
    X(VSTR, 1) /* <width> raw bytes at regs[rs] = vregs[rd] */                       \
    X(VMOV3,                                                                         \
      1)       /* vregs[rd] = vregs[rs1] (full-register copy, all 64 bytes) */       \
    ...
    X(VADD_F64, 1)     /* vregs[rd].f64[i] = vregs[rs1].f64[i] +                     \
                          vregs[rs2].f64[i], i<count */                              \
    X(VSUB_F64, 1)     /* ditto, - */                                                \
    X(VMUL_F64, 1)     /* ditto, * */                                                \
    X(VDIV_F64, 1)     /* ditto, / */                                                \
    X(VNEG_F64, 1)     /* vregs[rd].f64[i] = -vregs[rs1].f64[i], i<count */          \
    X(VADD_F32, 1)     /* vregs[rd].f32[i] = vregs[rs1].f32[i] +                     \
                          vregs[rs2].f32[i], i<count */                              \
    X(VSUB_F32, 1)     /* ditto, - */                                                \
    X(VMUL_F32, 1)     /* ditto, * */                                                \
    X(VDIV_F32, 1)     /* ditto, / */                                                \
    X(VNEG_F32, 1)     /* vregs[rd].f32[i] = -vregs[rs1].f32[i], i<count */          \
    X(VADD_I64, 1)     /* vregs[rd].i64[i] = vregs[rs1].i64[i] +                     \
                          vregs[rs2].i64[i], i<count */                              \
    X(VSUB_I64, 1)     /* ditto, - */                                                \
    X(VMUL_I64, 1)     /* ditto, * */                                                \
    X(VNEG_I64, 1)     /* vregs[rd].i64[i] = -vregs[rs1].i64[i], i<count */          \
    X(VADD_I32, 1)     /* vregs[rd].i32[i] = vregs[rs1].i32[i] +                     \
                          vregs[rs2].i32[i], i<count */                              \
    X(VSUB_I32, 1)     /* ditto, - */                                                \
    X(VMUL_I32, 1)     /* ditto, * */                                                \
    X(VNEG_I32, 1)     /* vregs[rd].i32[i] = -vregs[rs1].i32[i], i<count */          \
    X(VADD_I16, 1)     /* vregs[rd].i16[i] = vregs[rs1].i16[i] +                     \
                          vregs[rs2].i16[i], i<count */                              \
    X(VSUB_I16, 1)     /* ditto, - */                                                \
    X(VMUL_I16, 1)     /* ditto, * */                                                \
    X(VNEG_I16, 1)     /* vregs[rd].i16[i] = -vregs[rs1].i16[i], i<count */          \
    X(VADD_I8,                                                                       \
      1) /* vregs[rd].i8[i] = vregs[rs1].i8[i] + vregs[rs2].i8[i], i<count */        \
    X(VSUB_I8, 1) /* ditto, - */                                                     \
    X(VMUL_I8, 1) /* ditto, * */                                                     \
    ...
    X(VAND, 1) /* vregs[rd].i64[i] = vregs[rs1].i64[i] & vregs[rs2].i64[i],          \
                  i<words */                                                         \
    X(VOR, 1)  /* ditto, | */                                                        \
    X(VXOR, 1) /* ditto, ^ */                                                        \
    X(VNOT, 1) /* vregs[rd].i64[i] = ~vregs[rs1].i64[i], i<words */                  \
    /* Integer lane division/modulo (tracker #715): traps on divide-by-zero          \
       and INT_MIN/-1 overflow, same policy as scalar DIVC/MODC. */                  \
    X(VDIV_I64, 1) /* vregs[rd].i64[i] = vregs[rs1].i64[i] /                         \
                      vregs[rs2].i64[i], traps on 0 or overflow, i<count */          \
    X(VDIV_I32, 1) /* ditto, i32 lanes */                                            \
    X(VDIV_I16, 1) /* ditto, i16 lanes */                                            \
    X(VDIV_I8, 1)  /* ditto, i8 lanes */                                             \
    X(VMOD_I64, 1) /* vregs[rd].i64[i] = vregs[rs1].i64[i] %                         \
                      vregs[rs2].i64[i], traps on 0 or overflow, i<count */          \
    ...
    /* Comparisons (tracker #715): GCC semantics -- per-lane all-ones (-1) if        \
       ...
       VCLT/VCLE compare the signed view; VCLTU/VCLEU compare the unsigned           \
       view (int lanes only -- float lanes are always signed-ordered).               \
       `>`/`>=` are parsed as swapped-operand `<`/`<=`, so no separate opcodes       \
       are needed. */                                                                \
    X(VCEQ_F64, 1)  /* vregs[rd].i64[i] = (vregs[rs1].f64[i] ==                      \
                       vregs[rs2].f64[i]) ? -1 : 0, i<count */                       \
    X(VCNE_F64, 1)  /* ditto, != */                                                  \
    X(VCLT_F64, 1)  /* ditto, < */                                                   \
    X(VCLE_F64, 1)  /* ditto, <= */                                                  \
    X(VCEQ_F32, 1)  /* vregs[rd].i32[i] = (vregs[rs1].f32[i] ==                      \
                       vregs[rs2].f32[i]) ? -1 : 0, i<count */                       \
    X(VCNE_F32, 1)  /* ditto, != */                                                  \
    X(VCLT_F32, 1)  /* ditto, < */                                                   \
    X(VCLE_F32, 1)  /* ditto, <= */                                                  \
    X(VCEQ_I64, 1)  /* vregs[rd].i64[i] = (vregs[rs1].i64[i] ==                      \
                       vregs[rs2].i64[i]) ? -1 : 0, i<count */                       \
    X(VCNE_I64, 1)  /* ditto, != */                                                  \
    X(VCLT_I64, 1)  /* ditto, signed < */                                            \
    X(VCLE_I64, 1)  /* ditto, signed <= */                                           \
    X(VCLTU_I64, 1) /* ditto, unsigned < */                                          \
    X(VCLEU_I64, 1) /* ditto, unsigned <= */                                         \
    X(VCEQ_I32, 1)  /* vregs[rd].i32[i] = (vregs[rs1].i32[i] ==                      \
                       vregs[rs2].i32[i]) ? -1 : 0, i<count */                       \
    X(VCNE_I32, 1)  /* ditto, != */                                                  \
    X(VCLT_I32, 1)  /* ditto, signed < */                                            \
    X(VCLE_I32, 1)  /* ditto, signed <= */                                           \
    X(VCLTU_I32, 1) /* ditto, unsigned < */                                          \
    X(VCLEU_I32, 1) /* ditto, unsigned <= */                                         \
    X(VCEQ_I16, 1)  /* vregs[rd].i16[i] = (vregs[rs1].i16[i] ==                      \
                       vregs[rs2].i16[i]) ? -1 : 0, i<count */                       \
    X(VCNE_I16, 1)  /* ditto, != */                                                  \
    X(VCLT_I16, 1)  /* ditto, signed < */                                            \
    X(VCLE_I16, 1)  /* ditto, signed <= */                                           \
    X(VCLTU_I16, 1) /* ditto, unsigned < */                                          \
    X(VCLEU_I16, 1) /* ditto, unsigned <= */                                         \
    X(VCEQ_I8, 1)  /* vregs[rd].i8[i] = (vregs[rs1].i8[i] == vregs[rs2].i8[i])       \
                      ? -1 : 0, i<count */                                           \
    X(VCNE_I8, 1)  /* ditto, != */                                                   \
    ...
    /* Select (tracker #715): GCC vector ?: -- nonzero-per-lane condition.           \
       rd is pre-loaded with the else-arm by codegen; VSEL then overwrites           \
       only the lanes where cond is nonzero, leaving the rest (the else values       \
    ...
    X(VSEL_8, 1)  /* vregs[rd].i8[i]  = vregs[rcond].i8[i]  ?                        \
                     vregs[rthen].i8[i]  : vregs[rd].i8[i], i<count */               \
    ...
    /* __builtin_convertvector (tracker #715): same lane count on both sides,        \
       ...
       operand as with every other op. Integer conversion truncates toward           \
       zero (C cast semantics). */                                                   \
    X(VCVT_I32_F32, 1) /* vregs[rd].i32[i] = (int32_t)vregs[rs1].f32[i],             \
                          truncating, i<count */                                     \
    X(VCVT_F32_I32,                                                                  \
      1) /* vregs[rd].f32[i] = (float)vregs[rs1].i32[i], i<count */                  \
    X(VCVT_I64_F64, 1) /* vregs[rd].i64[i] = (int64_t)vregs[rs1].f64[i],             \
                          truncating, i<count */                                     \
    X(VCVT_F64_I64,                                                                  \
      1) /* vregs[rd].f64[i] = (double)vregs[rs1].i64[i], i<count */                 \
                                                                                     \
    ...
    X(DCMP, 0)   /* a(A0)=addr, b(A1)=addr, width(A2) -> A0 =                        \
                    0=EQ/1=LT/2=GT/3=UNORDERED */                                    \
    X(DFROMI, 0) /* dst(A0)=addr, val(A1)=int64, width(A2), is_unsigned(A3) */       \
    X(DTOI, 0)   /* src(A0)=addr, width(A1), is_unsigned(A2) -> A0 = int64           \
                    (truncating) */                                                  \
    X(DFROMBITS, 0) /* dst(A0)=addr, bits(A1)=raw f32/f64 bit pattern (via           \
                       FR2R), width(A2), src_is_f32(A3) */                           \
    ...
    X(DFMT,                                                                          \
      0)       /* buf(A0), n(A1), val(A2)=addr, width(A3)                            \
...
typedef enum {
    ...
    CCCC_FMA = (1 << 23), // 0x00800000 - FMA fusion codegen (--fma)
...
static inline bool cc_heap_reclaim_flags_ok(uint32_t flags) {
    return !(flags &
             (CCCC_POINTER_CHECKS | CCCC_TYPE_CHECKS | CCCC_UNINIT_DETECTION |
              CCCC_MEMORY_LEAK_DETECT | CCCC_HEAP_CANARIES));
}
...
typedef enum {
    ...
    CCCC_WARN_FALLTHROUGH =
        (1ULL << 21), // switch case falls through without [[fallthrough]]
    CCCC_WARN_STATIC_ARRAY_SIZE =
        (1ULL << 22), // [static N] array param: argument too small
    CCCC_WARN_STRICT_PROTOTYPES = (1ULL << 23), // () without void in pre-C23
    CCCC_WARN_DISCARDED_QUALIFIERS =
        (1ULL << 24), // const/volatile/restrict discarded in pointer assignment

    // Ticket #410: lower-priority / safety-overlap flags
    CCCC_WARN_NULL_DEREFERENCE =
        (1ULL << 25), // no-op; runtime safety covers at -S2/-S3
    CCCC_WARN_RESTRICT = (1ULL << 26), // no-op; runtime safety covers
    CCCC_WARN_ARRAY_BOUNDS =
        (1ULL << 27), // no-op; runtime safety covers at -S2/-S3
    CCCC_WARN_STRINGOP_OVERFLOW = (1ULL << 28), // no-op; runtime safety covers
    CCCC_WARN_STRINGOP_TRUNCATION =
        (1ULL << 29),                           // no-op; runtime safety covers
    CCCC_WARN_DUPLICATED_BRANCHES = (1ULL << 30), // identical then/else bodies
    CCCC_WARN_DUPLICATED_COND =
        (1ULL << 31), // same condition repeated in if/else-if chain
    CCCC_WARN_UNUSED_VALUE =
        (1ULL << 32), // expression result discarded, no side effects
    ...
    CCCC_WARN_SHIFT_NEGATIVE_VALUE =
        (1ULL << 38),                     // shift amount is a negative constant
    CCCC_WARN_SHIFT_OVERFLOW =
        (1ULL << 39), // shift amount >= promoted type bit-width
    CCCC_WARN_LOGICAL_OP = (1ULL << 40), // constant operand in && or ||
    CCCC_WARN_TAUTOLOGICAL_COMPARE =
        (1ULL << 41),                // comparison always true or always false
    CCCC_WARN_SIZEOF_POINTER_MEMACCESS =
        (1ULL << 42),                // sizeof(ptr) passed as size to mem fn
    CCCC_WARN_SWITCH = (1ULL << 43), // missing enum case, no default
    CCCC_WARN_SWITCH_ENUM =
        (1ULL << 44),                // missing enum case, even with default
    CCCC_WARN_ENUM_COMPARE =
        (1ULL << 45), // == / != / < etc. between different named enums
    CCCC_WARN_OLD_STYLE_DEFINITION =
        (1ULL << 46), // K&R-style function parameter declarations

    // Ticket #469: pointer safety, cast quality, and prototype flags
    CCCC_WARN_INCOMPATIBLE_POINTER_TYPES =
        (1ULL << 47), // implicit pointer type mismatch (not void*)
    CCCC_WARN_CAST_QUAL =
        (1ULL << 48), // explicit cast drops const/volatile/restrict
    CCCC_WARN_CAST_ALIGN =
        (1ULL << 49), // explicit cast raises pointer alignment requirement
    CCCC_WARN_MISSING_PROTOTYPES =
        (1ULL << 50), // external fn defined without prior full prototype
    CCCC_WARN_MISSING_DECLARATIONS =
        (1ULL << 51), // external fn defined without any prior declaration

    // Ticket #471: misc analysis flags
    CCCC_WARN_REDUNDANT_DECLS =
        (1ULL << 52), // same-linkage declaration seen twice in same scope
    CCCC_WARN_OVERRIDE_INIT =
        (1ULL << 53), // later designator overrides earlier initializer
    CCCC_WARN_UNUSED_MACROS = (1ULL << 54), // #define that is never expanded
    CCCC_WARN_NONNULL       = (1ULL << 55), // null passed to a nonnull param /
                                            // returned from returns_nonnull
    CCCC_WARN_MAYBE_NONNULL =
        (1ULL << 56), // maybe-null (post-branch-merge) value reaching a nonnull
                      // param / returns_nonnull return; opt-in, not in
                      // -Wall/-Wextra (#687)
    CCCC_WARN_SENTINEL =
        (1ULL << 57), // missing/non-literal NULL terminator in a call to a
                      // sentinel-marked variadic function (#658)
    CCCC_WARN_DESIGNATED_INIT =
        (1ULL << 58), // positional member initializer of a struct declared
                      // __attribute__((designated_init)); opt-in, not in
                      // -Wall/-Wextra (#659)
    CCCC_WARN_INT_CONVERSION =
        (1ULL << 59), // implicit integer <-> pointer conversion with no cast
    CCCC_WARN_NATIVE_NAME_COLLISION =
        (1ULL << 60), // -m/-c=native/-c=generated: a colliding name reaches the
    ...
    CCCC_WARN_ALL =
        ...
        CCCC_WARN_CPP | CCCC_WARN_EXTRA_TOKENS | CCCC_WARN_LARGE_FILE_EMBED |
        CCCC_WARN_CCCC_MACRO | CCCC_WARN_IGNORED_FEATURES |
        CCCC_WARN_ATTRIBUTES | CCCC_WARN_NODISCARD |
        CCCC_WARN_STATIC_ARRAY_SIZE | CCCC_WARN_DISCARDED_QUALIFIERS |
        CCCC_WARN_MULTICHAR | CCCC_WARN_MAIN | CCCC_WARN_SWITCH_DEFAULT |
        CCCC_WARN_SWITCH_BOOL | CCCC_WARN_FLOAT_EQUAL |
        CCCC_WARN_SHIFT_NEGATIVE_VALUE | CCCC_WARN_SHIFT_OVERFLOW |
        CCCC_WARN_LOGICAL_OP | CCCC_WARN_TAUTOLOGICAL_COMPARE |
...
typedef enum {
    ...
    TK_MACRO_SCOPE_PUSH, // Synthetic: push macro-table snapshot (#283); never
                         // from lexer
...
typedef enum {
    ...
    TY_BITINT =
        20, // C23 _BitInt(N), N in [1,256]; N>64 uses multi-word storage
...
struct Type {
    ...
    bool           is_flexible;
    bool           is_packed;
    bool designated_init; // __attribute__((designated_init)): all initializers
                          // of this struct type must be designated (#659)
    Token *
        struct_tag; // tag name token (survives declarator name-overwrite, #892)
    ...
    // Nonnull argument / return checking (__attribute__((nonnull /
    // returns_nonnull)))
    ...
    int sentinel_pos; // trailing non-sentinel args allowed before the NULL (0 =
                      // last arg)
    ...
    int alloc_size_idx2; // second factor (calloc-style product), 0 if 1-arg
                         // form
    ...
    int  init_priority; // CCCC_NO_INIT_PRIORITY unless an explicit priority was
                        // given
...
// constructors and first for destructors relative to explicitly prioritised
// ones.
...
typedef enum {
    ...
    ND_UNREACHABLE =
        53, // __builtin_unreachable() - code path that must not execute
    ND_BITOP =
        54, // Bit-manipulation builtins; val = (op<<8)|bit_width, lhs = arg
    ND_OVERFLOW_ARITH = 55, // Checked arithmetic; val = 0/1/2 (add/sub/mul),
                            // lhs=a, rhs=b, cas_addr=ptr
    ND_INIT_SPLICE =
        56,                 // Deferred compound-literal $@k splice; expanded by
            // quote_substitute var=lvar, lhs=ND_VAR($@k placeholder)
    ND_ALOAD = 57, // Atomic load via __builtin_atomic_load; lhs=addr ptr
    ND_ASTORE =
        58, // Atomic store via __builtin_atomic_store; lhs=addr ptr, rhs=value
    ND_RETURN_ADDR =
        59, // __builtin_return_address(n) - returns return address n frames up
            // val = level (compile-time constant); ty = void*
    ND_DYNOBJ_SIZE =
        60, // __builtin_dynamic_object_size(ptr, type) — runtime heap size.
            // lhs = ptr expr (evaluated at runtime); val = type arg (0-3);
            // ty = size_t (ulong).  Emits DYNOBJSZ opcode.
    ND_CONVERTVECTOR = 61, // __builtin_convertvector(expr, type) (tracker
                           // #715). lhs = source vector expr (already
                           // add_type'd at parse time); ty = target vector type
                           // (preset at parse time, like ND_CAST via new_cast)
...
struct Node {
    ...
    int cleanup_scope_depth; // parse-time cleanup_scope_depth when this block
                             // was built
    ...
    Obj *var;
    struct Node
        *init_tail; // Tail expressions after a deferred initializer splice
    ...
    struct Node
        *va_last;        // VA_START only: the `last` fixed-parameter expression
...
struct Obj {
    ...
    Node *objsize_init_assign; // the initializer ND_ASSIGN node (exempt from
                               // poisoning)
    ...
    bool has_pending_macro_init; // gvar init deferred: init expr contains
                                 // ND_MACRO_CALL
    ...
    struct Obj *
        block_outer_locals; // Parent scope's locals at definition time
                            ...
                            // block defined inside a nested function can reach
                            // a variable owned by that function's own ancestor.
    ...
    long long code_addr;     /**< For functions compiled to VM bytecode: start
    ...
    // Populated on first query; keyed by (long long)(intptr_t)var_ptr →
    // var_ptr. Free with hashmap_deinit_borrowed after codegen completes.
    HashMap local_set;
...
/*!
 ...
 @details Macro functions are functions marked with [[cccc::comptime]] or
             __attribute__((comptime)) that execute during compilation to
 generate or transform AST nodes.
*/
typedef struct MacroFn {
    char  *name;        /**< Function name. */
    Token *body_tokens; /**< Original token stream for function body (from
                           preprocessor). */
    Obj *compiled_fn;   /**< Compiled function object (NULL until compiled). */
    bool is_compiled;   /**< True after successful compilation. */
    bool
        is_macro_entry; /**< True if callable from user program macro call sites
                           (always true for [[cccc::comptime]] functions). */
    bool is_void_macro; /**< True if declared with void return type
                           (definition-only). */
...
struct TestFnRecord {
    ...
    char
        *display_name; // human-readable name from name = "..."; NULL = use name
    ...
    CmpOp
        error_count_op; // CMP_NONE = not set; otherwise the comparison operator
    ...
    union {
        ...
        TestRetField
            *ret_fields;   // for RET_STRUCT (linked list of expected fields)
    } ret_expect;
    char
        *ret_struct_text; // raw source span of compound literal (for error msg)
    double ret_epsilon;   // 0.0 = use default 1e-9; set by return_epsilon=
    int expect_exit_code; // -1 = disabled; >=0 = expected shell-convention exit
                          // code
...
// A setup or teardown function registered via [[cccc::test_setup]] /
// [[cccc::test_teardown]].
...
typedef struct ComptimeVar {
    ...
    ComptimeVarMember
        *members;      // Per-field values for struct vars (NULL for scalars)
...
typedef struct VarScopeNode {
    ...
    int64_t enum_val; /**< Enum constant value (int64_t to support C23 wide
                         underlying types). */
    bool  is_deprecated;
    char *deprecated_msg;
...
*/
typedef void (*AsmCallback)(VirtualMachine *vm, const char *asm_str,
                            void *user_data);
...
typedef struct ForeignFunc {
    ...
    size_t
        name_len; // strlen(name), cached to avoid O(n) strlen per lookup (#164)
    void *func_ptr;     /**< Pointer to the native C function. */
    int   num_args;     /**< Number of arguments the function expects (total for
                           non-variadic, fixed args for variadic). */
    int returns_double; /**< True if function returns double, false if returns
                           long long. */
    int returns_float;  /**< True if function returns a single-precision float
                           (mutually exclusive with returns_double). */
    int is_variadic; /**< True if function is variadic (accepts ... arguments).
                      */
    int num_fixed_args; /**< For variadic functions: number of fixed args before
...
typedef enum {
    ...
    ALLOC_KIND_FRAME = 1,     /**< VLA backing storage (ALCA); block-scoped --
                                 reclaimed at HREL and LEV3. */
    ALLOC_KIND_BLOCK_BOX = 2, /**< __block variable's heap box (ALCB); may
                                 outlive its frame -- never reclaimed. */
    ALLOC_KIND_ALLOCA = 3,    /**< Bare __builtin_alloca storage (ALCA);
                                 frame-scoped -- reclaimed at LEV3 only. */
} AllocKind;
...
typedef struct AllocHeader {
    size_t size; /**< Size of allocation (excluding header), rounded up for
                    alignment */
    ...
    long long canary; /**< Front canary value for heap overflow detection (when
                         enabled) */
    int freed;        /**< Flag indicating if this block has been freed (for UAF
                         detection) */
...
typedef struct StackVarMeta {
    ...
    long long offset; /**< Offset from BP (negative for locals, positive for
                         params). */
    Type *ty;         /**< Type information for the variable. */
    int   scope_id;   /**< Unique identifier for the scope where variable was
                         declared. */
    int is_alive;     /**< 1 if variable is in scope, 0 if out of scope. */
    int initialized;  /**< 1 if variable has been initialized, 0 if
                         uninitialized. */
...
typedef struct ScopeVarNode {
    ...
    struct ScopeVarNode
        *next;          /**< Pointer to the next variable in the scope. */
} ScopeVarNode;
...
typedef struct SourceMap {
    long long pc_offset; /**< Offset in text segment (bytecode) where this
                            mapping applies. */
...
/*!
 @brief Secondary index keyed by (file, line_no) for O(log n) breakpoint
 lookups.
*/
...
typedef struct DebugSymbol {
    ...
    long long offset; /**< Offset from BP (negative for locals) or address in
...
typedef struct TypeNameRecord {
    ...
    // (-c=native, -m without -c=generated) to avoid re-emitting a definition
    // the consumer's own #include already provides -- e.g. `typedef void FILE;`
...
typedef struct Breakpoint {
    ...
    char *condition; /**< Optional condition expression source (NULL if
                        unconditional). */
    struct Obj
        *cond_fn; /**< Compiled condition wrapper function (NULL until the first
                     hit compiles it, or compilation fails -- see
                     cond_compile_failed). Compiled once per breakpoint (its pc
                     fixes the enclosing function and therefore every local's
                     frame offset) and reused on every later hit; only the live
                     frame pointer varies between hits (ticket 113). */
    bool cond_compile_failed; /**< Set when compiling `condition` failed, so
                                 later hits don't retry and re-print the same
...
typedef struct ArenaBlock {
    char *base;              /**< Pointer to the start of the memory block (from
                                mmap/VirtualAlloc). */
...
typedef struct Arena {
    ...
    ArenaBlock
        *blocks; /**< Linked list of all allocated blocks (for cleanup). */
    size_t
        default_block_size; /**< Default size for new blocks (typically 1MB). */
} Arena;
...
typedef struct Compiler {
    ...
    TestSetupRecord
        *test_setups; // Linked list of [[cccc::test_setup/teardown]] records
    BuildFnRecord
        *build_fns;   // Linked list of [[cccc::build]] entry function names
    BuildTargetFnRecord *
        build_target_fns; // Linked list of [[cccc::build_target]] factory names
    ...
    char *current_suite; // Active suite path ("a/b/c") set by nested #pragma
                         // cccc suite begin
    ...
    bool comptime_include_all;  // --comptime-include-all: forward all #define
                                // macros to the comptime pass, and widen the
                                // demand-driven declaration index (#894) to
    ...
    Scope *macro_context_scope; // Parser scope produced by compiling the macro
                                // program
    StringArray
        comptime_dropped_globals; // #893: names of initialized globals that the
                                  // demand-driven declaration index (below)
                                  // declined to splice into the comptime
                                  // program (non-constant initializer, or not
                                  // in the primary file); used to give the
                                  // undefined-variable error at parse.c a
    ...
    Scope *macro_file_scope; // #894: the comptime program's own file scope, so
                             ...
                             // parser happens to be in when the miss occurs
    ...
    // System-header mode (--use-system-headers / --no-builtin-includes /
    // --sysroot)
    bool use_system_headers;  // Prefer SDK headers over CCCC polyfills for
                              // non-owned std headers
    bool no_builtin_includes; // Do not fall back to ./include for non-owned std
                              // headers
    const char *builtin_include_dir; // Path of CCCC's own header dir; excluded
                                     // from SDK-first search

    // Tokenization state
    File  *current_file; // Input file
    File **input_files;  // A list of all input files
    File *primary_file; // Top-level source file (set in cc_preprocess; used for
                        // auto-emit capture)
    ...
    int embedded_file_no; // Next embedded/in-memory file counter (negative file
                          // numbers); was a function-local static in
                          // tokenize_string (#181)
    ...
    // See cc_file_is_cccc_only() / mark_cccc_only_file() /
    // record_include_edge() in preprocess.c and run_native_backend's
    // re-emission filter (main.c).
...
struct VirtualMachine {
    ...
    long long stack_canary; // Stack canary value (random if
                            // CCCC_RANDOM_CANARIES set, else fixed)
    ...
    // internal so the public CCCC struct does not expose host pthread ABI
    // types.
    void             *gil_mutex;
    int               gil_initialized;
    ...
    int    lock_graph_size;
    int    lock_graph_cap;
    // Race detection shadow map: address -> last thread that wrote without a
    // lock. Value is ThreadRecord* for worker threads or (void*)1 for main
    // thread.
    HashMap race_shadow;
    // Atomic shadow map: address -> thread id that last accessed it atomically.
    // Used by check_race_access to detect non-atomic access to
    // atomically-tagged addresses (mixed atomic/non-atomic access). Tags set by
    // ALDR/ASTR/AXCHG/ACAS.
...
*/
void cc_set_asm_callback(VirtualMachine *vm, AsmCallback callback,
                         void *user_data);
...
*/
void cc_register_cfunc(VirtualMachine *vm, const char *name, void *func_ptr,
...
/*!
 ...
 format).
*/
void cc_register_variadic_cfunc(VirtualMachine *vm, const char *name,
                                void *func_ptr, int num_fixed_args,
                                int returns_double);
...
typedef enum {
    ...
    REPL_UNIT_DECL,  // one or more declarations were added to
                     // vm->compiler.globals
...
/*!
 ...
 @details Called from cc_finalize_macro_gvar_inits while in_macro_expansion is
 true. Must only be called after compile_all_macros has run (i.e. from within
             cc_expand_macros or cc_finalize_macro_gvar_inits).
 @param vm The CCCC instance.
 @param node The node to expand; must be non-NULL.
 @return The expanded (replacement) node, or the original node if not
 ND_MACRO_CALL.
*/
Node *cc_eager_expand_macro_call(VirtualMachine *vm, Node *node);

/*!
 @brief Finalize global variable initializers that were deferred due to macro
 calls.
 @details During cc_parse, gvar initializers containing ND_MACRO_CALL are
 deferred (their has_pending_macro_init flag is set and the Initializer tree
 stored in constexpr_init). This function, called from cc_expand_macros after
 macros are compiled and expanded, processes those deferred initializers: it
 expands the macro calls via transform_node and then serializes the result to
 .data.
 @param vm The CCCC instance.
 @param prog Linked list of top-level Obj returned by cc_parse.
*/
...
typedef struct {
    const char **inc_paths;
    int          inc_paths_count;
    const char **sys_inc_paths;
    int          sys_inc_paths_count;
...
typedef struct {
    ...
    const CcNativeCompileArgs
                *defaults;   // CLI -I/-D/-U/--std forwarded to each target
    ...
    const char *profile; // --build-profile=NAME:
                         // debug|release|relwithdebinfo|minsizerel
    const char *cross_triple; // --build-triple=TRIPLE: clang-style cross target
                              // triple (#547)
    const char
