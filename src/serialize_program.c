...
// preprocess.c PP_INCLUDE URL branch registered in emit_include_paths, keyed
// on this exact line text -- the two replay channels do NOT share the same
// pointer (cc_record_emit_source, src/macros.c, arena-copies its argument
// for the -c=generated channel), but the lookup is still sound because
// hashmap_get() compares key *bytes*, not identity. Returns true (and has
// already printed) when `line` was a URL include; false means the caller
// should print it itself. `--emit-cccc` is exempted like every other
// replay-time rewrite in this file -- dialect-fidelity output expects a
// cccc-aware reader that understands URL includes directly.
static bool emit_url_include_rewrite(FILE *f, VirtualMachine *vm,
    ...
    char *resolved = hashmap_get(&vm->compiler.emit_include_paths, line);
    if (!resolved)
        // #1324: not necessarily a failed fetch -- an `#include @emit
        // "https://..."` (preprocess.c's INCLUDE_ROUTE_EMIT branch) pushes
        // its line into the replay stream and returns before the URL fetch
        // branch (and its emit_include_paths registration) ever runs, so
        // this can also mean "never fetched at all".
        error("cccc: URL #include was never resolved to a cache entry "
              "(fetch failed, or the directive bypassed the normal fetch "
              "path, e.g. #include @emit): %s",
              line);
    fprintf(f, "#include \"%s\"\n", resolved);
    return true;
}
...
void cc_serialize_program(FILE *f, VirtualMachine *vm, Obj *prog,
    ...
    // is replayed by its includer verbatim (never auto-captured, so #1313's
    // emit_include_paths rewrite never sees it) and resolves only because
    // fetch_url_to_cache() (src/url_fetch.c) also mirrors the fetch under a
    // URL-shaped path in the cache directory. -c=native forwards that
    // directory to the host cc itself (`-idirafter`, src/main.c); a -m/
    // -c=generated dump is compiled by the caller, who has no other way to
    // learn it needs this flag -- so name it here.
    if (vm->compiler.url_mirror_used && vm->compiler.url_cache_dir)
        fprintf(
            f,
            "/* cccc: compile with -idirafter %s (URL #include mirror) */\n\n",
            vm->compiler.url_cache_dir);
