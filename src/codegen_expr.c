...
static void gen_vector_expr(VirtualMachine *vm, Node *node, int dest_reg) {
    switch (node->kind) {
        ...
        case ND_MEMBER: {
            ...
            // CHKD, same local-frame exemption as emit_load_ex's
            // dangling_check.
        ...
        case ND_ASSIGN: {
            ...
            // VLDR case above.
            if ((vm->flags & CCCC_BOUNDS_CHECKS) &&
                !addr_is_local_frame(vm, node->lhs))
                emit_rri(vm, CHKD, r_addr, 0, (long long)node->ty->size);
        ...
        case ND_COND: {
            if (!is_vector(node->cond->ty)) {
                ...
                int r_cond =
                    (dest_reg == REG_ZERO) ? alloc_temp_reg() : dest_reg;
            ...
            emit_rrrs(vm, vector_selop_for(node->ty->base->size), r_dest,
                      r_cond, r_then, node->ty->vec_len);
        ...
        case ND_NEG: {
            ...
            int           r_src = alloc_temp_reg();
            gen_expr(vm, node->lhs, r_src);
            emit_rrs(vm, vector_negop_for(fam), dest_reg, r_src,
                     node->ty->vec_len);
            free_temp_reg(r_src);
            return;
        }
        case ND_CAST: {
            ...
            emit_rrs(vm, vector_splatop_for(fam), dest_reg, r_scalar,
                     node->ty->vec_len);
        ...
        case ND_CONVERTVECTOR: {
            ...
            int           op;
            if (src_fam == VLANE_F32 && dst_fam == VLANE_I32)
                op = VCVT_I32_F32;
            else if (src_fam == VLANE_I32 && dst_fam == VLANE_F32)
                op = VCVT_F32_I32;
            else if (src_fam == VLANE_F64 && dst_fam == VLANE_I64)
                op = VCVT_I64_F64;
            else if (src_fam == VLANE_I64 && dst_fam == VLANE_F64)
                op = VCVT_F64_I64;
            else {
    ...
    switch (node->kind) {
        ...
        case ND_NUM:
            if (is_flonum(node->ty)) {
                ...
                long long lit_size = (node->ty->kind == TY_FLOAT)
                                         ? sizeof(float)
                                         : sizeof(double);
                ...
                emit_rr(vm, node->ty->kind == TY_FLOAT ? FLDR_F32 : FLDR,
                        dest_reg, temp);
                free_temp_reg(temp);
                ...
                // so dest_reg receives the literal's address, not a loaded
                // value.
                if (!node->dec_digits)
                    error_tok(
                        vm, node->tok,
                ...
                if (!cccc_dec_encode_literal(node->dec_digits,
                                             dec_width_code(node->ty), bits))
                    error_tok(vm, node->tok,
                              "_Decimal literals require a build with "
                              "CCCC_HAS_DECIMAL=1 "
                              "(run tools/fetch_intel_bid.sh, then `make "
                              "CCCC_HAS_DECIMAL=1`)");
                ...
                if (!node->wide_digits)
                    error_tok(vm, node->tok,
                              "internal error: wide _BitInt literal missing "
                              "digit text");
        ...
        case ND_VAR:
            if (node->var->is_function) {
                ...
                // writes go through a raw pointer (WIDE_* opcode or CALLF
                // helper), never STR_LOCAL — meaning MARKI/MARKW are never
                // emitted for those slots. Excluding wide _BitInt from the
                // read-side checks too keeps the two sides symmetric and avoids
                // the false "uninitialized variable read" trap under
                // CCCC_UNINIT_DETECTION (-2/-3) that ticket #457 reported.
                // (Consistent with structs/unions/arrays, which are already
                // exempt here.)
                //
                if (node->var->is_local && !node->var->is_param &&
                    ...
                    !is_decimal(
                        node->var->ty)) { // #402: address-based, same exemption
                ...
                if (is_simple_local_scalar(vm, node)) {
                    ...
                    // REG_A0-A7. Use a temp register to avoid clobbering int
                    // regs.
                    int r_addr = alloc_temp_reg();
                    gen_addr(vm, node, r_addr);
                    emit_load_ex(vm, node->ty, dest_reg, r_addr,
                                 !addr_is_local_frame(vm, node));
                    free_temp_reg(r_addr);
                } else {
                    gen_addr(vm, node, dest_reg);
                    // For scalars, load the value (wide _BitInt/_Decimal stay
                    // as address)
        ...
        case ND_DEREF:
            ...
            gen_addr(vm, node, dest_reg);
            // TY_FUNC: dereferencing a function pointer is a no-op in C — *f
            // and f are interchangeable when f has pointer-to-function type. Do
            // not emit a data load; the register already holds the callable
            // address.
            //
        ...
        case ND_ADDR:
            // #973: for a VLA operand the array's base address IS its value,
            // not the address of the thing holding it. A VLA *variable*'s frame
            // slot holds the alloca'd data pointer -- gen_addr's ND_VAR case
            // would hand back the slot's own address (wrong: `(*p)[i]` through
            ...
            // gen_addr agree there and this early-out is a no-op for it.
            // Routing through gen_expr (rather than special-casing gen_addr's
            // ND_VAR) picks up all three of its slot-address branches -- normal
            // local, block capture, outer-function static chain -- for free.
        ...
        case ND_NEG:
            ...
            if (is_decimal(node->ty)) {
                ...
                long long dst_offset =
                    node->ret_buffer ? (long long)node->ret_buffer->offset
        ...
        case ND_NOT:
            if (node->lhs &&
                (is_wide_bitint(node->lhs->ty) || is_decimal(node->lhs->ty))) {
        ...
        case ND_LE: {
            if (is_complex(node->lhs->ty) || is_complex(node->rhs->ty)) {
                ...
                emit_frrr(vm, fop_for_type(node->lhs->ty->base, FEQ3), r_real,
                          ar, br);
                emit_frrr(vm, fop_for_type(node->lhs->ty->base, FEQ3), r_imag,
                          ai, bi);
            ...
            // Check if RHS contains a function call - if so, we need to save
            // LHS because function calls clobber caller-saved temp registers
            bool rhs_has_call = contains_funcall(node->rhs);

            // Evaluate LHS first, then allocate r_rhs. gen_expr frees all temps
            // before returning, so the pool is empty after LHS completes. This
            // keeps peak usage at O(1) per level instead of O(chain-depth),
            // fixing register exhaustion on long left-associative chains
            // (ticket #295).
            if (is_flonum(node->lhs->ty)) {
                ...
                // Re-mark dest_reg as used so r_rhs allocation doesn't clobber
                // it.
                mark_temp_reg_used(dest_reg);

                // Operands fed to the final float op, and the temp to release
                // after. Fast path: LHS stays in dest_reg (float), RHS goes to
                // a fresh float temp. Spill path (#587): under register
                // pressure, save the LHS float bits to the stack and reuse
                // dest_reg for the RHS so no temp stays live across the RHS
                // recursion. PSH3/POP3 operate on integer regs, so the bits go
                // through FR2R/R2FR (same idiom as the rhs_has_call branch
                // below).
                ...
                if (temp_regs_free() <= TEMP_REG_SPILL_THRESHOLD) {
                    ...
                    free_temp_reg(
                        r_tmp);         // nothing held across the RHS recursion
                    gen_expr(vm, node->rhs,
                             dest_reg); // RHS reuses dest; pool free
                    ...
                    int r_rhs = alloc_temp_reg();
                    if (rhs_has_call) {
                        // For floats: convert to int, push to stack, evaluate
                        // RHS, pop, convert back. dest_reg is FREG_*, so we use
                        // FR2R to move bits to an int temp.
                ...
                emit_frrr(vm, fop_for_type(node->lhs->ty, fop), dest_reg,
                          r_lhs_op, r_rhs_op);
                free_temp_reg(r_free);
            } else if (is_decimal(node->lhs->ty) || is_decimal(node->ty)) {
                // _Decimal32/64/128 (#402): address-based like wide _BitInt,
                // but dispatches to the dedicated DADD/DSUB/DMUL/DDIV/DCMP
                // opcodes (fixed A-register convention, fully opaque to the
                // optimizer -- see the OPS_X comment in cccc.h) instead of a
                // CALLF helper. usual_arith_conv already rejected mixing
                // decimal with a binary float or _BitInt operand, so
                // node->lhs->ty == node->rhs->ty (modulo decimal-vs-decimal
                // rank) here.
                ...
                int work_reg =
                    (dest_reg == REG_ZERO) ? alloc_temp_reg() : dest_reg;
                gen_expr(vm, node->lhs,
                         work_reg); // decimal operand -> its address
                ...
                // CALLF resets the whole temp-reg bitmap, marking work_reg's
                // bit free again even though it still holds the live RHS value.
                ...
                if (is_cmp) {
                    ...
                    switch (node->kind) {
                        ...
                        case ND_LT:
                            ...
                            break;
                        case ND_LE: {
                            // LE is EQ(0) or LT(1) -- i.e. code <= 1, which
                            // also correctly excludes GT(2) and UNORDERED(3).
                            // code is always in [0,3], so plain signed SLE3 is
                            // exact here.
                ...
                free_temp_reg(r_rhs);
                free_temp_reg(r_lhs);
            } else if (is_wide_bitint(node->lhs->ty) ||
                ...
                // For arithmetic/bitwise ops: allocate a stack temp for the
                // result.
                ...
                // AND/OR/XOR/comparisons stay on the CALLF/emit_wide_helper
                // path.
                int wide_opcode = 0;
                switch (node->kind) {
                    case ND_ADD:
                        wide_opcode = WIDE_ADD;
                        break;
                    case ND_SUB:
                        wide_opcode = WIDE_SUB;
                        break;
                    case ND_MUL:
                        wide_opcode = WIDE_MUL;
                        break;
                    case ND_DIV:
                        wide_opcode = WIDE_DIV;
                        break;
                    case ND_MOD:
                        wide_opcode = WIDE_MOD;
                        break;
                    case ND_SHL:
                        wide_opcode = WIDE_SHL;
                        break;
                    case ND_SHR:
                        wide_opcode = is_signed ? WIDE_SHR : WIDE_USHR;
                        break;
                    default:
                        break;
                }
                ...
                switch (node->kind) {
                    case ND_BITAND:
                        fn = "__cccc_bitint_and";
                        break;
                    case ND_BITOR:
                        fn = "__cccc_bitint_or";
                        break;
                    case ND_BITXOR:
                        fn = "__cccc_bitint_xor";
                        break;
                    case ND_EQ:
                    case ND_NE:
                    case ND_LT:
                    case ND_LE:
                        fn = "__cccc_bitint_cmp";
                        break;
                    case ND_ADD:
                    case ND_SUB:
                    case ND_MUL:
                ...
                if (node->kind == ND_SHL || node->kind == ND_SHR) {
                    // Shift: dst(A0), src_addr(A1), shift_amount(A2),
                    // words(A3), width(A4)
                    emit_lea3(vm, REG_A0, dst_offset);
                    emit_mov3(vm, REG_A1, r_lhs);
                    ...
                    switch (node->kind) {
                        case ND_EQ:
                            emit_rrr(vm, SEQ3, dest_reg, REG_A0, tmp);
                            break;
                        case ND_NE:
                            emit_rrr(vm, SNE3, dest_reg, REG_A0, tmp);
                            break;
                        case ND_LT:
                            emit_rrr(vm, SLT3, dest_reg, REG_A0, tmp);
                            break;
                        case ND_LE:
                            emit_rrr(vm, SLE3, dest_reg, REG_A0, tmp);
                            break;
                        default:
                            break;
                    }
                ...
                // Operands fed to the final op, and the temp to release
                // afterwards. Fast path: LHS stays in dest_reg, RHS goes to a
                // fresh temp. Spill path (#587): under register pressure, save
                // LHS to the stack and reuse dest_reg for the RHS so no temp
                // stays live across the RHS recursion — bounds peak register
                // use on deeply nested trees. The spill path also subsumes the
                // rhs_has_call case (PSH3 protects the LHS across any call
                // inside the RHS).
                int r_lhs_op, r_rhs_op, r_free;
                if (temp_regs_free() <= TEMP_REG_SPILL_THRESHOLD) {
                ...
                switch (node->kind) {
                    ...
                    case ND_MUL:
                        ...
                        break;
                    case ND_DIV:
                        op = node->ty->is_unsigned
                                 ? UDIV3
                                 : (checked_arith ? DIVC : DIV3);
                        break;
                    case ND_MOD:
                ...
                // For pointer add/sub, emit bounds check (CHKB/CHKBN) before
                // the add and provenance check (CHKPA) after.
                bool is_ptr_arith =
                ...
                // see its own comment.
                bool is_ptr_diff = node->kind == ND_SUB && node->lhs->ty &&
                                   node->lhs->ty->base && node->ty &&
                                   !node->ty->base;
                if (is_ptr_arith && !is_ptr_diff &&
                ...
                // registers so SLT3/SLE3 already give the correct signed
                // result.
                bool is_u64_cmp =
                    (node->kind == ND_LT || node->kind == ND_LE) &&
                ...
                if (is_ptr_arith && (vm->flags & (CCCC_INVALID_ARITH |
        ...
        case ND_ASSIGN: {
            ...
            if (node->ty &&
                ...
                // If the LHS address expression contains a call, gen_addr below
                // will clobber every caller-saved temp at runtime, destroying
                ...
                // #939: a store through a [[cccc::ntarray]] + count(n)
                // pointer's widened terminator slot must stay all-zero-bytes
                // for the struct/union/wide-_BitInt/_Decimal pointees that
                // lower to this memcpy path -- CHKNT (the scalar guard below)
                // can't check these, since their value never passes through a
                // single register. CHKNTZ scans r_src (the RHS address, already
                ...
                // -- so no CHKR/CHKNT-widened-hi vs. MCPY-size mismatch is
                // possible.
                if ((vm->flags & CCCC_CHECKED_BOUNDS) &&
                    ...
                    emit_chkntz(vm, r_dest, r_hi, r_src,
                                node->lhs->checked_access_size);
                    free_temp_reg(r_hi);
                }
                ...
                // local-frame address, same rule as
                // emit_load_ex/emit_store_ex's dangling_check.
                if (vm->flags & CCCC_BOUNDS_CHECKS) {
                    mark_temp_reg_used(r_dest);
                    mark_temp_reg_used(r_src);
                    if (!addr_is_local_frame(vm, node->lhs))
                        emit_rri(vm, CHKD, r_dest, 0,
                                 (long long)node->ty->size);
            ...
            // (though dest_reg is output) But critically, if LHS is a bitfield,
            // we definitely need temp regs for RMW
            if (dest_reg == REG_ZERO || (node->lhs->kind == ND_MEMBER &&
                                         node->lhs->member->is_bitfield)) {
            ...
            // called. We need to re-mark r_val as in-use before allocating
            // r_addr!
            mark_temp_reg_used(r_val);
            ...
            // the already-evaluated RHS in r_val. (reset_temp_regs() inside the
            ...
            if (lhs_has_call) {
                // The address is now in r_addr; reload r_val into a fresh temp
                // that is guaranteed distinct from r_addr. (The old r_val
                // register was clobbered by the call and its allocator bit
            ...
            // widened terminator slot must stay null -- CHKR (emitted by
            ...
            if ((vm->flags & CCCC_CHECKED_BOUNDS) &&
                ...
                // gate excludes it, src/parse.c's
                // checked_nt_pointee_supported())
                // -- an 8-byte FR2R read would silently ignore its other 8
                // bytes.
                int  r_ntval    = r_val;
                bool free_ntval = false;
                if (is_flonum(node->lhs->ty)) {
                    r_ntval = alloc_temp_reg();
                    emit_rr(vm, fop_for_type(node->lhs->ty, FR2R), r_ntval,
                            r_val);
                    free_ntval = true;
                }
                ...
                // always fires first.
                int r_hi = gen_checked_nt_hi(vm, node->lhs);
                emit_chknt(vm, r_addr, r_hi, r_ntval,
                           node->lhs->checked_access_size);
                free_temp_reg(r_hi);
                if (free_ntval)
            ...
            vm->compiler.in_union_member_access =
                is_union_member_access(node->lhs);
            ...
            if (node->lhs->kind == ND_MEMBER &&
                ...
                emit_load_ex(vm, mem->ty, r_container, r_addr,
                ...
                emit_rrr(vm, AND3, r_new, r_new,
                         r_mask);           // Truncate val to width
                ...
                // Store back
                emit_store_ex(vm, mem->ty, r_container, r_addr,
                              !lhs_local_frame); // Use member type (container)
                ...
                // Standard store
                emit_store_ex(vm, node->ty, r_val, r_addr,
                              !addr_is_local_frame(vm, node->lhs));
            }
            ...
            // but it still writes the same frame slot a later ND_VAR read of
            // the VLA loads through, so it needs the identical MARKI or that
            // read's CHKI trips a false UNINITIALIZED VARIABLE READ (#980).
            // Unlike ND_VAR, ND_VLA_PTR only ever names a non-param local VLA,
            ...
            if (node->lhs->kind == ND_VLA_PTR) {
                ...
                if (vm->flags & CCCC_UNINIT_DETECTION)
                    emit_marki(vm, node->lhs->var->offset);
                       ...
                       !node->lhs->var->is_param && node->lhs->var->ty &&
        ...
        case ND_COND: {
            ...
            // Reuse dest_reg for the condition scratch to avoid O(depth)
            // register accumulation on deeply nested && / || conditions (#587
            // gap). Guard: dest_reg may be REG_ZERO (discarded expression
            // statement) — the zero register silently discards writes, so the
            // condition is always read back as 0, making the branch always take
            // the else arm. Allocate a real temp when dest_reg == REG_ZERO and
            // free it before generating the branches.
        ...
        case ND_CAST:
            ...
            if (is_wide_bitint(node->lhs->ty) || is_wide_bitint(node->ty)) {
                ...
                if (is_wide_bitint(dst) && !is_wide_bitint(src)) {
                    ...
                    long long dst_offset =
                        alloc_wide_bitint_temp(vm, dst->size / 8);
                    emit_lea3(vm, REG_A0, dst_offset);
                    if (is_flonum(src)) {
                        // float/double → wide: pass double bits as raw int64
                        // Float regs always store as double internally, so FR2R
                        // gives us the IEEE-754 double representation
                        // regardless of f32/f64.
                        int r_tmp = alloc_temp_reg();
                        gen_expr(vm, node->lhs,
                                 r_tmp); // puts float/double in float reg
                        emit_rr(vm, FR2R, REG_A1,
                                r_tmp);  // double bits → int reg
                        free_temp_reg(r_tmp);
                        emit_lea3(vm, REG_A0, dst_offset);
                    ...
                    if (is_flonum(dst)) {
                        ...
                        // result is raw double bits in REG_A0; reinterpret as
                        // float reg
                        emit_rr(vm, R2FR, dest_reg, REG_A0);
                        if (dst->kind == TY_FLOAT)
                            emit_fround_f32(vm, dest_reg, dest_reg);
                    } else if (dst->kind == TY_BOOL) {
                        // (_Bool) must reflect the whole value, not just the
                        // low word: a wide value with only high bits set is
                        // still true.
                        emit_li3(vm, REG_A1,
                                 src->size / 8); // words (A0 = address)
                        ...
                        if (dst->kind == TY_BOOL)
                            ...
                            emit_rr(vm, dst->is_unsigned ? ZX1 : SX1, dest_reg,
                                    dest_reg);
                    ...
                    long long dst_offset =
                        alloc_wide_bitint_temp(vm, words_dst);
                    int r_src = alloc_temp_reg();
                    gen_expr(vm, node->lhs, r_src);
                    emit_lea3(vm, REG_A0, dst_offset);
                    emit_mov3(vm, REG_A1, r_src);
            ...
            if (is_decimal(node->lhs->ty) || is_decimal(node->ty)) {
                ...
                if (is_decimal(dst) && !is_decimal(src)) {
                    ...
                    if (is_flonum(src)) {
                        ...
                        // 32-bit float pattern (not a misread of the full
                        // 64-bit double pattern FReg would otherwise hold it
                        // as).
                        int r_tmp = alloc_temp_reg();
                        ...
                        emit_lea3(vm, REG_A0,
                                  dst_offset); // REG_A1 may share REG_A0's
                                               // slot; reload
                        emit_li3(vm, REG_A2, dec_width_code(dst));
                    ...
                    // statement, e.g. `(void)(_Decimal64)d;`) -- REG_ZERO
                    // always reads back 0, so staging the source address
                    // through it would hand DTOI/DTOBITS/DCMP a null pointer.
                    // Use a fresh temp in that case instead.
                    int r_src =
                        (dest_reg == REG_ZERO) ? alloc_temp_reg() : dest_reg;
        ...
        case ND_FUNCALL: {
            ...
            if (node->lhs->kind == ND_VAR &&
                ...
                // Evaluate size argument into REG_A0 (ALCA/ALCV read from
                // REG_A0)
                reset_temp_regs();
                gen_expr(vm, node->args, REG_A0);
                emit(vm,
                     node->is_vla_alloca_call
                         ? ALCV
                         : ALCA); // Size in REG_A0, returns pointer in REG_A0
                if (dest_reg != REG_A0) {
                    emit_mov3(vm, dest_reg, REG_A0);
                }
                return;
            ...
            if (is_extern_func_name(node->lhs, "signal") ||
                ...
                if (!node->args || !node->args->next) {
                    error_tok(vm, node->tok,
                              "signal requires sig and handler arguments");
                }
                reset_temp_regs();
            ...
            if (is_extern_func_name(node->lhs, "raise") ||
                ...
                gen_expr(vm, node->args, REG_A0); // sig
                emit(vm, VRAISE);
                /* VRAISE may jump into a VM handler; when the handler returns
                   it lands here. Normalise REG_A0 = 0 so raise() always returns
                   0. */
            ...
            if (is_extern_func_name(node->lhs, "dlopen")) {
                ...
                if (!node->args || !node->args->next)
                    error_tok(vm, node->tok,
                              "dlopen requires path and mode arguments");
                gen_expr(vm, node->args, REG_A0);
            ...
            if (is_extern_func_name(node->lhs, "dlsym")) {
                ...
                if (!node->args || !node->args->next)
                    error_tok(vm, node->tok,
                              "dlsym requires handle and symbol arguments");
                gen_expr(vm, node->args, REG_A0);
                gen_expr(vm, node->args->next, REG_A1);
                emit(vm, DLSYM);
                if (dest_reg != REG_A0)
            ...
            if (is_extern_func_name(node->lhs, "dlclose")) {
                ...
                if (!node->args)
                    error_tok(vm, node->tok,
                              "dlclose requires a handle argument");
                gen_expr(vm, node->args, REG_A0);
                emit(vm, DLCLOSE);
                if (dest_reg != REG_A0)
                    emit_mov3(vm, dest_reg, REG_A0);
            ...
            // When CCCC_VM_HEAP is set, route malloc/free/calloc/realloc
            // through VM heap opcodes instead of system allocators via FFI.
            if (vm->flags & CCCC_VM_HEAP) {
                if (is_extern_func_name(node->lhs, "malloc")) {
                    if (!node->args)
                        error_tok(vm, node->tok,
                                  "malloc requires a size argument");
                    reset_temp_regs();
                    gen_expr(vm, node->args, REG_A0);
                    emit(vm, MALC);
                    if (dest_reg != REG_A0)
                ...
                // free_sized/free_aligned_sized (C23) route through the same
                // MFRE opcode as free: MFRE derives the real size from the
                // AllocHeader and already falls back to the host free() for
                // non-VM-heap pointers (ops.c op_MFRE_fn), so the
                // size/alignment arguments are only evaluated for side effects
                // and otherwise discarded. Without this, a VM-heap malloc()
                // paired with free_sized() would hand a VM-heap pointer
                // straight to the host's free_sized() via FFI and abort (#665
                // fallout: VM heap is on by default now).
                if (is_extern_func_name(node->lhs, "free_sized")) {
                    if (!node->args || !node->args->next)
                        error_tok(vm, node->tok,
                                  "free_sized requires ptr and size arguments");
                    reset_temp_regs();
                    gen_expr(vm, node->args, REG_A0);
                    gen_expr(vm, node->args->next, REG_A1);
                    emit(vm, MFRE);
                ...
                if (is_extern_func_name(node->lhs, "free_aligned_sized")) {
                    if (!node->args || !node->args->next ||
                        !node->args->next->next)
                        error_tok(vm, node->tok,
                                  "free_aligned_sized requires ptr, alignment, "
                                  "and size arguments");
                    reset_temp_regs();
                    gen_expr(vm, node->args, REG_A0);
                    gen_expr(vm, node->args->next, REG_A1);
                ...
                if (is_extern_func_name(node->lhs, "calloc")) {
                    if (!node->args || !node->args->next)
                        error_tok(vm, node->tok,
                                  "calloc requires nmemb and size arguments");
                    reset_temp_regs();
                    gen_expr(vm, node->args, REG_A0);
                    gen_expr(vm, node->args->next, REG_A1);
                    emit(vm, CALC);
                ...
                if (is_extern_func_name(node->lhs, "realloc")) {
                    if (!node->args || !node->args->next)
                        error_tok(vm, node->tok,
                                  "realloc requires ptr and size arguments");
                    reset_temp_regs();
                    gen_expr(vm, node->args, REG_A0);
                    gen_expr(vm, node->args->next, REG_A1);
                    emit(vm, REALC);
                ...
                if (is_extern_func_name(node->lhs, "reallocarray")) {
                    if (!node->args || !node->args->next ||
                        !node->args->next->next)
                        error_tok(vm, node->tok,
                                  "reallocarray requires ptr, nmemb, and size "
                                  "arguments");
                    reset_temp_regs();
                    gen_expr(vm, node->args, REG_A0);
                    gen_expr(vm, node->args->next, REG_A1);
                ...
                if (is_extern_func_name(node->lhs, "aligned_alloc")) {
                    if (!node->args || !node->args->next)
                        error_tok(vm, node->tok,
                                  "aligned_alloc requires alignment and size "
                                  "arguments");
                    reset_temp_regs();
                ...
                if (is_extern_func_name(node->lhs, "posix_memalign")) {
                    if (!node->args || !node->args->next ||
                        !node->args->next->next)
                        error_tok(vm, node->tok,
                                  "posix_memalign requires memptr, alignment, "
                                  "and size arguments");
                    reset_temp_regs();
            ...
            if (ffi_idx >= 0) {
                ...
                // FFI would be mis-marshalled as a 64-bit int/double slot
                // rather than rejected. Remains out of scope here; see #830.
                if (is_decimal(node->ty))
                    error_tok(
                        vm, node->tok,
                ...
                // #829: a decimal argument in the *variadic tail* of an FFI
                // call (our own cccc_printf/cccc_fprintf/... engine, which
                // expects a pointer for %Hf/%Df/%DDf) is passed by pointer --
                // see gen_decimal_arg_ptr. A FIXED decimal FFI parameter has no
                // such convention to lean on (no libffi decimal ffi_type
                // exists), so it stays rejected; that by-value FFI case is
                // #830's scope.
                bool ffi_is_variadic_call =
                    node->func_ty && node->func_ty->is_variadic;
                ...
                for (Node *arg = node->args; arg; arg = arg->next) {
                    bool ffi_arg_is_vararg =
                        ffi_is_variadic_call && nargs >= ffi_fixed_param_count;
                    if (is_flonum(arg->ty)) {
                        if (nargs >= 64)
                            error_tok(vm, arg->tok,
                ...
                // Check which register arguments contain function calls (to
                // handle clobbering). Nested FFI calls will clobber REG_A0-A7.
                bool *arg_has_call =
                    calloc(nargs > 0 ? nargs : 1, sizeof(bool));
                if (!arg_has_call)
                    error("out of memory");
                ...
                // REG_A0-A7. We must save any previous args before evaluating
                // such an arg.
                int saved_reg_count = 0;
                for (int i = 0; i < nargs && i < 8; i++) {
                    Node *arg = arg_array[i];

                    // Before evaluating this arg, check if it contains a
                    // function call. If so, save all previously-evaluated arg
                    // registers.
                    ...
                    if (is_zero_size_aggregate(arg->ty)) {
                        ...
                        int fs = gen_flonum_arg_to_scratch(vm, arg);
                        emit_rr(vm, fop_for_type(arg->ty, FR2R), REG_A0 + i,
                                fs);
                        free_temp_reg(fs);
                    } else if (is_decimal(arg->ty)) {
                        // #829: only reachable here for a variadic-tail decimal
                    ...
                    // After evaluating this arg, if we saved previous regs,
                    // restore them now.
                ...
                if (callee->is_inline && callee->is_static && callee->body &&
                    ...
                    if (vm->compiler.opt_level >= 2 &&
                        ...
                        if ((void_ret || !(ret_ty->kind == TY_STRUCT ||
                            ...
                            // Clone entire function body with parameter
                            // substitution
                            Node *inlined_body = clone_subst(
                                vm, callee->body, callee->params, node->args);
                            ...
                            if (nlocals > 0) {
                                ...
                                for (Obj *v = callee->locals; v; v = v->next) {
                                    ...
                                    *nv        = *v;
                                    char *name = arena_format(
                                        vm, "%s_inline%d", v->name,
                            ...
                            char *exit_name = arena_format(
                                vm, ".Linline_exit_%d",
                                vm->compiler.unique_name_counter++);

                            // Set inlining context
                            vm->compiler.inline_exit_name = exit_name;
                            vm->compiler.inline_result_reg =
                                void_ret ? REG_ZERO : dest_reg;

                            // Generate inlined body
                            gen_stmt(vm, inlined_body);

                            // Define exit label (JMP targets from inlined
                            // returns)
            ...
            // Check if we're calling a nested function - need to pass static
            // link
            bool calling_nested =
            ...
            // #885: for an indirect call whose callee expression itself
            // contains a call (e.g. `((int(*)(int,int))dlsym(h,"f"))(a,b)`),
            // evaluating the callee AFTER staging args into REG_A0-A7 (the old
            // order, below) lets the nested call clobber those registers --
            // op_ENT3_fn only saves bp, not the register file, so a temp
            // register isn't safe across it either. Overflow args (8+) are
            // pushed straight to the stack, not through REG_A0-A7, so they
            // aren't at risk -- only the register-arg staging loop below is.
            // Direct calls (node->lhs is a known Obj*) are unaffected -- they
            // never evaluate node->lhs at all.
            bool is_indirect_call =
                !(node->lhs->kind == ND_VAR && node->lhs->var->is_function);
            bool hoist_callee =
                is_indirect_call && nargs > 0 && contains_funcall(node->lhs);
            ...
            if (num_stack_args > 0) {
                for (int j = nargs - 1; j >= 8; j--) {
                    ...
                    if (is_zero_size_aggregate(arg->ty)) {
                        ...
                        // Float arg: evaluate to a temp-numbered float scratch
                        // (not FREG_A0 -- see gen_flonum_arg_to_scratch, #712),
                        // move bits to int reg, push
                        ...
                        // Vector arg: copy value to scratch slot, push its
                        // address like a struct-by-value arg (#714). A variadic
                        // tail arg
                        ...
                        emit_psh3(vm, REG_T0);
                    } else if (is_decimal(arg->ty) && is_variadic_call &&
                               j >= fixed_param_count) {
                        // Decimal variadic tail arg (#829), stack-passed: same
                        // padded-scratch-copy rationale as
                        // gen_decimal_arg_ptr's comment below. A fixed decimal
                        // param at a stack position instead falls to the
                        // generic branch, matching #402's existing
                        // address-passthrough ABI.
                        gen_decimal_arg_ptr(vm, arg, REG_T0);
                        emit_psh3(vm, REG_T0);
                    } else {
            ...
            // #885: spill the callee now -- after the overflow-arg pushes
            // (which op_CALLN_fn reads via vm->sp[i-8], so nothing may
            // intervene between them and CALLN except a strictly-balanced
            // push/pop) but before the register-arg staging loop the callee's
            // own call would clobber.
            ...
            for (int i = 0; i < nargs && i < 8; i++) {
                ...
                // call. If so, save all previously-evaluated arg registers to
                // the stack.
                if (arg_has_call[i] && (int_arg_idx > 0 || float_arg_idx > 0)) {
                    // Push int regs in reverse order (so we pop in correct
                    // order later)
                ...
                if (is_zero_size_aggregate(arg->ty)) {
                    ...
                    if (is_vararg) {
                        // Variadic double: put in integer register (as bit
                        // pattern) ENT3 will spill REG_A* to stack; va_arg
                        // reads from stack
                        if (int_arg_idx < 8) {
                            // Generate double value into a temp-numbered float
                            // scratch (not FREG_A0 -- see
                            // gen_flonum_arg_to_scratch, #712), then move bits
                            // to int reg
                            int freg = gen_flonum_arg_to_scratch(vm, arg);
                            // Move double bits from freg to int reg
                            // (bit-pattern, not conversion)
                            emit_rr(vm, FR2R, REG_A0 + int_arg_idx, freg);
                            free_temp_reg(freg);
                            int_arg_idx++;
                        }
                    } else {
                        // Fixed param double: evaluate into a temp-numbered
                        // float scratch (#712), then move into place with FMOV3
                        // (writes only fregs[], can never clobber a live int
                        // arg register).
                        if (float_arg_idx < 8) {
                            int freg = gen_flonum_arg_to_scratch(vm, arg);
                    ...
                    // width-independent scratch copy -- see
                    // gen_decimal_arg_ptr's comment. A FIXED decimal param
                    // falls through to the generic branch below instead,
                    // unchanged from #402: gen_expr's address-based decimal
                    // representation already gives the right by-address ABI
                    // with no extra copy needed.
                    if (int_arg_idx < 8) {
                    ...
                    // address in the integer arg register. Works identically
                    // for a variadic tail arg (#721): the pointer occupies
                    // exactly one int arg slot, which ENT3 spills like any
                    // other, and <stdarg.h>'s va_arg dereferences it via
                    // __builtin_classify_type.
                    if (int_arg_idx < 8) {
                    ...
                    // Integer/pointer argument - always goes in integer
                    // register
                    if (int_arg_idx < 8) {
            ...
            if (node->lhs->kind == ND_VAR && node->lhs->var->is_function) {
                Obj *fn = node->lhs->var;
                if (is_tail) {
                    // Record callee; ND_RETURN emits CALLT + patch after
                    // cleanup. If inlining/builtins fired earlier and we never
                    // reach here, pending_tail_callee stays NULL and ND_RETURN
                    // falls back to LEV3.
                ...
                if (!skip_dead_calln) {
                    ...
                    emit_word(
                        vm,
                        ((InstrWord)(node->ty->kind == TY_FLOAT ? 0
                                     : is_flonum(node->ty)      ? 1
                                                                : 0)
                         << 16) |
                            ((InstrWord)(node->ty->kind == TY_FLOAT ? 1 : 0)
                             << 17) |
            ...
            // chained calls like f(g(), h()) automatically get different
            // buffers for g() and h()'s results. No caller-side copy is needed.
            ...
            if (is_flonum(node->ty)) {
                ...
                // unlike int/float write opcodes, VLDR has no built-in "writes
                // to REG_ZERO are discarded" guard (op_VLDR_fn, ops.c), so an
                // unconditional load here would write into vregs[0] instead of
                // being a no-op -- harmless in practice (index 0 is never
                // handed out by alloc_temp_reg) but skip it anyway rather than
                // relying on that.
                if (dest_reg != REG_ZERO) {
                    if (vm->flags & CCCC_POINTER_CHECKS)
                        emit_rr(vm, CHKP3, REG_A0, 0);
        ...
        // Emitted as the first operand of the ND_COMMA produced by
        // lvar_initializer for partial aggregate initialisers (e.g. `T arr[N] =
        // {0}`), and (#982) by var_definition's VLA initializer path for a
        // partial VLA brace initializer.  The MSET opcode mirrors MCPY:
        // dest=REG_A0, count=REG_A2.
        //
        // __block variables: their stack slot (bp+offset) holds an 8-byte heap
        // pointer written by the function prologue; a blind MSET of
        // var->ty->size bytes at the slot would corrupt the pointer.  Instead,
        // dereference the slot to obtain the heap cell address and zero through
        // that — mirroring gen_addr's normal-local block path.  This ensures
        // that `__block T arr[N] = {partial}` correctly zeroes the unspecified
        // elements in the heap cell.
        //
        ...
        case ND_LOGAND: {
            ...
            // Reuse dest_reg as the condition scratch to avoid O(depth)
            // register accumulation for deeply nested && chains (#587 gap).
            // Guard: dest_reg may be REG_ZERO (discarded expression statement)
            // — the zero register silently discards writes, so both conditions
            // read back as 0 and jz is always taken, preventing the rhs and any
            // side-effects from running. Allocate a real temp when dest_reg ==
            // REG_ZERO and free it after the last jz (before generating the
            // final 0/1 into dest_reg).
            int r_cond = (dest_reg == REG_ZERO) ? alloc_temp_reg() : dest_reg;
        ...
        case ND_LOGOR: {
            ...
            // Same dest_reg reuse + REG_ZERO guard as ND_LOGAND above (see
            // comment there).
            int r_cond = (dest_reg == REG_ZERO) ? alloc_temp_reg() : dest_reg;
            mark_temp_reg_used(r_cond);
            gen_cond_expr(vm, node->lhs, r_cond);
        ...
        case ND_STMT_EXPR: {
            ...
            // Generate all statements, result of last expression goes to
            // dest_reg
        ...
        case ND_RETURN_ADDR:
            // __builtin_return_address(n) - returns return address n frames up
            // as void* Emits RETADDR opcode which walks the saved-bp chain at
            // runtime and bounds-checks each step.  Returns NULL past the
            // outermost frame.
            emit_ri(vm, RETADDR, dest_reg, node->val);
            return;

        case ND_DYNOBJ_SIZE: {
            // __builtin_dynamic_object_size(ptr, type) — runtime heap size
            // lookup. Evaluate the pointer into a temp register, then emit
            // DYNOBJSZ which reads AllocHeader.requested_size for VM heap base
            // pointers.
        ...
        case ND_BITOP: {
            ...
            switch (bitop_op) {
                ...
                default:
                    error_tok(vm, node->tok,
                              "codegen: unknown ND_BITOP selector %d",
                              bitop_op);
            }
        ...
        case ND_ASTORE: {
            // Atomic-tagged store via ASTR: *(T*)addr = val, tags
        ...
        case ND_EXCH: {
            ...
            if ((sz != 1 && sz != 2 && sz != 4 && sz != 8) ||
                is_flonum(base_ty))
                error_tok(vm, node->tok,
                          "atomic_exchange: unsupported type (must be "
        ...
        case ND_CAS: {
            ...
            if ((sz != 1 && sz != 2 && sz != 4 && sz != 8) ||
                is_flonum(base_ty))
                error_tok(vm, node->tok,
                          "compare_and_swap: unsupported type (must be "
                          "1/2/4/8-byte integer or pointer)");
            long long width_enc =
                ((long long)sz << 1) | (base_ty->is_unsigned ? 1 : 0);
            reset_temp_regs();
            ...
            // (alloc_temp_reg(), disjoint from REG_A0-A2), and
            // checked_bounds_hi is guaranteed call-free (side-effect-free
            // bounds expression), so evaluating it here cannot clobber the
            // just-staged REG_A0/A2 this check (and the ACAS call right after)
            // both still need.
            if ((vm->flags & CCCC_CHECKED_BOUNDS) &&
        ...
        case ND_VLA_PTR:
            ...
            // VLAs are implemented by storing a pointer to dynamically
            // allocated memory The pointer itself is a local variable
            if (node->var->is_local) {
                // Slot address only feeds the immediate load below (#676).
                emit_lea3_internal(vm, dest_reg,
                                   node->var->offset); // Address of pointer
                emit_rr(vm, LDR_D, dest_reg,
                        dest_reg);                     // Load the pointer value
            } else {
        ...
        case ND_BLOCK_LITERAL: {
            // Block descriptor layout (stack-allocated in enclosing function's
            // frame):
            //   [0]  = invoke pointer (function address)
            //   [8]  = descriptor byte-size (for Block_copy to know how much to
            //   malloc) [16] = first captured value,
            //   cc_block_capture_offset(block_fn,1)-th
            ...
            //
            // Stack allocation (via block_desc_var) gives each function
            // invocation its own descriptor, so multiple calls to the same
            // function return independent block instances without aliasing.
            ...
            // Store descriptor size at descriptor[1] so Block_copy knows how
            // much to copy
            ...
            // A capture may come from the enclosing block's own stack frame
            // (direct local) or from the enclosing block's descriptor
            // (transitive capture from a grandparent scope).  Check the
            // enclosing function's capture list first so we read from the right
            // source.
            //
            ...
            for (int i = 0; i < num_captures; i++) {
                ...
                int  enc_cap_idx = (enc_fn && enc_fn->is_block)
                                       ? find_capture_index(enc_fn, cap)
                                       : -1;
                // #1076: when the enclosing function is itself genuinely nested
                // (not a block -- a block ancestor's own transitive captures
                // are already covered by enc_cap_idx above), this capture may
                // be owned by one of *its* ancestors rather than living
                // directly in its own frame, reachable only by chasing enc_fn's
                // own
                // __static_link, not a plain bp+offset of enc_fn's frame.
                // Mirrors gen_addr's identical ND_VAR case (codegen_addr.c) via
                // the shared emit_static_chain_var_addr.
                Obj *outer_owner = (enc_cap_idx < 0 && cap->is_local && enc_fn)
                                       ? belongs_to_outer_function(enc_fn, cap)
                                       : NULL;
                bool wide_copy   = !cap->is_block_var &&
                                   cap->ty->kind != TY_VLA &&
                                   block_capture_needs_mcpy(cap->ty);

                if (enc_cap_idx >= 0) {
                    // Variable lives in the enclosing block's descriptor: read
                    // via
                    // __static_link so we don't use a stale stack offset from
                    // an outer function's frame. Compiler-internal chase
                    // (#676): every intermediate address here feeds an
                    // immediate load.
                    ...
                    emit_addi3(vm, r_val, r_val,
                               cc_block_capture_offset(
                                   enc_fn, enc_cap_idx)); // slot addr
                    if (cap->is_block_var)
                        emit_rr(vm, LDR_D, r_val,
                                r_val);   // heap ptr from descriptor slot
                    else if (!wide_copy)
                        emit_load(vm, cap->ty, r_val,
                                  r_val); // value from descriptor slot
                    ...
                    if (cap->is_param &&
                        (cap->ty->kind == TY_STRUCT ||
                         cap->ty->kind == TY_UNION ||
                         cap->ty->kind == TY_VECTOR ||
                         is_wide_bitint(cap->ty) || is_decimal(cap->ty)))
                        emit_rr(vm, LDR_D, r_val,
                                r_val); // load pointer from slot
                    if (!wide_copy)
                ...
                // Store at descriptor[cc_block_capture_offset(node->block_fn,
                // i)]
        ...
        case ND_BLOCK_CALL: {
            ...
            // Generate user arguments into A1-A7 for ints, FREG_A0-A7 for
            // floats. The descriptor occupies REG_A0 (int slot 0), so int args
            // start at int_arg_idx=1; there is no float static-link, so float
            // args start at float_arg_idx=0. This must match the independent
            // int/float register counters ENT3 uses to spill incoming params
            // (op_ENT3_fn, src/ops.c) -- a single combined "user args start at
            // A1" counter applied to both register files (the previous scheme
            // here) puts float args in the wrong FREG_A* slot as soon as a
            // block takes both an int and a float parameter, or a float
            // parameter at all, since ENT3 counts int and float params
            // separately. Pre-existing bug, found and fixed alongside #712.
            int int_arg_idx   = 1;
            int float_arg_idx = 0;
            for (Node *a = node->args; a; a = a->next) {
                add_type(vm, a);
                ...
                if (is_flonum(a->ty)) {
                    ...
                    // already placed in REG_A1+k could otherwise be clobbered
                    // by an integer scratch this expression's own codegen
                    // reuses (e.g. deref address, int->float cast source).
