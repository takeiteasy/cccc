...
extern Token *__builtin_ast_current_token(void);
extern Token *__builtin_ast_synthetic_token(const char *label);
...
extern Type *__builtin_ast_find_type(const char *name);
extern bool __builtin_ast_type_exists(const char *name);
extern Type *__builtin_ast_get_type(const char *name);
...
extern const char *__builtin_ast_type_c_name(Type *ty);
extern Node *__builtin_ast_int_literal(int64_t value);
extern Node *__builtin_ast_float_literal(double value);
extern Node *__builtin_ast_string_literal(const char *str);
...
extern Node *__builtin_ast_if(Node *cond, Node *then_body, Node *else_body);
extern Node *__builtin_ast_switch(Node *cond);
extern void __builtin_ast_switch_add_case(Node *switch_node, Node *value,
                                Node *body);
extern void __builtin_ast_switch_set_default(Node *switch_node, Node *body);
extern Node *__builtin_ast_block_add_stmt(Node *block, Node *stmt);
extern Node *__builtin_ast_block_add_current_stmt(Node *stmt);
extern void __builtin_ast_switch_add_current_case(Node *value, Node *body);
extern void __builtin_ast_switch_set_current_default(Node *body);
extern int __builtin_ast_enum_count(Type *enum_type);
extern EnumConstant *__builtin_ast_enum_at(Type *enum_type, int index);
extern EnumConstant *__builtin_ast_enum_find(Type *enum_type, const char *name);
...
extern const char *__builtin_ast_enum_name(Type *e);
extern int __builtin_ast_enum_value_count(Type *e);
extern const char *__builtin_ast_enum_value_name(Type *e, int index);
extern int64_t __builtin_ast_enum_value(Type *e, int index);
extern int __builtin_ast_global_count(void);
extern Obj *__builtin_ast_global_at(int index);
extern Obj *__builtin_ast_find_global(const char *name);
...
extern Type *__builtin_ast_make_pointer(Type *base);
extern Type *__builtin_ast_make_array(Type *base, int len);
extern Type *__builtin_ast_make_func_ptr_type(Type *return_ty,
                                            Type **param_types, int nparams);
extern void __builtin_generate_sum(Type *elem_ty);
extern void __builtin_generate_map(Type *elem_ty);
extern void __builtin_generate_reduce(Type *elem_ty);
extern void __builtin_generate_filter(Type *elem_ty);

// Function generation
extern Obj *__builtin_ast_function(const char *name, Type *return_type);
extern void __builtin_ast_function_add_param(Obj *fn, const char *name,
                                    Type *type);
...
extern Node *__builtin_ast_publish(Obj *obj, Token *tok);
extern Node *__builtin_ast_publish_type(Type *ty, Token *tok);
extern Node *__builtin_ast_param_ref(Obj *fn, const char *name);
extern void __builtin_emit_directive(const char *line);

// Ticket #152: global variable generation
extern Obj  *__builtin_ast_global_var(const char *name, Type *ty);
extern void  __builtin_ast_global_var_set_init_data(Obj *var,
...
extern void __builtin_ast_push_fn(Obj *fn);
extern void __builtin_ast_pop_fn(void);
extern void __builtin_ast_push_block(Node *block);
extern void __builtin_ast_pop_block(void);
extern void __builtin_ast_push_struct(Type *ty);
extern void __builtin_ast_pop_struct(void);
extern void __builtin_ast_push_switch(Node *switch_node);
extern void __builtin_ast_pop_switch(void);
extern void __builtin_ast_push_enum(Type *ty);
extern void __builtin_ast_pop_enum(void);

// Ticket #58: AST dump
extern void __builtin_dump_tree(Node *node);
extern const char *__builtin_dump_tree_to_string(Node *node);
extern void __builtin_dump_ast_gen(Node *node);
extern const char *__builtin_dump_ast_gen_to_string(Node *node);
...
extern void __builtin_macro_error_at(Node *node, const char *fmt, ...);
extern void __builtin_macro_warning_at(Node *node, const char *fmt, ...);

// Previously unregistered statement builders (existing functions, now exposed)
extern Node *__builtin_ast_block(Node **stmts, int count);
extern Node *__builtin_ast_expr_stmt(Node *expr);

// Ticket #77: hygienic local variable injection
extern Node *__builtin_ast_local_var(const char *name, Type *ty);
extern Node *__builtin_ast_local_var_unique(Type *ty);

// Struct/union member introspection (previously unregistered)
extern int     __builtin_ast_struct_member_count(Type *struct_type);
extern Member *__builtin_ast_struct_member_at(Type *struct_type, int index);
extern Member *__builtin_ast_struct_member_find(Type *struct_type,
                                              const char *name);
...
extern int64_t  __builtin_ast_offsetof_chain(Type *ty,
                                          const char **names, int n);

// Ticket #235: serialization
extern Node *__builtin_ast_serialize(Type *ty, Node *expr, Node *buf);
extern Node *__builtin_ast_deserialize(Type *ty, Node *buf);

// Ticket #235: enum <-> string conversion
extern Node *__builtin_ast_enum_to_string_switch(Type *ty, Node *expr);
extern Node *__builtin_ast_enum_from_string_chain(Type *ty, Node *expr);

// Ticket #51: new expression/statement builders
extern Node *__builtin_ast_assign(Node *target, Node *value);
extern Node *__builtin_ast_member(Node *obj, const char *name);
extern Node *__builtin_ast_funcall(Node *callee, Node **args, int n);
extern Node *__builtin_ast_while(Node *cond, Node *body);
extern Node *__builtin_ast_for(Node *init, Node *cond, Node *inc, Node *body);
extern Node *__builtin_ast_do_while(Node *body, Node *cond);

// Ticket #1: quasi-quoting; Ticket #172: list splice helper
extern Node *__builtin_quote(const char *tmpl, ...);
extern Node *__builtin_quote_n(const char *tmpl, Node **nodes, int count);
extern Node *__builtin_node_list(Node **nodes, int count);

// Ticket #171: new expression builders
extern Node  *__builtin_ast_cond(Node *cond, Node *then_expr, Node *else_expr);
extern Node  *__builtin_ast_null(void);
extern Node  *__builtin_ast_sizeof_type(Type *ty);
extern Node  *__builtin_ast_alignof_type(Type *ty);
extern Node  *__builtin_ast_sizeof_expr(Node *expr);
extern Node  *__builtin_ast_subscript(Node *arr, Node *idx);
extern Node  *__builtin_ast_comma(Node *lhs, Node *rhs);

// Ticket #171: qualified type builders
extern Type  *__builtin_ast_make_const(Type *ty);
extern Type  *__builtin_ast_make_volatile(Type *ty);

// Ticket #171: function prototype builder
extern Obj   *__builtin_ast_function_prototype(const char *name,
                                            Type *return_type);

// Ticket #619: generic attribute application for AST-generated functions
extern void __builtin_ast_add_attribute(Obj *fn, const char *attr_text);
extern void __builtin_ast_add_build_target_attr(Obj *fn, const char *kind);

// Ticket #171: struct/union/enum/typedef type builders
extern Type  *__builtin_ast_make_struct(const char *name);
extern Type  *__builtin_ast_make_union(const char *name);
extern Type  *__builtin_ast_struct_add_field(Type *ty, const char *name,
                                          Type *field_type);
extern Type  *__builtin_ast_struct_add_current_field(const char *name,
                                                  Type *field_type);
extern Type  *__builtin_ast_make_enum(const char *name);
extern void   __builtin_ast_enum_add_constant(Type *ty, const char *name,
                                           int value);
extern void   __builtin_ast_enum_add_current_constant(const char *name,
                                                   int value);
extern Type  *__builtin_ast_make_typedef(const char *name, Type *underlying);

// Ticket #188: comptime variable access
extern int64_t __builtin_get_comptime_int(const char *name);
extern double  __builtin_get_comptime_float(const char *name);
extern Node   *__builtin_get_comptime_var(const char *name);
extern Node   *__builtin_get_comptime_ptr(const char *name);
extern Node   *__builtin_get_comptime_member(const char *var_name,
                                         const char *field);

// Ticket #189: constexpr variable access
extern Node   *__builtin_get_constexpr_value(const char *name);

// Ticket #296: initializer builders
extern Node   *__builtin_ast_compound_literal(Type *ty, Node **inits, int n);
extern Node   *__builtin_ast_init_array(Type *elem_ty, Node **elems, int n);
extern Node   *__builtin_ast_init_struct(Type *ty, const char **fields,
                                     Node **values, int n);

// Ticket #277: Lisp-style single-macro expansion
extern Node   *__builtin_macroexpand_1(Node *node);
extern Node   *__builtin_macroexpand(Node *node);
extern int     __builtin_ast_vararg_count(void);
extern Node   *__builtin_ast_vararg_at(int index);
extern Node   **__builtin_ast_varargs_as_array(void);
extern const char *__builtin_ast_vararg_str_at(int index);
...
static void register_reflection_ffi(VirtualMachine *vm) {
    cc_register_cfunc(vm, "__builtin_gensym", (void *)__builtin_gensym, 1, 0);

    // Source location helpers
    cc_register_cfunc(vm, "__builtin_ast_current_token",
                      (void *)__builtin_ast_current_token, 0, 0);
    cc_register_cfunc(vm, "__builtin_ast_synthetic_token",
                      (void *)__builtin_ast_synthetic_token, 1, 0);
    ...
    // Type lookup
    cc_register_cfunc(vm, "__builtin_ast_find_type", (void *)__builtin_ast_find_type, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_type_exists", (void *)__builtin_ast_type_exists, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_get_type", (void *)__builtin_ast_get_type, 1, 0);
    ...
    cc_register_cfunc(vm, "__builtin_ast_type_c_name", (void *)__builtin_ast_type_c_name, 1, 0);

    // Type construction
    cc_register_cfunc(vm, "__builtin_ast_make_pointer", (void *)__builtin_ast_make_pointer, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_make_array", (void *)__builtin_ast_make_array, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_make_func_ptr_type", (void *)__builtin_ast_make_func_ptr_type, 3, 0);
    cc_register_cfunc(vm, "__builtin_generate_sum", (void *)__builtin_generate_sum, 1, 0);
    cc_register_cfunc(vm, "__builtin_generate_map", (void *)__builtin_generate_map, 1, 0);
    cc_register_cfunc(vm, "__builtin_generate_reduce", (void *)__builtin_generate_reduce, 1, 0);
    cc_register_cfunc(vm, "__builtin_generate_filter", (void *)__builtin_generate_filter, 1, 0);

    // Literal construction
    cc_register_cfunc(vm, "__builtin_ast_int_literal", (void *)__builtin_ast_int_literal, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_float_literal", (void *)__builtin_ast_float_literal, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_string_literal", (void *)__builtin_ast_string_literal, 1,
                      0);
    cc_register_cfunc(vm, "__builtin_ast_var_ref", (void *)__builtin_ast_var_ref, 1, 0);

    // Expression construction
    cc_register_cfunc(vm, "__builtin_ast_binary", (void *)__builtin_ast_binary, 3, 0);
    cc_register_cfunc(vm, "__builtin_ast_unary", (void *)__builtin_ast_unary, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_cast", (void *)__builtin_ast_cast, 2, 0);

    // Statement construction
    cc_register_cfunc(vm, "__builtin_ast_return", (void *)__builtin_ast_return, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_if", (void *)__builtin_ast_if, 3, 0);
    cc_register_cfunc(vm, "__builtin_ast_switch", (void *)__builtin_ast_switch, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_switch_add_case", (void *)__builtin_ast_switch_add_case, 3,
                      0);
    cc_register_cfunc(vm, "__builtin_ast_switch_set_default",
                      (void *)__builtin_ast_switch_set_default, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_switch_add_current_case",
                      (void *)__builtin_ast_switch_add_current_case, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_switch_set_current_default",
                      (void *)__builtin_ast_switch_set_current_default, 1, 0);

    // Ticket #58: AST dump
    cc_register_cfunc(vm, "__builtin_dump_tree",              (void *)__builtin_dump_tree,              1, 0);
    cc_register_cfunc(vm, "__builtin_dump_tree_to_string",    (void *)__builtin_dump_tree_to_string,    1, 0);
    cc_register_cfunc(vm, "__builtin_dump_ast_gen",           (void *)__builtin_dump_ast_gen,           1, 0);
    cc_register_cfunc(vm, "__builtin_dump_ast_gen_to_string", (void *)__builtin_dump_ast_gen_to_string, 1, 0);
    cc_register_cfunc(vm, "__builtin_emit_directive",
                      (void *)__builtin_emit_directive, 1, 0);

    // Ticket #78: source-located macro diagnostics (variadic)
    cc_register_variadic_cfunc(vm, "__builtin_macro_error_at",   (void *)__builtin_macro_error_at,   2, 0);
    cc_register_variadic_cfunc(vm, "__builtin_macro_warning_at", (void *)__builtin_macro_warning_at, 2, 0);

    // Previously unregistered statement builders
    cc_register_cfunc(vm, "__builtin_ast_block",     (void *)__builtin_ast_block,     2, 0);
    cc_register_cfunc(vm, "__builtin_ast_block_add_stmt",
                      (void *)__builtin_ast_block_add_stmt, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_block_add_current_stmt",
                      (void *)__builtin_ast_block_add_current_stmt, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_expr_stmt", (void *)__builtin_ast_expr_stmt, 1, 0);

    // Ticket #77: hygienic local variable injection
    cc_register_cfunc(vm, "__builtin_ast_local_var",        (void *)__builtin_ast_local_var,        2, 0);
    cc_register_cfunc(vm, "__builtin_ast_local_var_unique",  (void *)__builtin_ast_local_var_unique,  1, 0);

    // Ticket #51: new expression/statement builders
    cc_register_cfunc(vm, "__builtin_ast_assign",   (void *)__builtin_ast_assign,   2, 0);
    cc_register_cfunc(vm, "__builtin_ast_member",   (void *)__builtin_ast_member,   2, 0);

    // Struct/union member introspection (previously unregistered)
    cc_register_cfunc(vm, "__builtin_ast_struct_member_count",
                      (void *)__builtin_ast_struct_member_count, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_struct_member_at",
                      (void *)__builtin_ast_struct_member_at, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_struct_member_find",
                      (void *)__builtin_ast_struct_member_find, 2, 0);
    ...
    cc_register_cfunc(vm, "__builtin_ast_offsetof_chain",
                      (void *)__builtin_ast_offsetof_chain, 3, 0);

    // Ticket #235: serialization
    cc_register_cfunc(vm, "__builtin_ast_serialize",
                      (void *)__builtin_ast_serialize, 3, 0);
    cc_register_cfunc(vm, "__builtin_ast_deserialize",
                      (void *)__builtin_ast_deserialize, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_enum_to_string_switch",
                      (void *)__builtin_ast_enum_to_string_switch, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_enum_from_string_chain",
                      (void *)__builtin_ast_enum_from_string_chain, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_funcall",  (void *)__builtin_ast_funcall,  3, 0);
    cc_register_cfunc(vm, "__builtin_ast_while",    (void *)__builtin_ast_while,    2, 0);
    cc_register_cfunc(vm, "__builtin_ast_for",      (void *)__builtin_ast_for,      4, 0);
    cc_register_cfunc(vm, "__builtin_ast_do_while", (void *)__builtin_ast_do_while, 2, 0);

    // Enum reflection
    cc_register_cfunc(vm, "__builtin_ast_enum_count", (void *)__builtin_ast_enum_count, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_enum_at", (void *)__builtin_ast_enum_at, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_enum_find", (void *)__builtin_ast_enum_find, 2, 0);
    ...
    cc_register_cfunc(vm, "__builtin_ast_enum_name", (void *)__builtin_ast_enum_name, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_enum_value_count",
                      (void *)__builtin_ast_enum_value_count, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_enum_value_name",
                      (void *)__builtin_ast_enum_value_name, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_enum_value", (void *)__builtin_ast_enum_value, 2, 0);

    // Global/object and custom-attribute target introspection
    cc_register_cfunc(vm, "__builtin_ast_global_count",
                      (void *)__builtin_ast_global_count, 0, 0);
    cc_register_cfunc(vm, "__builtin_ast_global_at",
                      (void *)__builtin_ast_global_at, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_find_global",
                      (void *)__builtin_ast_find_global, 1, 0);
    ...
    // Function generation
    cc_register_cfunc(vm, "__builtin_ast_function", (void *)__builtin_ast_function, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_function_add_param",
                      (void *)__builtin_ast_function_add_param, 3, 0);
    cc_register_cfunc(vm, "__builtin_ast_function_set_body",
                      (void *)__builtin_ast_function_set_body, 2, 0);
    ...
    cc_register_cfunc(vm, "__builtin_ast_publish",
                      (void *)__builtin_ast_publish, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_publish_type",
                      (void *)__builtin_ast_publish_type, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_param_ref", (void *)__builtin_ast_param_ref, 2, 0);

    // Ticket #152: global variable generation
    cc_register_cfunc(vm, "__builtin_ast_global_var",
                      (void *)__builtin_ast_global_var, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_global_var_set_init_data",
                      (void *)__builtin_ast_global_var_set_init_data, 3, 0);
    ...
    cc_register_cfunc(vm, "__builtin_ast_push_fn",
                      (void *)__builtin_ast_push_fn, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_pop_fn",
                      (void *)__builtin_ast_pop_fn, 0, 0);
    cc_register_cfunc(vm, "__builtin_ast_push_block",
                      (void *)__builtin_ast_push_block, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_pop_block",
                      (void *)__builtin_ast_pop_block, 0, 0);
    cc_register_cfunc(vm, "__builtin_ast_push_struct",
                      (void *)__builtin_ast_push_struct, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_pop_struct",
                      (void *)__builtin_ast_pop_struct, 0, 0);
    cc_register_cfunc(vm, "__builtin_ast_push_switch",
                      (void *)__builtin_ast_push_switch, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_pop_switch",
                      (void *)__builtin_ast_pop_switch, 0, 0);
    cc_register_cfunc(vm, "__builtin_ast_push_enum",
                      (void *)__builtin_ast_push_enum, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_pop_enum",
                      (void *)__builtin_ast_pop_enum, 0, 0);

    // Ticket #1: quasi-quoting
    cc_register_variadic_cfunc(vm, "__builtin_quote",      (void *)__builtin_quote,      1, 0);
    cc_register_cfunc(vm,          "__builtin_quote_n",    (void *)__builtin_quote_n,    3, 0);

    // Ticket #172: list splice helper
    cc_register_cfunc(vm,          "__builtin_node_list",  (void *)__builtin_node_list,  2, 0);

    // Ticket #171: new expression builders
    cc_register_cfunc(vm, "__builtin_ast_cond",         (void *)__builtin_ast_cond,         3, 0);
    cc_register_cfunc(vm, "__builtin_ast_null",         (void *)__builtin_ast_null,         0, 0);
    cc_register_cfunc(vm, "__builtin_ast_sizeof_type",  (void *)__builtin_ast_sizeof_type,  1, 0);
    cc_register_cfunc(vm, "__builtin_ast_alignof_type", (void *)__builtin_ast_alignof_type, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_sizeof_expr",  (void *)__builtin_ast_sizeof_expr,  1, 0);
    cc_register_cfunc(vm, "__builtin_ast_subscript",    (void *)__builtin_ast_subscript,    2, 0);
    cc_register_cfunc(vm, "__builtin_ast_comma",        (void *)__builtin_ast_comma,        2, 0);

    // Ticket #171: qualified type builders
    cc_register_cfunc(vm, "__builtin_ast_make_const",    (void *)__builtin_ast_make_const,    1, 0);
    cc_register_cfunc(vm, "__builtin_ast_make_volatile", (void *)__builtin_ast_make_volatile, 1, 0);

    // Ticket #171: function prototype builder
    cc_register_cfunc(vm, "__builtin_ast_function_prototype",
                      (void *)__builtin_ast_function_prototype, 2, 0);

    // Ticket #619: generic attribute application for AST-generated functions
    cc_register_cfunc(vm, "__builtin_ast_add_attribute",
                      (void *)__builtin_ast_add_attribute, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_add_build_target_attr",
                      (void *)__builtin_ast_add_build_target_attr, 2, 0);

    // Ticket #171: struct/union/enum/typedef type builders
    cc_register_cfunc(vm, "__builtin_ast_make_struct",
                      (void *)__builtin_ast_make_struct, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_make_union",
                      (void *)__builtin_ast_make_union, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_struct_add_field",
                      (void *)__builtin_ast_struct_add_field, 3, 0);
    cc_register_cfunc(vm, "__builtin_ast_struct_add_current_field",
                      (void *)__builtin_ast_struct_add_current_field, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_make_enum",
                      (void *)__builtin_ast_make_enum, 1, 0);
    cc_register_cfunc(vm, "__builtin_ast_enum_add_constant",
                      (void *)__builtin_ast_enum_add_constant, 3, 0);
    cc_register_cfunc(vm, "__builtin_ast_enum_add_current_constant",
                      (void *)__builtin_ast_enum_add_current_constant, 2, 0);
    cc_register_cfunc(vm, "__builtin_ast_make_typedef",
                      (void *)__builtin_ast_make_typedef, 2, 0);

    // Ticket #188: comptime variable access
    cc_register_cfunc(vm, "__builtin_get_comptime_int",
                      (void *)__builtin_get_comptime_int, 1, 0);
    cc_register_cfunc(vm, "__builtin_get_comptime_float",
                      (void *)__builtin_get_comptime_float, 1, 1); // returns double
    cc_register_cfunc(vm, "__builtin_get_comptime_var",
                      (void *)__builtin_get_comptime_var, 1, 0);
    cc_register_cfunc(vm, "__builtin_get_comptime_ptr",
                      (void *)__builtin_get_comptime_ptr, 1, 0);
    cc_register_cfunc(vm, "__builtin_get_comptime_member",
                      (void *)__builtin_get_comptime_member, 2, 0);

    // Ticket #189: constexpr variable access
    cc_register_cfunc(vm, "__builtin_get_constexpr_value",
                      (void *)__builtin_get_constexpr_value, 1, 0);

    // Ticket #296: initializer builders
    // NOTE: these three were already under-registered by one arg *before*
    // this refactor (e.g. compound_literal took vm+ty+inits+n = 4 args but
    // was registered with arity 3) -- harmless in practice since the FFI
    // table's arity only feeds --ffi-type-checking's diagnostic, not the
    // actual calling convention, but corrected here to the true arg count
    // now that vm is gone (ty+inits+n = 3, elem_ty+elems+n = 3,
    // ty+fields+values+n = 4) while touching this table anyway.
