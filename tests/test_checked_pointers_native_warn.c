// CCCC_FLAGS: --build
// CCCC_EXPECT_STDOUT: warning: -c=native ignores VM runtime safety/debug options \(--checked-pointers\): they are enforced by the CCCC VM only.*run=42
//
...
int build_main(Builder *ctx) {
    ...
    if (!f) return 1;
    fputs(
        "int f(int * [[cccc::array, cccc::count(3)]] p) { return p[0]; }\n"
