...
int main(void) {
    ...
    float f = 99.0f;
    if (setpayloadf(&f, -1.0f) == 0)
        return 7;
    if (f != 0.0f)
        return 8;

    f = 99.0f;
    if (setpayloadf(&f, 1.5f) == 0)
        return 9;
    if (f != 0.0f)
        return 10;

    f = 99.0f;
    if (setpayloadf(&f, 1.0e20f) == 0)
        return 11;
    if (f != 0.0f)
        return 12;
    ...
    d = 99.0;
    if (setpayloadsig(&d, 0.0) == 0)
        return 15;
    if (d != 0.0)
        return 16;

    f = 99.0f;
    if (setpayloadsigf(&f, 0.0f) == 0)
        return 17;
    if (f != 0.0f)
        return 18;

    ld = 99.0;
    if (setpayloadsigl(&ld, 0.0) == 0)
        return 19;
    if (ld != 0.0)
        return 20;
    ...
    d = 0.0;
    if (setpayload(&d, 42.0) != 0)
        return 21;
    if (!(d != d))
        return 22; // must be NaN, not 0.0
