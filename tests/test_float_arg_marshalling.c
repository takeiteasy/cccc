...
static int chkf(int n, float f) {
    return (n == 7 && f == 3.5f) ? 1 : 0;
}
...
int main(void) {
    ...
    if (strcmp(buf, "7 1.000000 2.000000") != 0)
        return 3;

    snprintf(buf, sizeof(buf), "%f", (double)n);
    if (strcmp(buf, "3.000000") != 0)
        return 4;
