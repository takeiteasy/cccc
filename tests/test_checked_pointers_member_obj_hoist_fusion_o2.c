...
// hi -- test_checked_pointers_member_fusion_o2.c only proves the TRIVIAL
// (bare-variable) object-expression case survives -O2's indexed load/store
// fusion, which wouldn't have caught a regression specific to the hoisted
// temp itself (e.g. the temp's store getting fused away, or a stale value
// surviving across optimization passes). This proves the out-of-bounds
