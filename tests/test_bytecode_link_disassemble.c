...
int build_main(Builder *ctx) {
    ...
    char lib_out[512], app_out[512], disasm_direct[512], disasm_from_file[512],
        cmd[1024];
    snprintf(lib_out, sizeof(lib_out), "%s/lib/mathlib_link_disasm_test.c4a",
             out_dir);
    snprintf(app_out, sizeof(app_out), "%s/bin/app_link_disasm_test.c4",
             out_dir);
    snprintf(disasm_direct, sizeof(disasm_direct), "%s/link_disasm_direct.txt",
             out_dir);
    snprintf(disasm_from_file, sizeof(disasm_from_file),
             "%s/link_disasm_from_file.txt", out_dir);
    ...
    snprintf(cmd, sizeof(cmd),
             ...
             "-I tests/fixtures/build_bytecode_libs_demo/include --link %s -o "
             "%s 2>&1",
             lib_out, app_out);
    if (!CaptureCommand(ctx, cmd))
        return 1;

    snprintf(cmd, sizeof(cmd), "./cccc %s --disassemble > %s 2>&1", app_out,
             disasm_from_file);
    if (!CaptureCommand(ctx, cmd))
        return 1;

    snprintf(cmd, sizeof(cmd),
             "diff -q %s %s >/dev/null 2>&1 && echo SAME || echo DIFFERENT",
