...
// ordering edge from the executable to the custom step without adding a -l
// flag. Verified via dry-run: the custom command line must appear in the output
// and the link step must NOT contain -lgen (DependsOn is order-only, not
// link-with).
...
int build_main(Builder *ctx) {
    ...
    // Executable depends on gen for ordering (DependsOn), not linking
    // (LinkWith)
