...
//
// When --build-target=NAME matches a [[cccc::build_target]] factory, the factory
// is invoked directly and build_main is skipped entirely.  The dry-run output
// must contain the factory's source and must NOT contain build_main's source.
