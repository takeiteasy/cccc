...
// CCCC_EXPECT_LEAK: c4's own sym/e/data/sp interpreter buffers are malloc'd
// once and never freed by design (process exits right after)
        ...
        if (tk == '\n') {
            if (src) {
                ...
                while (le < e) {
                    printf(
                        "%8.4s",
                        &"LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC "
                         " ,SI  ,SC  ,PSH ,"
                         "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL "
                         ",SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
                         "OPEN,READ,CLOS,PRTF,MALC,FREE,MSET,MCMP,EXIT,"[*++le *
                                                                         5]);
                    if (*le <= ADJ)
                        printf(" %d\n", *++le);
                    else
