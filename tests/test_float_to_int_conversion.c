...
int main(void) {
    ...
    if (g_u64_from_double != 15000000000000000000ULL)
        return 25;
    ...
    if (local_const != 15000000000000000000ULL)
        return 26;
    ...
    if (ur != 0)
        return 41;
    if (!fetestexcept(FE_INVALID))
        return 43; // 42 is this file's success code
    ...
    volatile double just_under_2p63 =
    ...
    if (r != -1)
        return 17; // truncates toward zero, like a plain cast
    if (fetestexcept(FE_INVALID))
        return 18;
