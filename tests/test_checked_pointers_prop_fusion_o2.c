...
// + one -f pass. #919's propagated deref (q[i], q snapshotted from p) reaches
// its address through the same ND_ADD/ND_VAR shape as a direct checked-pointer
// access, but the checked_bounds_lo/hi fields are only populated post-parse by
