...
static void *readv_blocker(void *arg) {
    ...
    if (buf1[0] != 'h' || buf1[1] != 'i')
        return (void *)(long long)2;
    if (buf2[0] != '!' || buf2[1] != '!')
        return (void *)(long long)3;
    return 0;
}
...
int main(void) {
    // Phase 1: readv
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, rv_sv) != 0)
        return 10;
    struct timeval tv1 = {1, 0};
    if (setsockopt(rv_sv[0], SOL_SOCKET, SO_RCVTIMEO, &tv1, sizeof(tv1)) != 0)
        return 11;

    pthread_t r1, r2;
    if (pthread_create(&r1, 0, readv_blocker, 0) != 0)
        return 12;
    if (pthread_create(&r2, 0, readv_writer, 0) != 0)
        return 13;
    void *rres = 0;
    if (pthread_join(r1, &rres) != 0)
        return 14;
    if (pthread_join(r2, 0) != 0)
        return 15;
    close(rv_sv[0]);
    close(rv_sv[1]);
    if (rres != 0)
    ...
    // Phase 2: writev
    for (int i = 0; i < WV_PAYLOAD_LEN; i++)
        wv_payload[i] = (char)(i & 0xff);

    if (socketpair(AF_UNIX, SOCK_STREAM, 0, wv_sv) != 0)
        return 20;
    int small_buf = 4096;
    if (setsockopt(wv_sv[0], SOL_SOCKET, SO_SNDBUF, &small_buf,
                   sizeof(small_buf)) != 0)
        return 21;
    if (setsockopt(wv_sv[1], SOL_SOCKET, SO_RCVBUF, &small_buf,
                   sizeof(small_buf)) != 0)
        return 22;
    struct timeval tv2 = {1, 0};
    if (setsockopt(wv_sv[0], SOL_SOCKET, SO_SNDTIMEO, &tv2, sizeof(tv2)) != 0)
        return 23;

    pthread_t w1, w2;
    wv_written = -1;
    if (pthread_create(&w1, 0, writev_blocker, 0) != 0)
        return 24;
    if (pthread_create(&w2, 0, writev_drainer, 0) != 0)
        return 25;
    if (pthread_join(w1, 0) != 0)
        return 26;
    if (pthread_join(w2, 0) != 0)
        return 27;
    close(wv_sv[0]);
    close(wv_sv[1]);
    if (wv_written != WV_PAYLOAD_LEN)
        return 28;
