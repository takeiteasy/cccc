...
// 'i'.*\[-Woverride-init\] Ticket #960 follow-up: struct_designator()'s two
// -Woverride-init call sites used to print mem->name->len/loc for a
// designator-resolved member, which is NULL for an anonymous struct/union
// member -- a second NULL deref alongside #960's main crash, latent in the same
// code path. Both sites are guarded (src/parse.c) to skip the wrapper-level
// check for an anonymous member entirely, deferring to the recursive
// designation() call that resolves the real leaf field.
//
