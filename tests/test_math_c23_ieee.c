...
int main(void) {
    ...
    // non-NaN operand).
    if (fmaximum(3.0, 5.0) != 5.0)
        return 1;
    if (fminimum(3.0, 5.0) != 3.0)
        return 2;
    if (!(fmaximum(3.0, nan_val) != fmaximum(3.0, nan_val)))
        return 3; // NaN
    if (!(fminimum(3.0, nan_val) != fminimum(3.0, nan_val)))
        return 4; // NaN
    ...
    if (!(fmaximum(0.0, -0.0) == 0.0 && 1.0 / fmaximum(0.0, -0.0) > 0.0))
        return 5;
    if (!(fminimum(0.0, -0.0) == 0.0 && 1.0 / fminimum(0.0, -0.0) < 0.0))
        return 6;

    // fmaximum_num/fminimum_num: ignore a single NaN like fmax/fmin.
    if (fmaximum_num(3.0, nan_val) != 3.0)
        return 7;
    if (fminimum_num(3.0, nan_val) != 3.0)
        return 8;
    if (!(fmaximum_num(nan_val, nan_val) != fmaximum_num(nan_val, nan_val)))
        return 9;

    // fmaximum_mag/fminimum_mag: compare by magnitude.
    if (fmaximum_mag(-5.0, 3.0) != -5.0)
        return 10;
    if (fminimum_mag(-5.0, 3.0) != 3.0)
        return 11;
    ...
    if (!totalorder(&tneg0, &tpos0))
        return 13;
    if (totalorder(&tpos0, &tneg0))
        return 14;
    if (!totalorder(&tnegone, &tposone))
        return 15;
    if (totalorder(&tposone, &tnegone))
        return 16;
    float ftnegone = -1.0f, ftposone = 1.0f;
    if (!totalorderf(&ftnegone, &ftposone))
        return 17;

    // totalordermag: compares |x|, |y|.
    double tm5 = -5.0, tm3 = 3.0, tm3b = -3.0, tm5b = 5.0;
    if (totalordermag(&tm5, &tm3))
        return 18; // |-5| > |3| -> false
    if (!totalordermag(&tm3b, &tm5b))
        return 19; // |-3| < |5| -> true

    // canonicalize: IEEE 754 binary formats have no non-canonical
    // encodings, so this is a copy that always succeeds.
    double cx, src = 3.5;
    if (canonicalize(&cx, &src) != 0)
        return 20;
    if (cx != 3.5)
        return 21;
    ...
    if (p != 0.0)
        return 22; // a plain 0.0/0.0 NaN has payload 0

    double non_nan = 1.5;
    if (getpayload(&non_nan) != -1.0)
        return 23; // not a NaN -> -1

    double sp;
    if (setpayload(&sp, 42.0) != 0)
        return 24;
    if (!(sp != sp))
        return 25; // must be NaN
    if (getpayload(&sp) != 42.0)
        return 26;

    double ssig;
    if (setpayloadsig(&ssig, 7.0) != 0)
        return 27;
    if (!issignaling(ssig))
        return 28;
    if (setpayloadsig(&ssig, 0.0) == 0)
        return 29; // payload 0 -> would be Inf, must fail
    if (ssig != 0.0)
        return 53; // #1079: failure must zero the destination
                   // (glibc-conforming)
    if (setpayloadsig(&ssig, 7.0) != 0)
        return 54; // re-establish a signaling NaN for iseqsig below
    ...
    if (fromfp(3.7, FP_INT_TONEAREST, 8) != 4)
        return 32;
    feclearexcept(FE_ALL_EXCEPT);
    (void)fromfp(10000000000.0, FP_INT_TONEAREST, 8); // overflow
    if (!fetestexcept(FE_INVALID))
        return 33;
    if (ufromfp(3.7, FP_INT_UPWARD, 8) != 4)
        return 34;
    feclearexcept(FE_ALL_EXCEPT);
    ...
    if (!fetestexcept(FE_INEXACT))
        return 38; // 'x' variant: rounded != input

    // issignaling/iseqsig/iscanonical.
    if (issignaling(nan_val))
        return 39; // 0.0/0.0 is a quiet NaN
    if (!iscanonical(1.0))
        return 40;

    feclearexcept(FE_ALL_EXCEPT);
    if (iseqsig(ssig, ssig))
        return 41; // sNaN != itself
    if (!fetestexcept(FE_INVALID))
        return 51; // signaling NaN operand -> FE_INVALID
    if (!iseqsig(1.0, 1.0))
        return 52;
