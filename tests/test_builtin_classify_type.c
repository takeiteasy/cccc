...
int main(void) {
    if (__builtin_classify_type(1) != 1) return 1;             // int
    char ch = 'a';
    if (__builtin_classify_type(ch) != 2) return 2;              // char
    ...
    if (__builtin_classify_type((enum color)RED) != 3) return 3; // enum
    if (__builtin_classify_type((_Bool)1) != 4) return 4;        // bool
    if (__builtin_classify_type((void *)0) != 5) return 5;      // pointer
    if (__builtin_classify_type(1.0) != 8) return 6;            // double
    if (__builtin_classify_type(1.0f) != 8) return 7;           // float
    struct point p;
    if (__builtin_classify_type(p) != 12) return 8;             // struct
    union u un;
    if (__builtin_classify_type(un) != 13) return 9;             // union
    int arr[4];
    if (__builtin_classify_type(arr) != 14) return 10;           // array

    v4sf v;
    if (__builtin_classify_type(v) != 99) return 11;             // vector
    ...
    if (__builtin_classify_type(*(v4sf *)0) != 99) return 12;
    if (__builtin_classify_type(*(int *)0) != 1) return 13;

    int x = 0;
    int cls = __builtin_classify_type(x++);
    if (cls != 1) return 14;
    if (x != 0) return 15; // x++ must not have run
