...
int main(void) {
    ...
    if (abs(-5) != 5) return 13;
    if (labs(-7L) != 7L) return 14;
    if (llabs(-9LL) != 9LL) return 15;

    // strtof: float-returning.
    float sf = strtof("3.5", NULL);
    if (sf != 3.5f) return 16;
