// Test ticket #195: $@k unquote-splicing in compound-literal initializer-list
// position. main() returns 42 on success.
...
// Builds a two-element chain and splices it as positional initializers for
// Point.
[[cccc::comptime]]
...
int main(void) {
    if (!test_struct_splice())
        return 1;
    if (!test_array_splice())
        return 2;
    if (!test_struct_triple())
        return 3;
    if (!test_scalar_and_splice())
        return 4;
