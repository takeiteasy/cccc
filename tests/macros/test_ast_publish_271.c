...
void define_published_fn(void) {
    ...
    FunctionSetBody(fn, MakeReturn(MakeBinary(NK_ADD, MakeParamRef(fn, "x"),
                                              MakeIntLiteral(1))));
}
define_published_fn();
...
int main(void) {
    if (published_add_one(41) != 42)
        return 1;
    if (published_bytes[0] != 'C')
        return 2;
    if (published_bytes[1] != 'C')
        return 3;
    if (published_bytes[2] != 'C')
        return 4;
    ...
    if (n != 42)
        return 6;
    if (PUBLISHED_TAG_OK != 7)
        return 7;
    if (published_alias_fn() != 5)
        return 8;
    return 42;
}
