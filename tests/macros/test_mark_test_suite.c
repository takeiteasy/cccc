...
// attribute string — equivalent to the old
// TestSetSuite/TestSetDisplayName/TestSetTimeout.
...
void gen_suite_test(void) {
    ...
    PublishNode(fn);
    AddAttribute(fn, "cccc::test(suite=\"generated_suite\", name=\"custom "
                     "display name\", timeout=5000)");
}
gen_suite_test();
