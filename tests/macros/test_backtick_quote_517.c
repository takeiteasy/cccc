...
int plain_result(void) {
    plain_quote();
}
int single_result(void) {
    single_splice(41);
}
int multiple_result(void) {
    multiple_splices(20, 22);
}
int call_result(void) {
    call_splice(1, 42);
}
int nested_result(void) {
    nested_brace_splice(1, 42);
}
int ignored_braces_result(void) {
    ignored_braces_splice(42);
}
int reflect_result(void) {
    reflect_in_quote();
}
int backtick_result(void) {
    escaped_backtick();
}
int backslash_result(void) {
    preserved_backslash();
}
int placeholder_text_result(void) {
    literal_placeholder_text();
}
int multiline_result(void) {
    multiline_quote();
}
...
int main(void) {
    ...
    if (single_result() != 42)
        return 2;
    if (multiple_result() != 42)
        return 3;
    if (call_result() != 42)
        return 4;
    if (nested_result() != 42)
        return 5;
    if (ignored_braces_result() != 42)
        return 6;
    if (reflect_result() != sizeof(void *))
        return 7;
    if (backtick_result() != '`')
        return 8;
    if (backslash_result() != '\n')
        return 9;
    if (placeholder_text_result() != 3)
        return 10;
    if (multiline_result() != 42)
        return 11;
    return 42;
}
