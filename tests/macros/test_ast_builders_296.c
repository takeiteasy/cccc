...
Node *gen_array_fn(void) {
    ...
    WithFn(fn) {
        // int[3]{7, 14, 21}[1] == 14
        Node *arr = InitArray(int_ty,
            MakeIntLiteral(7), MakeIntLiteral(14), MakeIntLiteral(21));
...
Node *is_both_sum(void) {
    ...
    // Fresh instance for y access (each InitStruct call creates its own anon var).
