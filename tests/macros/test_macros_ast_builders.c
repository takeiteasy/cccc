// Test ticket #51: __builtin_ast_assign, __builtin_ast_member,
// __builtin_ast_funcall builders

// ---- __builtin_ast_assign test
// ----------------------------------------------- Macro that returns an
// assignment expression (target = value). Used as: int x; int y = set_var(x,
// 99); After expansion: int y = (x = 99);  => y == 99, x == 99
[[cccc::comptime]]
...
// ---------------------------------------------- Helper function called by the
// generated funcall node
int triple(int n) {
    return n * 3;
}
