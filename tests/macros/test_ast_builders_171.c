...
Node *choose(void) {
    ...
    Node *body = MakeReturn(
        MakeCond(cond, MakeIntLiteral(1), MakeIntLiteral(2)));
...
Node *get_null_ptr(void) {
    ...
    Node *body = MakeReturn(
        MakeCast(MakeCast(MakeNull(), long_ty), int_ty));
    FunctionSetBody(fn, body);
        return MakeIntLiteral(0);
}
get_null_ptr();
...
Node *sizeof_expr_test(void) {
    ...
    Node *body = MakeReturn(
        MakeCast(MakeSizeofExpr(MakeParamRef(fn, "x")), int_ty));
    FunctionSetBody(fn, body);
        return MakeIntLiteral(0);
}
sizeof_expr_test();
...
Node *subscript_test(void) {
    ...
    Node *body = MakeReturn(
        MakeSubscript(MakeParamRef(fn, "arr"), MakeIntLiteral(2)));
    FunctionSetBody(fn, body);
        return MakeIntLiteral(0);
}
subscript_test();
...
Node *comma_test(void) {
    ...
    Node *body = MakeReturn(
        MakeComma(MakeIntLiteral(0), MakeIntLiteral(42)));
    FunctionSetBody(fn, body);
        return MakeIntLiteral(0);
}
comma_test();
...
// Provide definition via MakeFunction — returns the same Obj (params already set).
