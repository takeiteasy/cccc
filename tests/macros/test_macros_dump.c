// Test ticket #58: __builtin_dump_tree / __builtin_dump_ast_gen (print +
// to_string forms). The _to_string variants return a string we can inspect at
// compile time. Verified by checking the returned strings contain expected
// substrings.
...
int main(void) {
    ...
    if (!contains(gs, "__builtin_ast_int_literal"))
        return 3;

    // gen_string for a binary expression
    const char *bgs = gen_string(1 + 2);
    if (!contains(bgs, "__builtin_ast_binary"))
        return 4;
