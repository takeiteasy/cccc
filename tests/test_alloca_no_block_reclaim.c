// CCCC_FLAGS: -O0
// CCCC_MATRIX_SKIP: address-identity assertion, sensitive to bytecode reordering under an individual -f pass
// #981: a bare __builtin_alloca in the SAME block as a VLA declaration must
// NOT be reclaimed at that block's exit -- only the VLA's own storage
// (ALLOC_KIND_FRAME) dies at block exit; a bare alloca (ALLOC_KIND_ALLOCA)
// lives until the *frame* returns, matching every mainstream compiler's
// alloca semantics. This is the load-bearing regression test for that
// split: reusing one AllocKind for both (as ALCA alone did before this
// ticket) would have HREL sweep the still-live alloca'd block too,
...
int main(void) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        u[0] = i;
        char *p = __builtin_alloca(8); // ALLOC_KIND_ALLOCA -- must survive this block's HREL
        p[0] = (char)(i + 1);
        ptrs[i] = p;
    }
