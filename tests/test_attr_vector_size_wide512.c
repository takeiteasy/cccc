...
int main(void) {
    ...
    v16sf sel = cmp ? a : b;
    if (sel[0] != 1.0f)
        return 9;  // a[0]=1 < 5 -> then-arm
    if (sel[15] != 160.0f)
        return 10; // a[15]=16 not< 5 -> else-arm
    ...
    v8di vl;
    for (int i = 0; i < 8; i++)
        vl[i] = 1000000000000LL + i;
    v8di vl2 = vl + vl;
    if (vl2[0] != 2000000000000LL)
        return 22;
    if (vl2[7] != 2000000000014LL)
        return 23;
