...
int main(void) {
    ...
    v4si eq = (a == b);
    if (eq[0] != 0 || eq[1] != -1 || eq[2] != 0 || eq[3] != -1)
        return 1;
    v4si ne = (a != b);
    if (ne[0] != -1 || ne[1] != 0 || ne[2] != -1 || ne[3] != 0)
        return 2;
    v4si lt = (a < b);
    if (lt[0] != -1 || lt[1] != 0 || lt[2] != 0 || lt[3] != 0)
        return 3;
    v4si le = (a <= b);
    if (le[0] != -1 || le[1] != -1 || le[2] != 0 || le[3] != -1)
        return 4;
    v4si gt = (a > b);
    if (gt[0] != 0 || gt[1] != 0 || gt[2] != -1 || gt[3] != 0)
        return 5;
    v4si ge = (a >= b);
    if (ge[0] != 0 || ge[1] != -1 || ge[2] != -1 || ge[3] != -1)
        return 6;
    ...
    v4su ult =
        (ua < ub); // 1 < 0xFFFFFFFF: true; 2<2: false; 3<1: false; 4<5: true
    if (ult[0] != -1)
        return 9;
    if (ult[1] != 0)
        return 10;
    ...
    if (uge[1] != -1)
        return 14;         // 2<=2 -> 2>=2 true
    if (uge[0] != 0)
        return 15;         // 1 >= 0xFFFFFFFF: false
