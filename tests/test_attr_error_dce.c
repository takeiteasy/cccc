...
//   - runtime-length / unknown-pointer FORTIFY wrapper  (unsigned tautology
//   tier)
...
void __chk_fail(
    void) { /* body needed for link; never called at runtime (dead branches) */
}
...
void test_error_fixed_buffer_overflow_check(void) {
    ...
    char buf[64];
    if (10 > __builtin_object_size(buf, 0))
        __chk_fail();
}
...
// returns unsigned long SIZE_MAX for an unknown/parameter pointer (type 0
// fallback). The condition becomes: (unsigned long)len > UINT64_MAX — always
// false, so the branch is statically dead and the error is suppressed.
...
void test_error_elvis_dead_b(void) {
    // 1 ?: chk_i() — b is never evaluated (a is truthy, so tmp?tmp:b takes
    // tmp).
    (void)(1 ?: __chk_fail_i());
}
