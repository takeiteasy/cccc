...
int build_main(Builder *ctx) {
    ...
    if (MkDir(ctx, "/tmp/cccc_test_fsops/subdir") != 0)
        return 1;
    if (!DirExists(ctx, "/tmp/cccc_test_fsops/subdir"))
        return 1;
    ...
    if (WriteFile(ctx, src, "hello cccc\n") != 0)
        return 1;
    if (!FileExists(ctx, src))
        return 1;

    // CopyFile.
    const char *copy = "/tmp/cccc_test_fsops/copy.txt";
    if (CopyFile(ctx, src, copy) != 0)
        return 1;
    if (!FileExists(ctx, copy))
        return 1;
    ...
    if (MoveFile(ctx, copy, moved) != 0)
        return 1;
    if (!FileExists(ctx, moved))
        return 1;
    if (FileExists(ctx, copy))
        return 1; // old path must be gone

    // DeleteFile.
    if (DeleteFile(ctx, moved) != 0)
        return 1;
    if (FileExists(ctx, moved))
        return 1;

    // DeleteDir removes the whole tree.
    if (DeleteDir(ctx, tmpdir) != 0)
        return 1;
    if (DirExists(ctx, tmpdir))
        return 1;
