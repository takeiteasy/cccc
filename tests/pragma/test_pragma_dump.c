// Test ticket #58: jcc_dump_tree / jcc_dump_ast_gen (print + to_string forms).
...
#pragma macro
JCC_Node *dump_and_pass(JCC_Node *n) {
    JCC *vm = jcc_get_vm();
...
#pragma macro
JCC_Node *tree_string(JCC_Node *n) {
    JCC *vm = jcc_get_vm();
    const char *s = jcc_dump_tree_to_string(vm, n);
...
#pragma macro
JCC_Node *gen_string(JCC_Node *n) {
    JCC *vm = jcc_get_vm();
    const char *s = jcc_dump_ast_gen_to_string(vm, n);
    return jcc_ast_string_literal(vm, s);
}
...
int main(void) {
    ...
    if (!contains(gs, "jcc_ast_int_literal")) return 3;

    // gen_string for a binary expression
    const char *bgs = gen_string(1 + 2);
    if (!contains(bgs, "jcc_ast_binary")) return 4;
