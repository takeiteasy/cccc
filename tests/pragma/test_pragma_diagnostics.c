// Test ticket #78: jcc_warning_at emits a source-located warning.
...
#pragma macro
JCC_Node *warn_if_zero(JCC_Node *n) {
    JCC *vm = jcc_get_vm();
    // Always emit a warning — we're testing the mechanism, not the logic
    jcc_warning_at(vm, n, "warn_if_zero: inspecting argument (test warning)");
    return n;
}
