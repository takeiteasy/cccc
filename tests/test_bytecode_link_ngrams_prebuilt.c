// CCCC_FLAGS: --build
// CCCC_EXPECT_STDOUT: ngrams:.*--link is not supported when running a prebuilt
// \.c4 file.*exit=1.*fusion:.*--link is not supported when running a prebuilt
// \.c4 file.*exit=1
//
...
int build_main(Builder *ctx) {
    ...
    snprintf(lib_out, sizeof(lib_out), "%s/lib/mathlib_link_ngrams_test.c4a",
             out_dir);
    snprintf(app_out, sizeof(app_out), "%s/bin/app_link_ngrams_test.c4",
             out_dir);
    ...
    printf("fusion:\n");
    snprintf(cmd, sizeof(cmd),
             "sh -c './cccc %s --link %s --fusion-candidates=10 -0 -V 2>&1; "
             "echo \"exit=$?\"'",
             app_out, lib_out);
