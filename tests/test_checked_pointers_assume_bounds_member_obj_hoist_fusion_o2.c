...
// assignment-time bounds implication: `arr[k].p = s.p;` hoists `arr[k]`
// into a single temp for the post-store dst_lo/dst_hi read, rather than
// test_checked_pointers_assume_bounds_fusion_o2.c's trivial (bare-variable)
// target. This proves the target's own-bounds-not-implied trap through a
// runtime-indexed array-of-structs target still survives -O2.
