...
static v4sf scale2(v4sf v) {
    v4sf two;
    two[0] = 2.0f; two[1] = 2.0f; two[2] = 2.0f; two[3] = 2.0f;
    return v * two;
}
...
static float sum4_after8(int a, int b, int c, int d, int e, int f, int g,
                          int h, v4sf v) {
...
int main(void) {
    ...
    if (sum4(a) != 10.0f) return 17; // 'a' must be unaffected by the above
