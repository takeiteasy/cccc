...
// Source tests: test_const_comprehensive, test_const_correct_search, test_const_pointer_to_const, test_const_ptr, test_funcptr_callbacks, test_funcptr_comprehensive, test_funcptr_simple, test_pointers, test_restrict_derived_locals, test_restrict_indexed_const, test_restrict_memcpy_loop, test_restrict_memcpy_types, test_restrict_noalias_opt
...
static int derived_addr_taken(int *restrict p) {
    ...
    *p = 0;         // modifies p[0], not p[1]
    return a + **r; // a and **r both == p[1], tc_restrict_derived_locals_result == 2 * p[1]
}
