// CCCC_FLAGS: --testing
// Consolidated suite: initializers: designated, compound literals, compound
// assign Source tests: test_compound_assign_comprehensive,
// test_compound_literals, test_designated_array_simple, test_designated_nested,
// test_designated_struct_simple, test_designated_summary,
// test_indexed_store_call_index, test_init

// [from test_compound_assign_comprehensive]
// Comprehensive test for compound assignment operators
// Tests: Edge cases, arrays, pointers, struct members, globals, and complex
// expressions Returns: 42
...
struct tc_union_multi_desig_Wrap {
    union tc_union_multi_desig_U u;
    int                          tag;
};
...
int test_designated_array_simple(void) {
    ...
    if (arr1[0] != 0)
        return 1; // Should be zero-initialized
    if (arr1[1] != 0)
        return 2; // Should be zero-initialized
    if (arr1[2] != 42)
        return 3; // Our designated value
    if (arr1[3] != 0)
        return 4; // Should be zero-initialized
    if (arr1[4] != 0)
        return 5; // Should be zero-initialized
...
int test_designated_struct_simple(void) {
    ...
    if (r1.width != 100)
        return 5;
    if (r1.height != 0)
        return 6; // Should be zero-initialized
    if (r1.depth != 0)
        return 7; // Should be zero-initialized

    // Test 4: Only last field
    struct tc_designated_struct_simple_Rect r2 = {.depth = 50};

    if (r2.width != 0)
        return 8; // Should be zero-initialized
    if (r2.height != 0)
        return 9; // Should be zero-initialized
    if (r2.depth != 50)
        return 10;
...
int test_designated_summary(void) {
    ...
    struct tc_designated_summary_Rect r2 = {
        .tl.x = 10, .tl.y = 20, .br.x = 30, .br.y = 40};
    if (r2.tl.x == 10 && r2.tl.y == 20 && r2.br.x == 30 && r2.br.y == 40) {
        result += 8; // total: 150
    }

    if (result != 150)
        return 1; // Assert result == 150
    return 42;
}
...
int test_anon_union_designator_960(void) {
    ...
    struct tc_anon_union_desig_S r =
        (struct tc_anon_union_desig_S){.i = 7, .tag = 1};
    if (r.i != 7 || r.tag != 1)
        return 1;
    ...
    if (tc_anon_union_desig_global.i != 7 ||
        tc_anon_union_desig_global.tag != 1)
        return 4;
...
int test_union_multi_designator_962(void) {
    ...
    union tc_union_multi_desig_U d = {
        .i = 1,
        .l = 42,
    };
    if (d.l != 42)
    ...
    if (tc_union_multi_desig_global.l != 99)
        return 6;
