// CCCC_FLAGS: --testing
// Consolidated suite: comparisons, logical operators, ternary, unsigned comparison
// Source tests: test_comparisons, test_cmp, test_cmp_lit, test_logical, test_ternary, test_ult_ule
...
void test_ternary(void) {
    int a = 1 ? 10 : 20;   AssertEq(a, 10);
    int b = 0 ? 10 : 20;   AssertEq(b, 20);
    int c = 1 ? (0 ? 1 : 2) : 3;  AssertEq(c, 2);
    int x = 5, y = 10;
    int max = (x > y) ? x : y;    AssertEq(max, 10);
    int result = 2 + (x < y ? 5 : 3);  AssertEq(result, 7);
    int grade = 85;
