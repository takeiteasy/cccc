...
// Source tests: test_vla_basic, test_vla_cleanup, test_vla_comprehensive,
// test_vla_enhancements, test_vla_expr, test_vla_nested, test_vla_param,
// test_vla_ultra_minimal,
//   test_vla_simple, test_vla_minimal, test_vla_init, test_vla_cleanup_simple,
//   test_vla_with_constructor
...
// Ticket: #413 — void f(int n, int a[n]) previously errored "undefined variable
// 'n'" Expected return: 42
...
int test_vla_with_constructor(void) {
    ...
    static int vla_sink(char *p, int n) {
        p[0] = (char)n;
        return p[0];
    }
    int  n = 8;
...
int test_vla_2d_brace_init_ragged(void) {
    int n = 2, m = 2;
    int v[n][m] = {{1, 2}, {3}};
    return v[0][0] == 1 && v[0][1] == 2 && v[1][0] == 3 && v[1][1] == 0 ? 42
                                                                        : 1;
}
...
int test_vla_2d_brace_init_short(void) {
    int n = 2, m = 2;
    int v[n][m] = {{1, 2}};
    return v[0][0] == 1 && v[0][1] == 2 && v[1][0] == 0 && v[1][1] == 0 ? 42
                                                                        : 1;
}
