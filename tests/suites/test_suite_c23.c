// CCCC_FLAGS: --testing
// Consolidated suite: C23: _BitInt, auto, nullptr, constexpr, char8_t, embed,
// literals Source tests: test_c23_attributes, test_c23_auto, test_c23_bitint,
// test_c23_bitint_wide,
//   test_c23_bitint_wide_inline_binop_operands,
//   test_c23_bitint_wide_operand_chain_pressure, test_c23_bitint_wide_safety,
//   test_c23_bitint_wide_global_init,
//   test_c23_bool_keywords, test_c23_bool_stdbool_compat, test_c23_char8,
//   test_c23_decimal, test_c23_empty_params_void_equiv, test_c23_exp10_pi,
...
//   test_has_feature, test_std_c23_binary_literal_ok,
//   test_std_c23_digit_separator_ok, test_std_c23_memlib,
//   test_c23_label_before_decl
//
// Deferred (file-scope OR non-recoverable compile errors — error_tok() aborts
// the whole
...
//   test_c23_duplicate_enum_constant_error,
//   test_c23_incompatible_{enum,struct,union}_redecl_error,
...
// Test attributes on functions (attributes after return type, like
// __attribute__)
...
// Test C23 _BitInt(N) for N in (64, 65535] — wide multi-word storage (#401,
// #454)
...
int test_c23_bitint_wide(void) {
    ...
    if (!(sneg1 == sneg2))
        return 17;
    if (!(sneg1 < (_BitInt(96))0))
        return 18;
    unsigned _BitInt(96) uneg = (unsigned _BitInt(96))(-1);
    if (uneg < (unsigned _BitInt(96))0)
        return 19;
    ...
    if (big1 / big2 != ((unsigned _BitInt(150))1 << 50))
        return 28;
    ...
    if (uneg512 < (unsigned _BitInt(512))0)
        return 48;
    ...
    if ((long long)fromll65535 != 123456)
        return 51;
    double d65535             = 7.0;
    _BitInt(65535) fromd65535 = (_BitInt(65535))d65535;
    if (fromd65535 != 7)
        return 52;
    if ((double)fromd65535 != 7.0)
        return 53;
    ...
    _BitInt(96) negval = -1;
    if ((negval >> 1) != -1)
        return 60; // arithmetic: sign-extends
    unsigned _BitInt(96) unegval = (unsigned _BitInt(96))(-1);
    if ((unegval >> 1) >= unegval)
        return 61; // logical: shifts in zero
...
// #1122: file-scope initializers of a _BitInt(>128), which is VM-only
// (refused at type-emission under -c=native/-m/-c=generated by #1121's
// serialize_type check -- see man/COVERAGE.md's serialized-output
// divergences section) -- so this lives here, off the native corpus,
// rather than in test_suite_typesystem.c's test_wide_global_init.
// write_gvar_data used to crash on any global needing a >8-byte scalar
// write regardless of width; these two exercise it well past the
// __int128-sized case that #1121 already prepared serialize_init_bytes
// for.
static _BitInt(256) g_bi256_1122 = 123456789012345678901234567890wb * 2;
static unsigned _BitInt(4096) g_bi4096_1122 = (unsigned _BitInt(4096))1 << 4000;

// test_c23_bitint_wide_global_init
[[cccc::test(return = 42)]]
int test_c23_bitint_wide_global_init(void) {
    _BitInt(256) l_bi256 = 123456789012345678901234567890wb * 2;
    if (g_bi256_1122 != l_bi256)
        return 1;

    unsigned _BitInt(4096) l_bi4096 = (unsigned _BitInt(4096))1 << 4000;
    if (g_bi4096_1122 != l_bi4096)
        return 2;
...
int test_c23_decimal(void) {
    ...
    if (sizeof(_Decimal32) != 4)
        return 1;
    if (sizeof(_Decimal64) != 8)
        return 2;
    if (sizeof(_Decimal128) != 16)
        return 3;
    ...
    if (sizeof(struct S) != 16)
        return 4; // 8-byte align pads after `c`
    ...
    if (a + b != 3.3dd)
        return 10; // false for every binary-FP representation
    if (0.1dd + 0.2dd != 0.3dd)
        return 11; // the load-bearing exactness check
    if (!(a < b) || (b <= a) || !(b > a) || (a >= b))
        return 12;
    if (-a + a != 0.dd)
        return 13;
    ...
    if ((int)3.9dd != 3)
        return 16; // truncating, C semantics
    if ((_Decimal64)7 != 7.dd)
        return 17;
    if ((double)0.5dd != 0.5)
        return 18;
...
int test_c23_exp10_pi(void) {
    ...
    if (exp10(0.0) != 1.0)
        return 1;
    if (fabs_d(exp10(2.0) - 100.0) > 1e-9)
        return 2;
    if (fabs_d((double)exp10l(2.0L) - 100.0L) > 1e-9)
        return 3;
    ...
    if (fabs_d(asinpi(1.0) - 0.5) > 1e-9)
        return 16;
    if (fabs_d(acospi(-1.0) - 1.0) > 1e-9)
        return 17;
    if (fabs_d(atanpi(1.0) - 0.25) > 1e-9)
        return 18;
    if (fabs_d(atan2pi(1.0, 1.0) - 0.25) > 1e-9)
        return 19;
    if (fabs_d(atan2pi(1.0, 0.0) - 0.5) > 1e-9)
        return 20;
    if (fabs_d((double)atanpil(1.0L) - 0.25L) > 1e-9)
        return 22;
...
int test_c23_mbrtoc8(void) {
    ...
    if (c8 != 0xE2)
        return 14;
    rc = mbrtoc8(&c8, "", 0, &st);
    if (rc != (size_t)-3 || c8 != 0x82)
        return 15;
    rc = mbrtoc8(&c8, "", 0, &st);
    if (rc != (size_t)-3 || c8 != 0xAC)
        return 16;
    ...
    if (c8 != 0xF0)
        return 18;
    rc = mbrtoc8(&c8, "", 0, &st);
    if (rc != (size_t)-3 || c8 != 0x9F)
        return 19;
    rc = mbrtoc8(&c8, "", 0, &st);
    if (rc != (size_t)-3 || c8 != 0x98)
        return 20;
    rc = mbrtoc8(&c8, "", 0, &st);
    if (rc != (size_t)-3 || c8 != 0x80)
        return 21;
    ...
    char buf[8];
    memset(&st, 0, sizeof(st));
    if (c8rtomb(buf, 0xE2, &st) != 0)
        return 24;
    if (c8rtomb(buf, 0x82, &st) != 0)
        return 25;
    rc = c8rtomb(buf, 0xAC, &st);
    if (rc == 0 || rc == (size_t)-1)
        return 26;
    if (memcmp(buf, "\xE2\x82\xAC", rc) != 0)
        return 27;

    // c8rtomb round trip for the 4-byte GRINNING FACE sequence.
    memset(&st, 0, sizeof(st));
    if (c8rtomb(buf, 0xF0, &st) != 0)
        return 28;
    if (c8rtomb(buf, 0x9F, &st) != 0)
        return 29;
    if (c8rtomb(buf, 0x98, &st) != 0)
        return 30;
    rc = c8rtomb(buf, 0x80, &st);
    if (rc == 0 || rc == (size_t)-1)
        return 31;
    if (memcmp(buf, "\xF0\x9F\x98\x80", rc) != 0)
        return 32;

    // c8rtomb with an invalid lead byte should report EILSEQ.
    memset(&st, 0, sizeof(st));
    if (c8rtomb(buf, 0xFF, &st) != (size_t)-1)
        return 33;
...
int test_c23_wb_suffix(void) {
    ...
    a            = a + 1wb;
    if (a != -128)
        return 9; // signed overflow wraps

    unsigned _BitInt(5) b = 31uwb;
    b                     = b + 1uwb;
    if (b != 0)
        return 10; // unsigned overflow wraps
    ...
    // Decimal literal needing 97/98 bits
    // (bit_length(123456789012345678901234567890) == 97).
    if (sizeof(123456789012345678901234567890wb) != 16)
        return 13; // width 98 (97 value bits + sign)
    if (sizeof(123456789012345678901234567890uwb) != 16)
        return 14; // width 97
    ...
    if ((unsigned long long)big_signed != 14083847773837265618ULL)
        return 15;
    if ((unsigned long long)(big_signed >> 64) != 6692605942ULL)
        return 16;
    if ((unsigned long long)big_unsigned != 14083847773837265618ULL)
        return 17;
    if ((unsigned long long)(big_unsigned >> 64) != 6692605942ULL)
        return 18;

    // Hex literal needing 65 bits (0x1FFFFFFFFFFFFFFFF == 2^65 - 1) —
    // exercises the base-16 fast width-inference path.
    if (sizeof(0x1FFFFFFFFFFFFFFFFuwb) != 16)
        return 19; // width 65
    if (sizeof(0x1FFFFFFFFFFFFFFFFwb) != 16)
        return 20; // width 66 (sign bit)
    unsigned _BitInt(65) hex_unsigned = 0x1FFFFFFFFFFFFFFFFuwb;
    _BitInt(66) hex_signed            = 0x1FFFFFFFFFFFFFFFFwb;
    if ((unsigned long long)hex_unsigned != 0xFFFFFFFFFFFFFFFFULL)
        return 21;
    if ((unsigned long long)(hex_unsigned >> 64) != 1ULL)
        return 22;
    if ((unsigned long long)hex_signed != 0xFFFFFFFFFFFFFFFFULL)
        return 23;
    if ((unsigned long long)(hex_signed >> 64) != 1ULL)
        return 24;
    ...
    if (sizeof(bin_unsigned) != 16)
        return 25;
    if ((unsigned long long)bin_unsigned != 0ULL)
        return 26;
    if ((unsigned long long)(bin_unsigned >> 64) != 1ULL)
        return 27;
    ...
    if (sizeof(9223372036854775808uwb) != 8)
        return 28;
    if (sizeof(9223372036854775808wb) != 16)
        return 29;
...
int test_constexpr_basic(void) {
    ...
    int local_purrs[N] = {0};
    if (sizeof(purrs) != 9 * sizeof(int))
        return 1;
    if (sizeof(local_purrs) != 10 * sizeof(int))
        return 2;
    if (sizeof(local_arr) != 3 * sizeof(int))
        return 3;
    if (global_from_constexpr != 9)
        return 4;
    return 42;
}
...
int test_std_c23_stdbit(void) {
    /* ---- stdc_leading_zeros ---- */
    if (stdc_leading_zeros_uc((unsigned char)0) != 8)
        return 1;
    if (stdc_leading_zeros_uc((unsigned char)1) != 7)
        return 2;
    if (stdc_leading_zeros_uc((unsigned char)0x80) != 0)
        return 3;
    if (stdc_leading_zeros_us((unsigned short)0) != 16)
        return 4;
    if (stdc_leading_zeros_us((unsigned short)1) != 15)
        return 5;
    if (stdc_leading_zeros_ui(0u) != 32)
        return 6;
    if (stdc_leading_zeros_ui(1u) != 31)
        return 7;
    if (stdc_leading_zeros_ull(0ull) != 64)
        return 8;
    if (stdc_leading_zeros_ull(1ull) != 63)
        return 9;
    if (stdc_leading_zeros_ul(1ul) != 63)
        return 10;

    /* ---- stdc_trailing_zeros ---- */
    if (stdc_trailing_zeros_uc((unsigned char)0) != 8)
        return 11;
    if (stdc_trailing_zeros_uc((unsigned char)8) != 3)
        return 12;
    if (stdc_trailing_zeros_us((unsigned short)0) != 16)
        return 13;
    if (stdc_trailing_zeros_us((unsigned short)8) != 3)
        return 14;
    if (stdc_trailing_zeros_ui(0u) != 32)
        return 15;
    if (stdc_trailing_zeros_ui(8u) != 3)
        return 16;
    if (stdc_trailing_zeros_ull(0ull) != 64)
        return 17;
    if (stdc_trailing_zeros_ull(8ull) != 3)
        return 18;

    /* ---- stdc_leading_ones ---- */
    if (stdc_leading_ones_uc((unsigned char)0xFF) != 8)
        return 19;
    if (stdc_leading_ones_uc((unsigned char)0xF0) != 4)
        return 20;
    if (stdc_leading_ones_uc((unsigned char)0) != 0)
        return 21;
    if (stdc_leading_ones_us((unsigned short)0xFFFF) != 16)
        return 22;
    if (stdc_leading_ones_us((unsigned short)0xFF00) != 8)
        return 23;
    if (stdc_leading_ones_ui(0xFFFFFFFFu) != 32)
        return 24;
    if (stdc_leading_ones_ui(0xF0000000u) != 4)
        return 25;
    if (stdc_leading_ones_ull(0xFFFFFFFFFFFFFFFFull) != 64)
        return 26;

    /* ---- stdc_trailing_ones ---- */
    if (stdc_trailing_ones_uc((unsigned char)0xFF) != 8)
        return 27;
    if (stdc_trailing_ones_uc((unsigned char)0x0F) != 4)
        return 28;
    if (stdc_trailing_ones_uc((unsigned char)0) != 0)
        return 29;
    if (stdc_trailing_ones_ui(0xFFFFFFFFu) != 32)
        return 30;
    if (stdc_trailing_ones_ui(7u) != 3)
        return 31;
    if (stdc_trailing_ones_ull(0xFFFFFFFFFFFFFFFFull) != 64)
        return 32;

    /* ---- stdc_count_ones ---- */
    if (stdc_count_ones_uc((unsigned char)0) != 0)
        return 33;
    if (stdc_count_ones_uc((unsigned char)0xFF) != 8)
        return 34;
    if (stdc_count_ones_us((unsigned short)0xFFFF) != 16)
        return 35;
    if (stdc_count_ones_ui(0u) != 0)
        return 36;
    if (stdc_count_ones_ui(0xFFu) != 8)
        return 37;
    if (stdc_count_ones_ull(0xFFFFFFFFFFFFFFFFull) != 64)
        return 38;

    /* ---- stdc_count_zeros ---- */
    if (stdc_count_zeros_uc((unsigned char)0xFF) != 0)
        return 39;
    if (stdc_count_zeros_uc((unsigned char)0) != 8)
        return 40;
    if (stdc_count_zeros_uc((unsigned char)0xF0) != 4)
        return 41;
    if (stdc_count_zeros_us((unsigned short)0) != 16)
        return 42;
    if (stdc_count_zeros_ui(0u) != 32)
        return 43;
    if (stdc_count_zeros_ui(0xFFFFFFFFu) != 0)
        return 44;
    if (stdc_count_zeros_ull(0xFFFFFFFFFFFFFFFFull) != 0)
        return 45;
    if (stdc_count_zeros_ull(0ull) != 64)
        return 46;

    /* ---- stdc_bit_width ---- */
    if (stdc_bit_width_uc((unsigned char)0) != 0)
        return 47;
    if (stdc_bit_width_uc((unsigned char)1) != 1)
        return 48;
    if (stdc_bit_width_uc((unsigned char)255) != 8)
        return 49;
    if (stdc_bit_width_us((unsigned short)0) != 0)
        return 50;
    if (stdc_bit_width_us((unsigned short)256) != 9)
        return 51;
    if (stdc_bit_width_ui(0u) != 0)
        return 52;
    if (stdc_bit_width_ui(1u) != 1)
        return 53;
    if (stdc_bit_width_ui(5u) != 3)
        return 54;
    if (stdc_bit_width_ull(0ull) != 0)
        return 55;
    if (stdc_bit_width_ull(0x100000000ull) != 33)
        return 56;
    if (stdc_bit_width_ul(0x100000000ul) != 33)
        return 57;

    /* ---- stdc_has_single_bit ---- */
    if (stdc_has_single_bit_uc((unsigned char)0))
        return 58;
    if (!stdc_has_single_bit_uc((unsigned char)1))
        return 59;
    if (!stdc_has_single_bit_uc((unsigned char)128))
        return 60;
    if (stdc_has_single_bit_uc((unsigned char)3))
        return 61;
    if (stdc_has_single_bit_ui(0u))
        return 62;
    if (!stdc_has_single_bit_ui(1u))
        return 63;
    if (!stdc_has_single_bit_ui(8u))
        return 64;
    if (stdc_has_single_bit_ui(6u))
        return 65;
    if (!stdc_has_single_bit_ull(0x100000000ull))
        return 66;

    /* ---- stdc_bit_floor ---- */
    if (stdc_bit_floor_uc((unsigned char)0) != 0)
        return 67;
    if (stdc_bit_floor_uc((unsigned char)5) != 4)
        return 68;
    if (stdc_bit_floor_uc((unsigned char)128) != 128)
        return 69;
    if (stdc_bit_floor_ui(0u) != 0)
        return 70;
    if (stdc_bit_floor_ui(1u) != 1)
        return 71;
    if (stdc_bit_floor_ui(5u) != 4)
        return 72;
    if (stdc_bit_floor_ui(8u) != 8)
        return 73;
    if (stdc_bit_floor_ull(0x180000000ull) != 0x100000000ull)
        return 74;

    /* ---- stdc_bit_ceil ---- */
    if (stdc_bit_ceil_uc((unsigned char)0) != 1)
        return 75;
    if (stdc_bit_ceil_uc((unsigned char)5) != 8)
        return 76;
    if (stdc_bit_ceil_uc((unsigned char)128) != 128)
        return 77;
    if (stdc_bit_ceil_uc((unsigned char)129) != 0)
        return 78; /* unrepresentable */
    if (stdc_bit_ceil_ui(0u) != 1)
        return 79;
    if (stdc_bit_ceil_ui(1u) != 1)
        return 80;
    if (stdc_bit_ceil_ui(5u) != 8)
        return 81;
    if (stdc_bit_ceil_ui(8u) != 8)
        return 82;
    if (stdc_bit_ceil_ui(0x80000000u) != 0x80000000u)
        return 83;
    if (stdc_bit_ceil_ui(0x80000001u) != 0)
        return 84; /* unrepresentable */
    if (stdc_bit_ceil_ull(0x180000000ull) != 0x200000000ull)
        return 85;

    /* ---- stdc_first_leading_one ---- */
    if (stdc_first_leading_one_uc((unsigned char)0) != 0)
        return 86;
    if (stdc_first_leading_one_uc((unsigned char)0x80) != 1)
        return 87;
    if (stdc_first_leading_one_uc((unsigned char)0x40) != 2)
        return 88;
    if (stdc_first_leading_one_uc((unsigned char)1) != 8)
        return 89;
    if (stdc_first_leading_one_ui(0u) != 0)
        return 90;
    if (stdc_first_leading_one_ui(1u) != 32)
        return 91;
    if (stdc_first_leading_one_ui(0x80000000u) != 1)
        return 92;
    if (stdc_first_leading_one_ull(0ull) != 0)
        return 93;
    if (stdc_first_leading_one_ull(1ull) != 64)
        return 94;

    /* ---- stdc_first_leading_zero ---- */
    if (stdc_first_leading_zero_uc((unsigned char)0xFF) != 0)
        return 95;
    if (stdc_first_leading_zero_uc((unsigned char)0x7F) != 1)
        return 96;
    if (stdc_first_leading_zero_uc((unsigned char)0) != 1)
        return 97;
    if (stdc_first_leading_zero_ui(0xFFFFFFFFu) != 0)
        return 98;
    if (stdc_first_leading_zero_ui(0u) != 1)
        return 99;
    if (stdc_first_leading_zero_ull(0xFFFFFFFFFFFFFFFFull) != 0)
        return 100;

    /* ---- stdc_first_trailing_one ---- */
    if (stdc_first_trailing_one_uc((unsigned char)0) != 0)
        return 101;
    if (stdc_first_trailing_one_uc((unsigned char)1) != 1)
        return 102;
    if (stdc_first_trailing_one_uc((unsigned char)8) != 4)
        return 103;
    if (stdc_first_trailing_one_ui(0u) != 0)
        return 104;
    if (stdc_first_trailing_one_ui(8u) != 4)
        return 105;
    if (stdc_first_trailing_one_ull(0ull) != 0)
        return 106;
    if (stdc_first_trailing_one_ull(8ull) != 4)
        return 107;

    /* ---- stdc_first_trailing_zero ---- */
    if (stdc_first_trailing_zero_uc((unsigned char)0xFF) != 0)
        return 108;
    if (stdc_first_trailing_zero_uc((unsigned char)0xFE) != 1)
        return 109;
    if (stdc_first_trailing_zero_uc((unsigned char)0) != 1)
        return 110;
    if (stdc_first_trailing_zero_ui(0xFFFFFFFFu) != 0)
        return 111;
    if (stdc_first_trailing_zero_ui(0u) != 1)
        return 112;
    if (stdc_first_trailing_zero_ull(0xFFFFFFFFFFFFFFFFull) != 0)
        return 113;
    ...
    if (stdc_leading_zeros(uc) != 0)
        return 114;
    if (stdc_leading_zeros(us) != 0)
        return 115;
    if (stdc_leading_zeros(ui) != 0)
        return 116;
    if (stdc_leading_zeros(ul) != 0)
        return 117;
    if (stdc_leading_zeros(ull) != 0)
        return 118;

    if (stdc_count_ones(uc) != 4)
        return 119;
    if (stdc_count_zeros(uc) != 4)
        return 120;
    if (stdc_has_single_bit(ui) != 0)
        return 121;

    if (stdc_bit_width((unsigned char)5) != 3)
        return 122;
    if (stdc_bit_floor((unsigned int)5u) != 4u)
        return 123;
    if (stdc_bit_ceil((unsigned int)5u) != 8u)
        return 124;

    if (stdc_first_leading_one(uc) != 1)
        return 125;
    if (stdc_first_leading_zero((unsigned char)0x7F) != 1)
        return 126;
    if (stdc_first_trailing_one((unsigned char)8) != 4)
        return 127;
    if (stdc_first_trailing_zero((unsigned char)0xFE) != 1)
        return 128;
...
int test_std_c23_stdckdint(void) {
    ...
    // long long variant
    long long rll;
    if (ckd_add(&rll, 1ll, 2ll))
        return 10;
    if (rll != 3ll)
        return 11;
    if (!ckd_mul(&rll, (long long)LLONG_MAX, 2ll))
        return 12;
...
int test_std_c23_strtol_binary(void) {
    ...
    if (strtol("123", &end, 10) != 123)
        return 9;
    if (strtol("0x1F", &end, 0) != 31)
        return 10;
    if (strtol("017", &end, 0) != 15)
        return 11; // octal

    // strtoul / strtoull with 0b prefix
    if (strtoul("0b11", &end, 0) != 3)
        return 12;
    if (strtoull("0B11111111", &end, 0) != 255ull)
        return 13;

    // strtoll with 0b prefix
    if (strtoll("0b10000000000", &end, 0) != (1ll << 10))
        return 14;
...
[[cccc::test(return = 42)]]
int test_std_c23_binary_literal(void) {
    return 0b1010 == 10 ? 42 : 1;
}
...
// test_std_c23_memlib: memset_explicit, memchr, memalignment, free_sized,
// timegm
[[cccc::test(return = 42)]]
int test_std_c23_memlib(void) {
    char buf[8] = "abcdefg";
    memset_explicit(buf, 'X', 3);
    if (memcmp(buf, "XXXdefg", 7) != 0)
        return 1;
    if (memset_explicit(buf + 3, 'Y', 4) != buf + 3)
        return 2;
    if (memcmp(buf, "XXXYYYY", 8) != 0)
        return 3;
    ...
    if (memalignment(a16) < 16)
        return 7;
    void *a64 = aligned_alloc(64, 128);
    if (memalignment(a64) < 64)
        return 8;
...
// test_c23_compound_literal_inline_error,
// test_c23_compound_literal_typedef_error,
// test_constexpr_nonconstant_init_error: migrated to
// test_suite_compile_errors.c.
...
int test_c23_compound_literal_storage(void) {
    if (*cls_global_static != 11)
        return 1;
    *cls_global_static = 12;
    if (*cls_global_static != 12)
        return 2;
    if (cls_next_value() != 8)
        return 3;
    if (cls_next_value() != 9)
        return 4;
    if (cls_register_value() != 6)
        return 5;
    if (cls_register_value() != 6)
        return 6;
    if (cls_tls_value() != 13)
