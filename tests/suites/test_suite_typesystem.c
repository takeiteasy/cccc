...
static int int_func(int x) { return 1; }

static int long_func(long x) { return 2; }

static int float_func(float x) { return 3; }

static int double_func(double x) { return 4; }

static int ptr_func(int *x) { return 5; }

static int char_func(char x) { return 6; }

static int default_func(void *x) { return 99; }
...
static int print_int(int x) { return 1; }

static int print_long(long x) { return 2; }

static int print_double(double x) { return 3; }

static int print_char(char x) { return 4; }
...
int test_sizeof_expressions(void) {
    ...
    if (sizeof(arr) != 40) return 4;  // 10 * 4

    // sizeof with array elements
    if (sizeof(arr[0]) != 4) return 5;

    // sizeof with pointer arithmetic expressions
    if (sizeof(arr[5]) != 4) return 6;

    // sizeof with complex expressions
    if (sizeof(x + 10) != 4) return 7;
    if (sizeof(x * 2) != 4) return 8;

    // sizeof with dereferenced pointers
    int *ptr = &x;
    if (sizeof(*ptr) != 4) return 9;
    
    // sizeof with struct
    struct Point { int x; int y; };
    if (sizeof(struct Point) != 8) return 10;

    struct Point p;
    if (sizeof(p) != 8) return 11;
    if (sizeof(p.x) != 4) return 12;
