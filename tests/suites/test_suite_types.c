...
// [from test_serialize_typedef_roundtrip] — file-scope type aliases for -M
// regression
...
struct SerT_Box {
    struct SerT_Point origin;
    SerT_Size         size;
    union SerT_Value  value;
    SerT_Color        color;
};
...
static void use_params(int a, int b) {
    (void)a;
    (void)b;
}
...
int test_serialize_typedef_roundtrip(void) {
    ...
    SerTR_Int         n = 10;
    SerTR_TaggedAlias tagged;
    tagged.value = 5;
    SerTR_AnonStruct anon;
    anon.width             = 6;
    anon.height            = 7;
    SerTR_EnumAlias  color = SER_TR_GREEN;
    SerTR_UnionAlias uni;
    uni.i = 8;
    LocalPoint point;
    point.x           = 9;
    point.y           = 10;
