...
int build_main(Builder *ctx) {
    ...
    if (WriteFile(ctx, tmp, expected) != 0) return 1;
    ...
    if (!got || strcmp(got, expected) != 0) return 1;

    // NULL path/content should fail gracefully.
    if (WriteFile(ctx, (void*)0, "x") != -1) return 1;
    if (WriteFile(ctx, tmp, (void*)0) != -1) return 1;
