...
int main(void) {
    ...
    if (!(pos_inf > 1e300 && pos_inf == pos_inf)) return 1;
    // -Inf: smaller than any finite double, and equal to itself.
    if (!(neg_inf < -1e300 && neg_inf == neg_inf)) return 2;
    // NaN: not equal to itself.
    if (!(nan_val != nan_val)) return 3;
    // float +Inf.
    if (!(fpos_inf > 1e30f && fpos_inf == fpos_inf)) return 4;

    // Dividing by -0.0 flips the sign versus dividing by +0.0.
    double neg_zero_div = 1.0 / nzero;
    if (!(neg_zero_div < -1e300)) return 5;
    ...
    double r = 1.0 / pzero;
    (void)r;
    if (!(fetestexcept(FE_DIVBYZERO))) return 6;

    feclearexcept(FE_ALL_EXCEPT);
    double s = 0.0 / pzero;
    (void)s;
    if (!(fetestexcept(FE_INVALID))) return 7;
