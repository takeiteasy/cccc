...
- **Real IEEE-754-2008 decimal floating-point** — `_Decimal32/64/128` arithmetic and `<decimal_math.h>` transcendentals (`sqrtd64`, `powd128`, ...) via the Intel BID library; build with `make CCCC_HAS_DECIMAL=1` after running `tools/fetch_intel_bid.sh` (optional, never vendored; see [VM.md](docs/VM.md#decimal-floating-point))
