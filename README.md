...
- **Compile-time macros** — C functions annotated with `[[cccc::macro]]` or `__attribute__((macro))` that run during compilation (see [MACROS.md](docs/MACROS.md))
...
- **Built-in test framework** — `[[cccc::test]]` attribute and `CCCC_ASSERT*` macros for writing tests in C (see [TESTING.md](docs/TESTING.md))
  - Run with `--testing`; outputs TAP 13 format; no external dependencies or includes needed
- **Attribute support** — GNU `__attribute__((...))` and C23 `[[...]]` syntax with partial semantic support (see [COVERAGE.md](docs/COVERAGE.md#attributes))
