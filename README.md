...
- **Bytecode optimizer** — optional passes on generated bytecode (see [OPTIMIZATION.md](docs/OPTIMIZATION.md))
  - `--optimize[=N]` with levels 1–3: constant folding, peephole, scalar local promotion, indexed load/store lowering, and dead code elimination
...
- **Indexed load/store opcodes (#251)**: under `--optimize=2` and `--optimize=3`, simple `base + index * scale` array and pointer accesses lower to `LDR_INDEX_*` / `STR_INDEX_*` opcodes, removing the separate multiply/add address sequence on hot loops.
