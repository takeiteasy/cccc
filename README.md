...
- **Built-in test framework** — `[[jcc::test]]` attribute and `JCC_ASSERT*` macros for writing tests in C (see [TESTING.md](docs/TESTING.md))
...
- **Attribute support** — GNU `__attribute__((...))` and C23 `[[...]]` syntax with partial semantic support (see [COVERAGE.md](docs/COVERAGE.md#attributes))
  - Covers `packed`, `aligned`, `unused`/`maybe_unused`, `deprecated`, and JCC-specific `macro`/`comptime`/`test`
