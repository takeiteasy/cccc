...
static void ccccl_mangle(const char *lisp_name, char *out, int out_cap) {
    ...
    if (*s >= '0' && *s <= '9' && oi < out_cap - 1) out[oi++] = '_';
    for (; *s && oi < out_cap - 1; s++) {
        char c = *s;
        if (c >= 'A' && c <= 'Z') { out[oi++] = (char)(c - 'A' + 'a'); continue; }
        if (c >= 'a' && c <= 'z') { out[oi++] = c; continue; }
        if (c >= '0' && c <= '9') { out[oi++] = c; continue; }
        switch (c) {
        case '-': out[oi++] = '_'; break;
        case '?': if (oi < out_cap - 2) { out[oi++] = '_'; out[oi++] = 'p'; } break;
        case '!':
            if (oi < out_cap - 5) { memcpy(out + oi, "_bang", 5); oi += 5; }
            break;
        case '*':
            if (oi < out_cap - 5) { memcpy(out + oi, "_star", 5); oi += 5; }
            break;
        case '/':
            if (oi < out_cap - 6) { memcpy(out + oi, "_slash", 6); oi += 6; }
            break;
        case '>': if (oi < out_cap - 3) { memcpy(out + oi, "_gt", 3); oi += 3; } break;
        case '<': if (oi < out_cap - 3) { memcpy(out + oi, "_lt", 3); oi += 3; } break;
        case '=': if (oi < out_cap - 3) { memcpy(out + oi, "_eq", 3); oi += 3; } break;
        default: out[oi++] = '_'; break;
        }
    }
    out[oi] = '\0';
}
...
static int ccccl_intern_sym(CccclPlan *p, const char *name) {
    ...
    for (i = 0; i < p->sym_count; i++)
        if (strcmp(p->syms[i].text, name) == 0) return i;
    ...
    }
    {
        int idx = p->sym_count++;
        int i2;
        for (i2 = 0; i2 < CL_NAME_LEN && name[i2]; i2++) p->syms[idx].text[i2] = name[i2];
...
static CccclOp *ccccl_emit(CccclPlanFn *fn, CccclOpKind kind, int a, int b) {
    CccclOp *op;
    if (*fn->emit_count >= CL_MAX_OPS) return NULL; /* caller checks plan->has_error via ccccl_lower_error path */
...
static int ccccl_find_param(CccclPlanFn *fn, const char *name) {
    ...
    for (i = 0; i < fn->param_count; i++)
        if (strcmp(fn->param_lisp[i], name) == 0) return i;
    return -1;
}
...
static CccclPlanFn *ccccl_new_fn(CccclPlan *p, const char *lisp_name, int is_lambda) {
    CccclPlanFn *fn;
...
static void ccccl_lower_lambda(CccclPlan *p, CccclForm *params, CccclForm *body,
                                CccclPlanFn *out_fn_slot_hint, const char *label_name);

static void ccccl_lower_expr(CccclPlan *p, CccclPlanFn *fn, CccclForm *e) {
    if (p->has_error) return;

    if (e->kind == CL_FORM_ATOM) {
        if (strcmp(e->atom, "NIL") == 0) { ccccl_emit(fn, CL_OP_NIL, 0, 0); return; }
        if (strcmp(e->atom, "T") == 0) { ccccl_emit(fn, CL_OP_T, 0, 0); return; }
        {
            int param = ccccl_find_param(fn, e->atom);
            int sym = ccccl_intern_sym(p, e->atom);
            (void)param; /* lookup always goes through env at runtime (dynamic scoping) */
    ...
    /* e is a PAIR: (head ...rest) */
    {
        ...
        if (head->kind == CL_FORM_ATOM && strcmp(head->atom, "LABEL") == 0) {
            ...
            CccclForm *lambda_form = rest->cdr->car; /* (LAMBDA (params) body) */
 ...
 * reference by index. */
static void ccccl_lower_toplevel_body(CccclPlan *p, CccclForm *form, CccclPlanFn *fn) {
    if (p->has_error || !fn) return;
    CccclForm *sig = form->cdr->car;      /* (name params...) */
